...
- 空模型响应不属于 JSON 或 schema 修复问题。原生 JSON 模式返回空内容时，结构化运行时应先用同一模型、同一 Prompt 和同一 Schema 降级到 `prompt_json`；降级后仍为空，才按模型传输 / 输出失败进入有限重试和备用模型。repair 没有原始语义可保留，禁止用空对象补造必填业务内容。
