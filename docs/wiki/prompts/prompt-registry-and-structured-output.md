...
- 同一结构化结果中承担不同职责的字段必须在 `postValidate` 中保护其最小差异边界。比如章节摘要与章节目标在去除空白后完全相同时，目标输出无效：应把失败原因交给 `semanticRetryPolicy` 重新生成；重试仍失败时中止本次写入并保留已保存内容，不能让一个字段静默覆盖成另一个字段的副本。
