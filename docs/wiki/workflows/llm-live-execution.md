...
9. 实况窗口按一次 LLM 调用分组显示。新调用开始时，必须展开并聚焦最新调用；此前已完成、失败或取消的调用应自动收起，用户仍可手动展开查看预览。实况卡片标头需显示算力 Provider/Model 标识、累计运行耗时（如 `14.2s`）与实时生成速率（`字/秒`）。
10. “清空前台”只清除当前浏览器窗口已显示的调用会话，不得删除服务端临时会话、业务任务、保存结果或任务中心运行记录。清空后仅展示新开始的调用。
11. 针对结构化 JSON 返回内容，面板提供一键 Pretty JSON 格式化与高亮；同时支持日志全文搜索过滤、Input/Output 一键复制与悬浮窗全屏/最大化切换。
12. 自动导演、简易章节书架等长时运行主界面应提供按 `taskId` 订阅的就地实况入口；检测到当前任务的新模型会话时可以自动打开一次，用户关闭同一会话后不得因后续片段重复弹出。
13. 自动导演各规划阶段调用 Prompt 时必须沿调用链传递同一个导演任务 ID，并补充小说、阶段和步骤上下文。候选、故事宏观、角色、卷规划与章节生产不能因为跨服务调用而丢失实况归属。

## Failure Modes

- SSE 断开：只影响可视化订阅，服务端继续消费模型流和执行后续逻辑。
- 模型流失败：会话发出失败事件，记录并透传 `errorMessage` 与错误堆栈，供实况面板渲染错误细节警示框。
...
- `server/tests/unit/llmLiveBroker.test.js`：LlmLiveBroker 会话与截断单元测试。
- `server/tests/unit/llmLiveRoutes.test.js`：SSE 路由与框架分发集成测试。
