...
- “创作承接合同”是照着写开书页上的独立方案，不等于导演准备阶段的读者兑现 `BookContract`。合同由 Prompt Registry 资产 `novel.creative_carryover.contract@v1` 结构化生成，记录模式、来源拆书与原文版本、依据、原书特点、本书实现与前三章方向；续写侧重终局人物状态与未完线索，参考创作侧重钩子、冲突循环、爽点节奏与新书转换方案。
- 创作承接合同在生成时冻结 `documentVersionId` / `documentVersionNumber`；刷新、退出和任务恢复必须读合同内冻结版本，不得改读文档当前活动版本。合同先写入开书草稿与导演任务 `seedPayload.creativeCarryoverContract`，用户采用后，建书时写入小说可空字段 `creativeCarryoverContractJson` + `creativeCarryoverContractSchemaVersion`。
- 材料门槛是输入校验：续写至少需要可读的「人物系统」「剧情结构」；参考创作至少需要「剧情结构」「文风与技法」「商业化卖点」；原文版本正文必须仍可读取。材料不足时页面列出缺失结论并引导补做拆书，禁止静默降级成普通原创开书。普通原创不生成、不保存、不注入该合同。
- 开书页「帮我填写」读者与卖点时，若表单已绑定续写或参考拆书，必须把对应拆书结论送进 `novel.framing.suggest`；不能只根据“参考《某某》结构写法”这类元描述或书名关键词空猜卖点。参考创作只继承结构与阅读体验，续写必须承接终局与未完线索。
