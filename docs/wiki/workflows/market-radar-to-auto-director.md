...
市场雷达是显式触发的创作前置分析入口，不是质量复审，也不是自动导演的新运行阶段。它不拥有一套独立的题材与推进模式分类：AI 分析形成的创作方向必须解析到统一的题材基底库和推进模式库，后续开书流程只引用这些统一资产。进入页面先自动启动公开榜单采集并展示事实数据，这一步不得调用 LLM；只有用户明确点击“开始AI分析”后，才执行题材聚类、套路识别、拥挤度、机会判断和创作基础推荐。用户选定分析信号后生成持久化 `MarketCreativeBrief`，自动导演只接收简报 ID，并由服务端解析可信 Prompt 块与资源引用。
...
- AI 优先从本次提供的统一资源目录选择 `existingId`。服务端只接受目录中存在的 ID；AI 判断没有等价资产时，才在对应资源库建立稳定的根级资产。写入前还要按规范化名称复用已有项，避免重复分析制造同名资产。
- 分析报告保存完成后，`productionFoundation` 必须包含真实资源 ID。报告中的市场信号仍作为判断证据与创作约束保留，但不得形成雷达专属的题材库或推进模式库。
- 分析结果页必须明确展示创作基础的资源库同步状态，并提供题材基底库与推进模式库入口。市场信号卡只负责选择本次开书偏好，不提供逐条入库按钮，避免把标题句式、拥挤套路等非资源结论误建为长期资产。
...
- 报告已有推荐名称但开书页没有预填：检查报告同步后是否写入真实资源 ID、简报是否保存 `productionFoundation`，以及旧格式兼容解析是否把新对象误当成信号数组。
