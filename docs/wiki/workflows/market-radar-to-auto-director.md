...
- AI 优先从本次提供的统一资源目录选择 `existingId`。分析阶段只保存候选，不得创建资源或标记为已加入。用户点击加入后，服务端只接受目录中仍存在的 ID；AI 判断没有等价资产时，才建立稳定的根级资产。写入前还要按规范化名称复用已有项，避免重复确认制造同名资产。
- 题材基底与推进模式分别确认、分别记录真实资源 ID。只有服务端复用或创建成功后，界面才能显示“查看资源库”；资源库入口必须携带资源 ID 并定位到对应节点。
- 历史自动同步报告不得继续投影为已加入。若其引用的资源仍存在，应转换为待确认候选，用户点击后复用原资源并记录确认状态；资源已不存在时要求重新分析，不用残缺信息补造资产。
