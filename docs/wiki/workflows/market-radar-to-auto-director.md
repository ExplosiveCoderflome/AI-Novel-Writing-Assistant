...
- 页面进入时自动发起一次扫榜，采集结果按平台和榜单展示排名、书名、作者及分类；AI 分析范围和主操作必须位于原始榜单列表之前，避免用户滚动到长页面末尾才能开始。分析完成后应直接定位结果，原始记录继续作为可回看的证据层。
- 用户可以在 AI 调用前按具体榜单选择分析范围。默认选择仍遵循新书信号优先：平台存在可用新书榜或晋江新晋作者榜时默认选择这些榜单；缺失时默认选择该平台可用的成熟榜单。用户可以主动加入成熟榜单作持续需求对照，最终分析必须只消费本次明确选择的榜单。旧客户端未提交范围时，服务端沿用同一默认规则。
