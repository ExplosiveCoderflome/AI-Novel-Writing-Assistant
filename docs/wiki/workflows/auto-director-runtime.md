...
当前自动导演已经收敛为一条从开书到章节生产的连续链路：

```text
灵感与创作基础
  -> 方向候选与书名确认
  -> auto_to_ready + fast_start
  -> 故事基础 / 开篇世界切片 / 核心角色
  -> 首批 3～5 章路线
...
阅读书架与完整工作台共享同一份自动导演任务、全书执行计划、章节 Runtime、问题策略和恢复链路。两者的区别是信息密度与操作入口，不是是否继续自动写作。用户完成界面选择后，原任务统一切换为 `full_book_autopilot` 并继续生产；界面切换不能创建第二条任务、改变章节范围或取消正在执行的章节。

全书自动生产采用滚动规划。开篇只准备近期路线，默认在可用路线少于 3 章时补到约 5 章；章节进入执行前，再由 JIT 边界补齐当前章的任务单、场景卡和其他执行合同，并把此前已发生事实加入规划上下文。远期卷章不在开书阶段一次性锁死，已生成正文在后续补卷、拆章和恢复时必须受到保护。

当前链路已经具备章节级的完成与恢复事实：可保存正文、资产同步边界、章节终态、质量债、人工恢复状态和下一未完成章节共同决定是否继续。`workflow_completed` 只表示当前任务授权范围执行结束；只有结合目标章节数、正文事实与书级收束证据，才能表达整本作品已经完成。
...
尚未创建小说的书级候选任务以 `directorSession.phase = candidate_selection` 作为恢复阶段的事实源。继续、重试或审批命令可以暂时把任务当前项写成 `approve_gate`，但该控制面标签不得覆盖候选阶段快照；恢复必须使用 Seed 中已保存的灵感、市场简报、创作基础与模型设置重新执行候选生成。只有会话阶段已经进入其他阶段时，才可以按当前项或小说资产进入后续导演恢复，避免候选任务被误送到要求 `directorInput + novelId` 的小说规划分支。
...
新书确认方向后采用 `auto_to_ready + fast_start` 进入开篇准备。自动导演先完成故事基础、开篇世界切片、核心角色和近期路线；准备完成后停在 `production_experience_required`，等待用户选择阅读书架或完整工作台。任一选择都会写入任务 `productionExperience` 和小说 `creationExperience`，把原任务转为 `full_book_autopilot` 并开始正文。任务 Seed 必须持久化 `startupPreparation`，使服务重启后仍能恢复路线窗口、下一章细化游标与延迟增强策略。后续因重规划再次进入结构化大纲时，应沿用已确认的界面与生产合同，不重复要求选择。
...
- 新书未选择创作界面却在开篇路线准备完成后直接开始第 1 章：检查确认接口、结构化大纲阶段或恢复逻辑是否把默认值误当成显式选择。只有用户明确选择阅读书架或完整工作台后才可进入 `chapter_batch_ready`；否则必须停在 `production_experience_required`。
...
当前实现已经把紧凑作品的三段式结构、目标章节、最迟结局章节和最多 5 章的收尾余量传入卷规划、章节路线与正文上下文。滚动路线剩余约 8 章时开始要求收束主冲突、关系变化和未兑现回报；剩余约 3 章时只允许生成结局所需路线，不再开启必须续写的新主线。

独立的书级语义验收目前不能作为既有完成事实。章节批次成功、达到目标章数或 `workflow_completed` 都不足以单独证明主冲突、主角目标、关系变化、核心回报和主题落点已经兑现。在统一的结局验收产物进入事实账本和完成投影之前，产品只能准确表达“已写至目标范围”或“当前任务已完成”，不得把它们自动等同于“故事已经完整收束”。
