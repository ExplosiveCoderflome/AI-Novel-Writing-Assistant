...
- 用户主动暂停运行中的自动导演时，控制面先原子占用同一幂等键，再提交取消 command；处理中重复请求只返回处理中，失败请求允许重试，陈旧占用在租约过期后才可接管。暂停后的任务保留最近检查点，并通过既有取消后重试/恢复入口继续，不能另建一条恢复链。
- 跟进中心在自动导演处于 `queued` 或 `running` 时，列表和当前任务详情必须持续刷新。恢复命令提交成功只代表已进入后台调度；页面要继续显示后续章节、心跳和执行标签的变化，不能因任务从“待恢复”变为运行中就停止轮询，造成“已恢复但没有执行”的假象。
