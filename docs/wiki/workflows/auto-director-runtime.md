...
- `replan_required` 的默认恢复动作是“重规划后继续”，不是跳过当前质量修复。恢复链必须保留已保存正文，以检查点记录的章节为锚点调用统一 `replanNovel` 生成相邻章节窗口计划；重规划成功后，才允许消费旧质量提示并从第一个没有正文的章节继续。只有用户或结构化策略明确选择 `skip_quality_repair` 时，运行时才能跳过重规划。
