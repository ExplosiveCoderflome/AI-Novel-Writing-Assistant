...
章节执行合同是共享的前置产物：同一章的 JIT、恢复和手动入口必须合并在同一个在途请求上，并以已保存的 `Chapter` 合同字段判定复用。全书自动执行时，合同结构缺失可以阻断正文；任务单语义检查的普通风险不得触发第二次整份合同生成，也不得把全书任务升级为暂停。后续正文验收会把仍存在的局部问题记录为质量债并继续。
