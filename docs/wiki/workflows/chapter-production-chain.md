...
- 手动审核与 AI 完整审校共用同一状态提交：审核结果写入 `AuditReport / QualityReport` 后，必须同步提交 `generationState=reviewed` 与 `chapterStatus=completed | needs_repair`；前端完成请求后必须刷新小说详情，不能只刷新报告列表而继续显示审核前的章节状态。
