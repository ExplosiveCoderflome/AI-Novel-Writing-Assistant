...
首章正文稳定保存后，才允许触发角色等延迟增强。正文生成、审校、修复、Fact Ledger 提交和下一章执行前 JIT 的优先级始终高于增强任务；增强失败只形成资料待补齐状态。路线窗口可以提前补齐，但正式执行合同只能在目标章进入上下文组装时确认一次。禁止后台预取与正式执行共同生成同一份正式合同。
...
- 生产阶段接收一次“创建正文流、创建修复流或启动流水线”的请求时，只能记录执行交接，不得把交接误投影为章节完成。流的稳定正文版本在 `onDone` 经过统一运行时、时间线和资产边界后才产生；流水线的完成、暂停和恢复必须读取持久 `GenerationJob` 与章节运行结果。
- 阶段交接记录必须携带目标章节或流水线任务、稳定正文版本是否已经存在、来源页恢复入口和自动处理额度的所有者。它是控制层与执行层之间的轻量事实，不能由外层导演把它再解释成独立质量流程；正文与修复的质量闭环仍由统一章节运行时拥有。
...
- 自动修文需要比较原稿与候选稿时，两次接收闸门只能生成运行时评估，不能提前覆盖持久审计报告或 open conflicts。候选选定后，只有最终保留稿的接收结果可以一次性持久化；回退原稿时，后续 Timeline、事实和资产也必须使用原稿对应的评估。
- 候选风险比较必须使用运行包中稳定的审计 `id/code` 与义务种类，不得把模型生成的 evidence 文本当作问题身份。evidence 是面向修复的说明，措辞变化不能单独构成新增风险。
...
- `acceptance` 门禁按同章、同正文 content hash 和有效验收输入指纹复用成功结果。指纹涵盖验收资产版本、实际渲染的上下文与槽位/模板覆盖，以及解析后的模型配置；仅正文不变不足以证明结果可复用。外围运行包时间戳不参与身份。旧版缺少完整指纹的缓存作为 miss，不清理用户数据。
- 指纹无法确认时跳过缓存。临时 `acceptance_gate_unavailable` 不进入内存或持久成功缓存；抛异常后释放在途请求，使后续调用可重试。模型运行期间配置发生变化时，不发布旧身份缓存。当前在途请求合并只覆盖同一服务实例，不能作为跨 Worker 排他保证。
...
- 章节资产同步必须返回可判别结果：`completed` 表示当前正文版本的必需产物已完成，`pending` 表示仍在运行或排队，`degraded` 表示已记录明确降级且正文可以继续，`failed` 表示无法安全完成当前边界。结果必须携带 content hash 与已完成产物；只有 `completed` 可以写入进程内成功缓存，不能把无异常返回、排队成功或已有 `running` 检查点当作完成。
- `generationState=approved/reviewed` 和章节完成投影只能在最终保留稿的 Timeline 与必需资产同步返回 `completed` 或明确 `degraded` 后发布。`pending / failed` 必须保留在可重入边界，旧的 approved 状态不能成为跳过当前正文版本收尾的依据。`autoReview=false` 只跳过质量审校，不能跳过连续性提交，也不能伪造审校通过记录。
- 同正文版本的并发资产同步必须共享“抽取、连续性边界持久化、成功缓存”这一整个结果，而不能只共享抽取。连续性边界写入失败时，所有等待者都必须收到相同的 `failed` 结果，且不得留下进程内 completed 缓存。
- 导演完成判定、完成比例、完成准则与流水线跳过策略必须使用同一个当前版本闭合定义：资产连续性边界、状态提交和可审阅/已批准事实缺一不可。历史 `approved`、`completed` 或质量债终态没有匹配边界时，恢复入口应保留现有正文并补齐收尾，不能重写正文，也不能静默跳过。
- `artifact_delta` 的检查点分为“版本化抽取结果”和“消费者应用结果”两层。抽取完成后必须先保存可解析的结构化输出；摘要/事实、状态、资源、伏笔、角色动态、知识边界、思维与对话影响分别领取自己的短事务检查点。恢复只能跳过当前正文版本已成功的消费者，不能因某个消费者失败而再次调用抽取模型。
- 消费者的最终写入点必须再次确认章节正文仍是本次输入版本。正文在抽取或应用期间被用户修改时，旧结果不得覆盖摘要、事实、时间线或任一连续性资产；同步以 `failed` 留下可恢复边界。基础摘要、事实和角色时间线等非 delta 写入也遵守这一规则。
- 版本校验必须使用持久化正文的原始内容；摘要或提示组装可以归一化空白，但不得把归一化结果再用于章节版本 fence，否则多段正文会被误判为用户已修改。
- 消费者重试必须具备来源级幂等性：使用稳定唯一键、来源章节覆盖或已提交提案去重。检查点尚未来得及写成成功时发生重试，也不能重复追加同一章节的事实、思维快照或资源变更。
- 严格伏笔对账是独立的完成义务。即使当前正文的增量抽取已完成，只要策略或风险要求全量对账，恢复仍必须领取并完成该对账检查点；不能让外层“delta 已完成”状态遮蔽它。
...
- 章节生成前的“准备”和“上下文装配”是两个不同边界：准备步骤可以补齐 autopilot 路线、章节执行合同和章节计划；上下文装配只能读取已准备产物并组合写作、审阅或修复上下文。审阅、修复、预览或其他只读入口不得因为读取上下文而触发规划模型或持久化写入。
- 正文流和批量流水线必须在 writer 之前显式完成准备。准备失败应在上下文装配前返回真实错误，不能把缺失合同降级为空上下文后继续生成，也不能由上下文读取偷偷补写。
...
- 自动流水线中的修复结果是候选稿，不是正史。原稿初审和候选复审都只能产生评估结果；候选复审结束并完成版本选择前，不得覆盖 `Chapter.content`，也不得提交 Timeline、Fact Ledger、终态事件或最终资产。
- 保留稿选择只复用原稿与候选已有的结构化验收结果，不增加第三次模型评选。候选新增 `high/critical` 问题、新增未兑现义务或没有明确结构化改善时保留原稿；候选通过、减少阻塞问题或减少义务/审校缺口且没有新增严重风险时才采用候选。总分只作诊断辅助，不能单独触发换稿。
- `ChapterContentFinalizationService` 的评估阶段负责生成 runtime package，终态提交阶段只消费已选中评估，不重新调用接收模型。Timeline、已接收事实、章节终态事件和最终资产同步必须只针对同一选中正文执行一次。
- 候选选择记录只持久化原稿/候选正文 hash、各自总分、选择结果和原因，不在 `riskFlags` 中复制正文。候选只需在单次运行内关联；跨运行长期恢复继续依赖流水线开始前的 `NovelSnapshot`，不得为同一职责再建章节版本系统。
...
章节正文、`generationState=approved` 或任意一份资产存在，都只能说明链路的一部分曾经发生。正文被编辑、后台同步中断或旧任务恢复时，这些历史痕迹可能对应旧内容，不能作为导演继续范围或完成比例的依据。

### Decision

章节生产拥有生成、验收、候选修复、终态资产同步与状态提交的执行事实；自动导演只拥有任务范围、调度和这些事实的只读投影。两者不得为同一质量结论建立第二套判断或重试循环。

### Current Rule

- `runtime/artifactSync/` 以归一化正文的 hash 标识一个稳定版本，并以 `artifact_sync_boundary:v1` 保存 `completed` 或明确 `degraded` 的连续性结果。
- 章节只有同时具备当前 hash 的连续性边界、状态提交以及可审阅或已批准事实，才计入导演的章节执行完成比例。
- 旧 `approved`、草稿、旧 hash 检查点和运行中的检查点都只能作为诊断信息，不能驱动“继续下一章”或让导演步骤完成。
- `degraded` 表示连续性边界已按规则收束，可继续生产，但投影必须保留它是降级而非完整资产成功的事实。
- 手动正文流、手动修复流和自动流水线共用章节生产能力。它们可以返回不同的源页或任务恢复入口，但“创建流/入队任务”始终是交接检查点，不是章节完成。

### Failure Modes

- 页面或导演任务显示章节已经完成，但当前正文没有匹配的连续性边界：检查后台同步是否被跳过、失败或仍在运行；不要通过回写 `approved` 修复显示。
...
- 修后正文质量倒退或原稿被覆盖：先检查候选是否在复审前写入 `Chapter.content`，再检查 `riskFlags.qualityLoop.repairSelection` 的正文 hash、分数和原因。若只有总分变化或候选新增严重问题/义务缺口，正确结果应保留原稿，且 Timeline、事实与最终资产都指向原稿。
...
章节流水线的自动处理预约按 `GenerationJob + chapterId` 持久化，质量修复与运行异常重试共用一次额度。预约先于模型调用，调用抛错也保留预约；同 job 恢复读取预约，新 job 使用独立额度。预约后取消可能消耗额度但没有发出请求，这是无法确认外部执行结果时采用的保守上限，而不是计费统计。

修复阶段应在阶段切换后、预算领取后分别检查取消状态；领取被拒绝时保留当前正文并进入原有质量债路径。该机制不覆盖 SDK 内部重试，也尚未替代导演命令租约恢复中的次数计算。历史任务没有逐章预约时，无法从 job 总 retryCount 推断是哪一章消耗，按未预约兼容读取，不回填猜测记录。
...
章节执行合同是共享的前置产物：同一章的 JIT、恢复和手动入口必须合并在同一个在途请求上，并使用同一准备度策略。准备度先判断当前卷规划中的章节要求是否结构完整，再判断已同步到正文入口的目标字数、冲突/揭露强度、禁止事项、任务单和场景卡是否与该要求兼容；“字段存在”不能替代版本兼容。只有结构完整且兼容时才能复用，卷规划变化后应同步或刷新当前合同。全书自动执行时，合同结构缺失可以阻断正文；任务单语义检查的普通风险不得触发第二次整份合同生成，也不得把全书任务升级为暂停。后续正文验收会把仍存在的局部问题记录为质量债并继续。

上下文提供者也必须有稳定合同：声明适用阶段、依赖输入、优先级、层级、必需性、预算和失败语义。角色硬事实、上一章实际结果和章节义务是写作、审阅、修复的必需提供者；第一章没有上一章属于不适用，后续章节缺失则返回明确缺口。世界切片可以降级，知识库召回可以省略，但不能用空数组把适用的必需上下文伪装成完整。

稳定层缓存的失效必须覆盖所有会改变章节上下文的写入口。小说基础设置和写作平台切换发布 `novel:updated`，角色、卷规划、大纲、书级合同和流水线完成继续发布专属事件；缓存只负责失效，不解释业务字段。Worker 在下一章边界重新读取快照，避免沿用修改前的配置。

上下文预算选择时，`required` 块先于软信息进入选择结果；预算不足时必需块最多被摘要，但不会被丢弃。提供者合同记录缺失原因，供运行态诊断和恢复入口使用。
