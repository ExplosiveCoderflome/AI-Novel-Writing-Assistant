...
章节定稿后的思路线变化由既有 `artifact_delta` 单次抽取链唯一回灌，避免增加另一条章节后置推理链。作者可在角色页刷新当前理解，但不能用自由文本直接改写该层。角色采访、影响提案和关系多方案推演仍属于后续阶段，必须和正史确认流程保持隔离。详见[角色智能层：思路线 MVP](../workflows/character-intelligence-layer.md)。

### 当前落地：角色影响提案

角色智能层的下一步不是开放式聊天，而是由 AI 给出少量可选影响方向。作者确认后，提案只在有限章节窗口内作为角色行为的软性引导；章节定稿仍由 `artifact_delta` 根据正文证据判断是否承接。作者补充的一句意图也必须先由 AI 结构化，不能直接改写正史或覆盖角色硬事实。详见[角色影响提案：作者干预的软性引导](../workflows/character-influence-proposals.md)。
