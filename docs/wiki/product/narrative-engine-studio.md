...
第二个优先模块是“故事宏观规划 / 故事引擎”。首屏应采用沉浸式主输入，而不是左右栏状态面板：先让用户围绕一个大输入框写故事想法，并在输入区底部展示就绪度、下一步动作、锁定数量和冲突缺口。只有已经生成主线骨架后，才展示卖点、长期对立、主线钩子、推进回路、成长路径、结局味道和关键兑现点的编辑区。高级字段、硬约束、约束引擎和故事状态应默认下沉，避免新手在没有理解主线骨架前被完整引擎字段淹没。
