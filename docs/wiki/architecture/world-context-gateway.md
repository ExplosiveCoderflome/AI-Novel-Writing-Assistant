...
世界图谱的前端展示分为三层：视图层负责图谱类型、筛选和图例；画布层负责缩放、平移与 SVG 渲染；布局层负责势力节点、地理地点、节点名称和关系文字的位置计算。外部模块只通过图谱面板入口使用，不应绕过画布直接调用内部布局函数。

势力节点应利用宽画布分散排布，节点名称和关系文字分别做碰撞避让，不能把名称固定在节点下方或把全部关系文字固定在线段中点。地理坐标是相对方位提示，不是精确 GIS 坐标；当坐标范围过窄或多地点重合时，展示层可以归一化铺开并进行稳定错位，但不得回写或篡改后端保存的原始坐标。缩放、平移和重置只改变当前视图，不改变世界资产数据。

地理图在坐标归一化后还应保证地点之间的最小展示间距；路线文字必须同时避让地点圆点、地点名称和其他路线文字。势力图与地理图的沉浸展示统一复用公共 `FullscreenView`，保留 Esc 退出、页面滚动锁定、缩放、平移和重置能力，不在图谱模块内重复实现全屏状态管理。
