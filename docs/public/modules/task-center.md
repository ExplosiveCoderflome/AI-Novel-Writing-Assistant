...
左侧“运行记录”用于查看后台任务的排队、运行、失败、完成和恢复状态。旧文档或提示中的“任务中心”指向同一入口。长时间 AI 任务不一定会在当前页面完成，运行记录可以帮助你确认真实进度。
...
执行详情会优先展示模型服务返回的余额、配额、限流或连接错误。看到这类错误时，先处理模型连接和额度，再从恢复入口继续；不要把“章节步骤未完成”当成正文丢失，也不要重新发起已完成章节范围。

## 暂停和失败怎么区分

| 状态 | 含义 | 推荐动作 |
| --- | --- | --- |
| 等待确认 / 暂停处理 | 本书规则或 checkpoint 在等用户决定 | 到导演跟进查看原因并显式恢复 |
| 失败 | 当前命令没有完成 | 查看真实错误，再决定重试或换模型 |
| 等待恢复 | 后台中断后仍保留可恢复位置 | 使用恢复入口，从首个未完成步骤继续 |
