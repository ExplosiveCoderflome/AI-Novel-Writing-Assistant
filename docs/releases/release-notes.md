...
本阶段开发任务全部完成，项目新增完整的世界观沙盒模拟仿真引擎、多维时空分支控制器，以及配套的电子沙盘可视化实时控制台，支持小说世界从物理参数到人物决策的全面演进。

- **地缘物理与宏观生态仿真**：实现了受季节与纬度共同作用的基准温度计算、海拔高度直减率温度降低逻辑、地表水分蒸发与降雨循环，以及动态的 Lotka-Volterra 捕食者与被捕食者群落平衡演进。
- **人物 Agent 社交与决策机制**：支持记忆信息在 Ticks 推移下按艾宾浩斯遗忘曲线衰减；实现谣言在空间距离扩散并伴随突变因子与夸大畸变；设计非焦点角色（LOD 2）基于心智、饥饿、精力的轻量行为树决策（REST/FORAGE/MOVE/SOCIALIZE）与核心主角（LOD 1）的决策拦截流。
- **戏剧张力与 Encounter Box 拦截**：整合地缘物理与人际摩擦计算出三级张力指数，自动调度时间流速流转（Tension $\ge 75$ 时降入 Micro 微观状态捕捉细节，Tension $\le 35$ 时升入 Macro 快速推进）；通过 Encounter Box 碰撞检测拦截相遇主角，进入文斗、公堂辩论等自定义仲裁流。
- **平行宇宙零拷贝分支与剪枝**：实现 Git-like 的宇宙时序分支检出（Checkout）机制，通过 parent 节点快照继承实现零拷贝分支数据库克隆；在修改历史位置 $\ge T$ 时自动裁剪未来时空线，并重置相应章节为 `needs_repair`。
- **虚拟摄像机视口与后置一致性校验**：根据机位提取本埠事件，支持相邻地块高分贝渗漏合并，生成 Markdown 摄像机观测 feed 回灌给 LLM 写作当前局面（`local_state`）；提供后置 `TimeConsistencyChecker`，拦截“角色已在沙盒死亡但在后续正文中开口说话”等致命的时空悖论。
- **电子沙盘可视化控制台**：在网页端新增“世界沙盒模拟”仿真面板，基于经纬度线性投影绘制 SVG 动态地图与联系通道；配备 VCR 统一控制器（播放、单步进、微观/标准/宏观流速换挡）及上帝强覆盖表单，支持暂停时人工覆写气温、张力，或动态注册自定义的世界模型/人物属性。

### 2026-07-14（章节编辑器 V2 残留功能与 AI 漫画/短剧成片合成）

本阶段开发任务全部补齐，包括章节编辑器 V2 功能打磨，以及 AI 漫画运镜合成、短剧剪映草稿导出等功能落地。

- **AI 续写之后**：支持在章节正文中选中前文片段，直接向 AI 申请续写方案，并提供 3 个不同的续写候选版本供对比、预览和一键应用。
- **定位问题一键修复**：章节诊断中发现的节奏、情绪、推进等开放问题会生成诊断卡片，用户可直接点击“直接用 AI 处理”，AI 会自动调取相应的审校问题详情（表现、证据、修改建议）并配合段落上下文生成精准的修改方案。
- **章节内版本历史抽屉**：编辑器侧边栏新增“版本入口”，可随时查看历史版本备份（包括手动保存、自动备份等），并支持一键将书籍内容恢复到选定版本，恢复前会自动备份当前状态。
- **动态漫剧视频合成**：新增 `ComicMotionService` 运镜渲染服务，支持为漫画格子配置运镜元数据（推、拉、摇、移、定等），并利用 ffmpeg 滤镜程序化合成 9:16 比例 MP4 片段与最终成片。
- **剪映/CapCut 工程草稿导出**：短剧单集导出功能中新增 `jianying` 格式，自动将首帧图、语音（TTS）和字幕（SRT）等多轨内容按时间线微秒（microsecond）级参数精准拼装输出为剪映草稿工程 `draft_content.json`，方便专业团队精剪。
...
### 2026-06-23（视频改编与 OpenMontage 桥接集成）
