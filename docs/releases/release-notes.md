...
- 自动导演会按整本目标章数校验节奏拆章进度。长篇项目里，已生成的少量短章节不会再把后续卷误判为完成；继续 1-1024 章这类整本范围时，会从第一个章数不足的卷重新补节奏板和拆章，再进入章节执行。
- 自动导演在节奏拆章恢复时会按卷顺序推进：后续卷节奏板为空时，先完成当前卷章节列表，再继续补后续卷节奏板和章节；进入章节执行前也会核对目标章节范围是否完整，避免前 10 章或指定范围缺章时误进章节执行。
- 自动导演接管已有项目时，重新生成故事宏观或角色准备可以正常清空后续卷战略、节奏拆章和章节细化资源；不会再因为清空后的卷工作区暂时没有卷而提示 `volumes` 至少需要 1 项。角色补齐后，也可以直接从卷战略重新生成。

### 2026-04-24

- 手机端导演跟进页的 4 个原因统计卡片改为 2×2 排布，待跟进总数单独保留在上方，减少窄屏横向挤压和单列过长的问题。
- 手机端全站入口完成适配。首页、小说列表、创建小说、创作中枢、拆书、任务中心、导演跟进、知识库、题材、推进模式、标题工坊、世界观、写法引擎、基础角色和设置类页面都会使用手机端页面壳，不再被桌面侧栏挤压。
- 手机端底部导航覆盖首页、小说、创作、任务和更多入口；更多菜单集中放置资产库、世界观和系统设置，让手机上切换主要工作区更直接。
- 手机端页面统一收敛卡片层级。复杂工作台会优先改为单列模块、轻量分区、横向可扫标签和全宽操作按钮，避免大卡片套多层子卡片造成阅读和点击负担。
- 手机端大型弹窗、表单、列表和多栏工作区统一限制宽度与滚动方式，减少横向溢出；桌面端仍保留原有侧栏、顶部栏和多栏布局。
- 手机端打开小说创作工作区更友好了。小说编辑页会在手机宽度下切换为专属移动端壳，保留横向创作步骤、任务进度和导出入口；章节正文编辑页也会优先展示正文区域，让手机上查看、编辑和继续 AI 修改更顺手，同时桌面端布局保持原样。
- 手机端「节奏 / 拆章」工作区减少了大卡片套子卡片的挤压感。当前处理卷改为更适合手机的下拉选择，当前卷节奏、章节导航和章节细化按模块纵向展开，减少横向滚动和内部上下滚动叠加。
- 手机端自动导演状态不再把整个工作区包进大蓝框。接管状态会在页面上方作为独立轻卡片展示，向下查看内容时保留一行悬浮标识；悬浮标识也避开顶部栏，不再被遮住。
- 手机端工作区整体继续减少外层大卡片。卷骨架、节奏拆章和章节相关区域会弱化最外层容器，保留必要模块分隔；保存入口改为可拖动悬浮按钮，减少顶部操作区挤占内容。
- 节奏 / 拆章的章数控制更可靠了。系统现在会要求单卷节奏板的章节跨度连续覆盖该卷目标章数，不会再把 5-8 个节奏段误当成整卷只有 6-7 章来拆；遇到 3-4 章短卷时，也会按实际章数压缩节奏段，不再因为短卷必须输出 5-8 个节奏段而卡住。
- 拆章标题也会重新检查表层句法是否过度集中。如果一批章节标题大量套用同一种“X 的 Y”或“A，B”结构，系统会让 AI 自动重写，减少整卷章名模板化。
- PostgreSQL 部署链路进一步收稳了。服务端现在会正确接管已经存在但迁移记录异常的历史结构，`k8s + PostgreSQL` 发布后不会再因为补列迁移与现有表结构重复而把 API 卡在启动阶段。
- 自动导演和小说主工作流的阶段同步恢复正常。像 `/api/novel-workflows/sync-stage` 这类阶段回写入口，不会再因为服务端任务更新链路缺失而直接报错，中途继续、恢复和状态推进会更稳定。
- 任务中心的全局任务列表可以重新稳定打开了。包含写法提取任务时，`/api/tasks` 不会再因为任务适配器未初始化而整页 500，混合任务列表、详情和相关操作的可见性恢复正常。
- 卷规划与拆章生成链路里的结构化输出超时封装已恢复。像 `/api/novels/:id/volumes/generate` 这类依赖结构化输出的生成请求，不会再因为超时控制函数未接入而直接返回 `transport_error`，卷级生成在 PostgreSQL 线上环境里更稳。
- 当前主干发布链也补齐了 API 镜像构建所需的运行时依赖和打包细节；在保持桌面 / SQLite 兼容的同时，PostgreSQL 线上环境的发布与回滚路径更清晰，减少“本地能跑、线上镜像启动就断”的情况。
- 角色动态的卷级投影重建这次也补齐了遗漏热修。即使模型在同一卷里给出重复的角色职责行，系统现在也会先稳定合并再落库，不会因为重复投影结果把自动导演后的角色动态重建直接打断，也能继续压住同一条链路上的重复模型消耗风险。
- 自动导演的结构化大纲恢复与继续执行这次也重新校准了“什么才算真正细化完成”。系统现在会把章节执行合同和场景卡一起纳入完成判定，不会再把只生成了部分细化结果的章节误当作已完成并直接跳到后续同步或执行阶段。
- 整本重规划会更尊重项目预设的总章数。重新生成卷战略、卷骨架、节奏板和章节列表时，系统会先按预计总章数重新推导卷数与每卷章节预算；旧草稿里过少的卷数、只有几章的种子章节或异常短节奏板，不会再把 430 章这类长篇目标压缩成第一卷只有 4 章。
- 自动导演接管现有项目时，选择从“卷战略 / 卷骨架”重新生成会先停在卷战略审核点。系统不会在刚重跑卷战略后立刻闯入节奏 / 拆章链路，从而避免后续步骤因为当前卷上下文尚未确认而提示缺少 `volume`。
- 重新生成章节执行批次时，系统也会按本次选择的执行范围重新启动，不再默认继承上一次前 10 章或旧批次状态，减少重跑范围和页面选择不一致的问题。
- 自动导演任务中断后再继续时，会优先回到中断时所在的真实环节。即使当前环节已经生成了部分产物，也不会把“已有阶段草稿”误当成“阶段已完成”而直接跳到下一个环节。
- 卷工作区、章节同步和历史更新链路的关键回归都重新跑通了。重新保存卷规划、恢复章节细化、继续自动导演或检查历史工作区版本时，更不容易因为一次主干合并残留把旧的更新能力带坏。
- 章节轻审校、执行合同生成、写法资产整理和图片素材清理这几条底层链路也一起补稳了。长链路继续推进时，相关能力不会再因为提示词契约错位、重复实现残留或类型漂移而在运行中突然中断。
...
- 系统重启、导演暂停或接管已有进度时，自动导演会重新核对卷规划、节奏拆章、章节细化和章节正文是否完整；缺少卷、缺少细化或章节未写完时，会停留在对应环节继续补齐，避免跳入章节执行后产生不同步内容。
