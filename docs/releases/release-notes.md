...
- “优先完成整本书”会保留可用正文并记录局部质量问题后继续；“质量优先”会在问题仍未解决时停在已保存的章节边界，等待用户明确恢复，不会把部分完成误报为成功或被后台自动解锁。
...
- 手动修复章节也会经过与自动创作相同的最终验收：通过后统一确认正文和章节状态，仍有问题时保留修复稿并记录待处理项，继续运行时不会丢失修复结果或误判完成状态。
