...
- “优先完成整本书”会在一次自动修复仍未解决局部问题时保留正文并继续下一章；“质量优先”会停在已保存的章节边界等待确认，整本自动创作也遵守所选规则。
- 服务重启或后台任务失去响应后，如果自动恢复仍未成功，任务会保留为可恢复状态并提示人工继续，不会直接变成不可恢复的失败；继续时会跳过已经完成的章节。
...
- “优先完成整本书”会保留可用正文并记录局部质量问题后继续；“质量优先”会按任务冻结的处理规则停在已保存的章节边界等待确认。
