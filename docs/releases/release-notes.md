...
- 长篇自动创作新增任务级与章节级成本保护，可限制模型调用次数和 Token 消耗；达到上限时会停在可恢复位置，经济模式还会减少非必要预取与重试，避免后台持续消耗额度。
- 旧项目缺少章节执行契约、场景卡或目标字数时会自动补齐可执行的安全版本；正文生成同时加强相邻章节上下文、重复桥段与模板化表达检查，减少章节之间内容重复和设定漂移。
- 局部且可延后处理的质量问题会保留为待优化项，不再无故打断整段章节生产；真正缺失的硬性义务仍会进入修复。

#### 修复

- 选择简易创作后会等待用户确认本次章节范围，不再立即启动全书生成；选择专业创作则正确结束自动导演准备阶段并交接到完整工作台。
- 修复自动导演已有排队命令时误显示“等待恢复”、人工恢复状态被旧运行快照覆盖，以及后台任务已停止却仍显示运行中的问题。
- Anthropic 接口遇到超时、限流或临时服务错误时会按上限自动重试，鉴权等永久错误仍会立即返回。
- 修复角色阵容与补充角色提示词版本已升级但注册表仍指向旧版本的问题，并将漫画事实提取纳入统一提示词管理。
