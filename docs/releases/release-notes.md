...
- 简易创作与专业创作共用同一套自动导演、章节写作、审校和恢复流程；简易页面只精简信息与操作，不再维护独立的续写入口。
...
- 可以为已经接入自动创作链的问题选择自动重试、提醒后继续、暂停处理或结束任务；每本小说也可按需覆盖全局偏好，运行中的任务继续遵循启动时的规则。
- 处理规则会真正控制章节流水线、自动导演安全暂停和后台任务恢复；运行记录只有在动作成功生效后才会显示为已执行。
- 后台执行失去响应时，会按本次任务的规则自动续跑、停在恢复入口或结束任务，避免反复显示异常却无法继续。
- 有可用正文的局部质量问题和修复遗留会作为可追踪提醒继续全书；明确需要重规划、异常用量、受保护内容和数据完整性风险会停在可恢复位置等待处理。
- 问题设置只展示当前运行链能够执行的可恢复选项，作品与数据安全规则由系统固定保护，避免保存无效偏好。
- 简易创作书架可以直接查看问题处理分数与最近记录，并就地打开本书的问题管理。
- 简易创作完成一个局部章节批次后，如果全书仍有待写内容，章节书架会直接显示后续范围的继续按钮，无需进入创作中枢或专业工作台。
- 继续生成会从首个没有正文的章节接续，并保留已有正文；全书进度按已保存正文与目标章节计算，局部批次完成不会再被误认为整本书完成。
