...
- 自动导演在按卷拆章节列表时，已不再把旧工作区里少量开篇占位章节误当成整卷可信章节数。现在即使工作区里只先落了几章 seed chapters，系统也会继续按原本的章节预算和节奏板推进，不会再把整卷目标章数压小后直接报“章节跨度异常”。
- 修复了“导演跟进”页面在部分待处理任务场景下打开直接白屏的问题。现在进入导演跟进中心时，列表、详情和批量操作区域都能稳定渲染，不会再因为前端状态同步异常而整页空白。
- 当前版本已经可以同时兼容原有本地 / 桌面 SQLite 数据库和 PostgreSQL 部署。桌面运行时会继续优先沿用原来的本地数据库路径；如果你已经在服务端或本地接入 PostgreSQL，也可以继续直接使用，不需要为了同步主干能力在两套数据库之间二选一。
- 主干里已经验证稳定的桌面运行、工作台和生产主链能力已同步合流到当前导演跟进版本；现有 lucky 分支上的导演跟进、自动恢复和相关增强不会因为这次同步而被覆盖掉。
- 角色动态系统在自动导演拆章和卷规划连续保存时，不会再在短时间内重复触发整本角色卷投影重建。卷级职责、关系阶段和角色动态重建现在会更稳，像 MiniMax 这类模型在短时间内被同一条链路打满调用的风险明显降低。
- 自动导演在继续执行或重新生成指定章节范围时，会严格沿用当前选择的区间。像 `11-190` 这类范围，不会再误清空 `1-10` 的已有章节内容，也不会因为旧批次的质检完成状态被错误带进人工处理流程。
- 当卷规划已经扩展到更大的章节数后，自动导演继续执行会继承这次扩展后的执行范围；即使请求里没有重新显式带上章节计划，也不会再退回默认前 `10` 章并提前结束。

### 2026-04-22

- 模型路由新增 `small_text` 任务类型，默认走 `DeepSeek`。系统现在会把渲染后提示近似 token 不超过 `2048` 的短规划请求单独分流，避免继续挤占主规划模型，也减少因为小请求混跑大模型而带来的异常波动。
- 模型路由页现在会直接展示 `small_text`，便于单独查看、调整和连通性检测；服务端也新增了更细的 prompt 运行观测字段，会记录实际生效的任务类型、渲染后提示长度和近似 token 数，后续排查“为什么被路由到短文本”或“为什么中转显示输入异常”时，能直接看到真实提示规模。
- `small_text` 路由不再对伏笔账本同步、角色动态卷投影这类后台 prompt 做写死特殊分流；这些请求只有在自身提示规模也足够短时才会进入短文本通道，模型选择与排障口径会更贴近真实负载。
...
- 默认 LLM 超时已提升到 600 秒，线上 Ingress 也同步补齐了 600 秒的连接、读取和发送超时配置。长思考模型在接口层被过早截断的概率会更低，更适合需要较长推理时间的请求。

- `tk/*` 这组代理模型的调用链路现在会按 Anthropic 协议发送，不再继续套 OpenAI 兼容的结构化输出语义。像 `tk/GLM-5.1`、`tk/Kimi-K2.5`、`tk/Kimi-K2.6` 这类模型，接到代理后更容易走到它们实际兼容的请求格式，减少“状态成功但输入输出 token 显示异常”的情况。
- 这组模型在结构化任务里也会同步改用更稳的 `prompt_json` 路线，不再误判成 OpenAI 原生 `json_schema/json_object` 能力。做节奏规划、结构化分析或其他需要 JSON 输出的任务时，代理链路和模型实际能力的口径会更一致。
- 服务端新增了针对 Anthropic `messages` 的 transport 映射，返回的 `input_tokens`、`output_tokens` 和缓存命中量会按代理返回结果写回统一用量追踪。后续在调用日志、调试输出和代理侧观测里，对 `tk/*` 模型的消耗情况会更容易对齐排查。
- 如果模型正文已经正常返回、但上游 usage 回传出 `output_tokens` 异常偏小甚至直接为 `0`，服务端现在会把该次请求的输入、输出、token 使用量和停止原因单独打成异常日志。线上排查不需要再临时打开全量 LLM debug，也能快速锁定是哪一类代理响应口径出了问题。
- 角色动态投影在重建时补上了同角色、同卷分配行的合并去重。遇到模型把同一个角色在同一卷里拆成多条 assignment 时，系统现在会先合并章节范围和阈值再落库，避免因为重复主键把整次动态重建打断。
- 自动导演现在新增了独立的“导演跟进中心”。需要人工继续、按任务模型重试、回到候选选择或处理重规划的导演任务，都可以在一个页面里集中查看，不用再在任务中心和小说编辑页之间来回找入口。
- 跟进中心会把待处理任务按原因、状态、是否支持批量和通道能力集中整理，并提供顶部统计、列表筛选、详情面板和侧边栏待跟进徽标。哪些书卡住了、为什么卡住、今天恢复了多少，现在会直观得多。
- 对低风险站内动作，系统已经支持单任务继续自动执行、按任务模型重试，以及批量继续 / 批量重试；动作执行后会自动刷新跟进列表和统计，重复提交也会被识别成已处理，减少误触发重复执行。
- 写法引擎现在会把“高保真仿写 / 平衡保留 / 写法迁移”这三套提取预设、每个特征的保留决策，以及关联的反 AI 规则一起保存下来。基于文本提取出来的写法资产，后续继续编辑、绑定或复用时，不会再轻易丢失当初的写法取舍。
- 旧写法资产在读取时也会自动补齐缺失的规则推导。就算早期数据里还没有保存完整预设和决策，系统现在也会尽量从现有特征里还原可用规则，减少升级后写法资产“能看到但不好继续用”的断层。
- 导演跟进中心里的钉钉 / 企微通道能力现在会按系统真实配置判断。只有对应 webhook 真正配好了，页面和接口才会把该通道标成可用，避免未配置时误以为已经能直接往外部通道发起跟进。
- 系统设置菜单里现在补上了“导演跟进通道配置”。钉钉和企微的 webhook、回调 token、人员映射、订阅事件以及站内访问地址都可以直接在页面里维护，不需要再只靠环境变量硬配。
- 导演跟进中心现在可以把自动导演的审核、异常、恢复和完成状态直接推送到钉钉或企微。收到消息卡片后，可以直接继续自动执行，或按任务当前模型发起低风险重试，不必先回到站内再找一次入口。
...
- 钉钉卡片里的低风险动作会继续走统一的跟进执行链路，并带上回调幂等键；遇到消息平台重放或重复点击时，系统会把它识别成同一轮动作，避免同一个跟进任务被异步重复触发。
- 跟进详情页会同步展示最近一次钉钉 / 企微通道触达状态，包括送达结果、响应码和时间；如果提醒没有真正送达，或回调在网关侧被拦住，排查时能更快知道问题出在哪一层。
...
- 章节人工审阅后，如果系统判断需要自动重规划，审阅接口现在会先返回当前结果，再把重规划放到后台继续跑，不会再因为后台重规划耗时把整次审阅一起拖住。
- 同一章节在短时间内重复触发相同原因的后台重规划时，系统会直接识别为同一轮处理中并跳过重复发起，减少连点、重试或并发请求带来的重复重规划和资源浪费。
