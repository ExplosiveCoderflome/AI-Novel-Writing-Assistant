...
### 2026-06-21（网文写手级四轮工作流）

自动导演开书入口更适合“用户只判断方向，AI 负责推进成书”的用法：首页会直接展示网文自动驾驶路径，开书设置会优先选用 `我的默认网文去 AI 味写法`，并提示正文生成后继续做去 AI 检测与修正。默认写法资产加入场景功能、状态变化、对白潜台词和场景代价规则；规划、章节任务、正文生成、审校、接收闸门也会按四轮标准检查：场景是否真的动了、设定是否落进现场、角色口吻是否分开、章末是否留下可延续的压力或义务。

- 首页新增网文工作流卡片，说明“一句灵感 -> 选择方向 -> 章节定功能 -> 正文四轮过闸”的推荐路径。
- 自动导演的书级默认写法会自动优先绑定 `我的默认网文去 AI 味写法`，并在设置区说明写手级四轮过闸会检查哪些风险。
- 默认写法模板加入“场景功能、角色选择、代价变化、章末钩子”的场景单元，新增“场景没有状态变化”“对白潜台词”“场景代价”等规则。
- 章节正文生成前会内部检查场景功能、现场落地、人物口吻和收束回灌；审校和接收闸门会把“语言顺滑但没有可见变化”视为质量风险。
