...
- 自动导演恢复和接管更稳了。继续已有任务、重新接管当前步骤、按卷或按章节范围推进时，会更重视当前真实进度和用户选择的范围，减少误回前 10 章、跳错阶段或覆盖已有拆章成果。
- `节奏 / 拆章` 到章节执行的衔接更可靠。系统会按卷、节奏段、章节列表和章节细化资产判断下一步，缺哪一段补哪一段，避免已经完成的卷规划或章节细化被无谓重跑。
- 模型路由和结构化输出容错补强了。小文本任务会走更合适的模型路由，兼容中转模型的结构化输出方式；当模型返回极低 token 或疑似空响应时，日志会更早暴露异常，排查模型或中转问题更直接。
- 角色动态和卷规划预算更不容易抖动。重复的角色-卷投影会被合并，卷级章节预算会优先保持整本规划目标，不再因为少量种子章节或短卷节奏板把后续章节数带偏。
