...
### 2026-08-03（安卓版：fork 可直接构建 + 漫画渲染稳定化 + 0.4.7.2）

- **v0.4.7.2**：解锁短剧工作台桌面端入口（此前误标"即将推出"，功能实际可用）；横竖屏按方向切换布局——竖屏为手机布局（底部导航）、横屏为桌面布局（左侧导航），默认竖屏可自由旋转。
- 安卓端构建资产全部入库（node 运行库 / 依赖包 / 数据库迁移），fork 仓库后按 README 两步即可构建出可运行的 APK，不再需要从发布包提取资产。
- 漫画工作台渲染改用 WASM sharp（@img/sharp-wasm32），免原生库依赖，真机渲染稳定；APK 体积进一步降至约 163MB。
- 漫画模块数据库迁移补全（角色性别、场景表、分镜引用列），新安装可直接建库使用。
- 修复首次启动模型配置弹窗无法录入自定义厂商的问题；修复安卓端网络连接失败的构建参数。
- WebView 导出功能：小说/资料导出文件写入公共下载目录 `Download/AINovel`，文件管理器直接可见。
- 手机端统一为桌面布局（横屏固定），操作入口与电脑端一致。

### 2026-08-02（安卓本地运行版首个可运行版本，漫画工作台已原生启用）

- 新增安卓手机本地运行版本：完整的创作工作台可以在手机本地运行，不依赖电脑或云端（模型调用需联网，用户自配模型服务）。
- 手机端顶部和底部导航的分类与命名和电脑端保持一致：资产、系统、运行记录等入口名称统一。
- 手机端启动不再白屏等待：启动时显示加载提示，本地服务就绪后自动进入工作台。
- 手机端顶部状态栏不再遮挡内容，切换横竖屏不会重启工作台，App 图标与电脑端一致。
