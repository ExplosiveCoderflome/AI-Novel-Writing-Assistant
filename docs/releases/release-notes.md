...
- 模型路由新增 `small_text` 任务类型，默认走 `DeepSeek`。系统现在会把提示很短的规划请求，以及伏笔账本同步、角色动态卷投影这类后台维护型 prompt 单独分流，避免继续挤占主规划模型，也减少因为小请求混跑大模型而带来的异常波动。
- 模型路由页现在会直接展示 `small_text`，便于单独查看、调整和连通性检测；同时后台链路里原本会把主请求模型强行透传给伏笔账本同步的覆盖逻辑也已移除，短文本路由不会再被上层调用意外带偏。
- 服务端新增了更细的 prompt 运行观测字段，会记录实际生效的任务类型、渲染后提示长度和近似 token 数，后续排查“为什么被路由到短文本”或“为什么中转显示输入异常”时，能直接看到真实提示规模。
