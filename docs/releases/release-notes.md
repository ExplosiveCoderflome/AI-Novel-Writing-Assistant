...
### 2026-08-10（macOS 与桌面 Beta 发布支持）

- 桌面版新增 macOS Apple 芯片安装包构建能力，可同时生成 DMG 与 ZIP，Windows x64 构建继续保留。
- macOS 上关闭主窗口后，点击 Dock 图标可以重新打开应用，不必退出并重新启动。
- 桌面公开 Beta 采用统一版本与下载规则，Windows x64 与 macOS Apple 芯片安装包会在同一个版本的 GitHub Release 中提供。
- 未签名的 macOS 测试包仍可手动下载，文件名会明确标注；这类包不会启用应用内自动更新，签名与公证完成后才会生成 macOS 更新通道。
- Windows 无签名公开包也会明确标注，现有应用内更新能力继续保留；用户可以据此区分测试分发与已签名包。
