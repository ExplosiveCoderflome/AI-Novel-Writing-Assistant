...
- 自动创作在审校、修复或资料同步期间失去执行权时，会在提交前停止旧任务写入，避免旧执行者覆盖当前章节结果。
- 自动修复会先在后台比较原稿和候选稿，只有最终选定的正文才会更新章节审校记录和待处理问题；保留原稿时不会留下候选稿的错误提示。
- 章节资料整理会正确识别多段正文的当前版本，不会因为段落换行而把未修改的正文误判为旧版本。
- 同一个章节问题即使在复审中换一种说法，也不会被当作新的严重问题而错误舍弃已经改善的修复稿。
- AI 实况的每条调用会显示本次使用的厂商和模型；折叠记录也能直接辨认调用来源，方便核对模型路由。

### 2026-09-08

- 章节后续资料同步在中断后会从已完成的位置继续：已经整理出的章节信息会被复用，只补齐尚未写入的状态、角色、伏笔和事实，减少重复等待与不必要的模型消耗。
- 编辑章节正文时，仍在后台处理的旧版本不会再覆盖当前摘要、事实、角色状态或伏笔；系统会保留可恢复状态，等待按最新正文继续整理。
- 手动修复和自动创作都使用同一份章节资料完成边界；资料仍在排队或失败时不会把章节提前标记为完成。
- 自动导演的进度会核对当前正文对应的资料完成边界；编辑后的旧完成记录或仅有草稿不会让章节被提前计入完成范围。
- 同一章节资料同步被多个入口同时触发时，所有等待者会看到一致的完成或失败结果，不会有人提前把未保存的资料当成已完成。
- 旧章节即使曾被标记完成，只要当前正文缺少对应资料边界，恢复会保留正文并补齐收尾；已完整闭合的范围会正常结束，不会误报没有可处理章节。
- 章节写作会先完成所需规划准备，再组装写作上下文；审阅或修复章节时只读取已有规划，不会因为打开上下文而额外生成规划。
- 章节合同会同时检查内容结构是否完整，以及是否仍匹配当前卷规划；规划调整后不会继续复用表面完整的旧合同，手动和自动入口采用同一判断标准。
- 章节生成会明确核对角色硬事实、上一章实际结果和本章任务义务；适用的必需上下文缺失时会指出缺口，首章不会因没有上一章而被误拦截。
- 修改小说基础设置或写作平台后，继续生成章节会及时读取最新配置，不会因后台缓存延迟沿用旧的节奏、文风或平台要求。

### 2026-09-07

- 同一章节流水线任务在服务恢复或多个执行进程同时调度时，只会由一个有效执行者继续；旧执行者失去执行权后不会覆盖新进度，执行状态无法安全确认时会在调用模型前保留恢复入口。
- 章节完成前收到的取消请求不会再被最后的成功状态覆盖；自动导演找不到原先关联的章节任务时会暂停等待恢复，避免把状态异常当成新任务重复调用模型。
- 模型连接失败不会再由底层兼容客户端自动重复请求；章节修复和结构修正仍按明确的业务规则执行，减少不可见的重复消耗。
- 同一章节流水线任务恢复后，会保留已领取的自动处理额度，减少异常后重复修复；进入修复前收到取消请求时停止发起修复。
- 自动导演后台中断后会重新连接原章节任务继续跟踪，不会因为外层任务重启刷新本章处理额度；质量优先流程等待人工确认时，后台恢复会保留暂停位置和原因。
- 调整章节验收要求、提示词或模型配置后，重新审校会依据对应输入重新判断；输入一致时继续复用已有结果。
- 章节接收检查暂时不可用时，后续复查可重新执行，避免反复沿用临时失败结果。
- 自动修复会在复查后比较原稿与修复稿；修复稿出现更严重的问题、遗漏更多章节任务或没有明确改善时保留原稿，只对最终选中的正文同步时间线、事实和后续写作资料。
- 章节只有在当前正文的后续写作资料完成同步或明确降级后才会进入完成状态；同步仍在排队、运行或失败时会保留恢复边界，避免下一章沿用缺失或过期的事实。

### 2026-09-04（模型目录、续写分卷与规划保护）

- 模型厂商的候选模型可逐个隐藏，并能在高级维护区单独或全部恢复；隐藏结果会随配置保存，刷新模型列表或重启应用后仍然有效。
- 当前使用的模型会受到保护，已有任务路由也会继续显示和运行原先选择的模型，整理候选列表不会中断历史配置。
- DeepSeek V4 可选择关闭、低、高或最大思考深度，默认使用高；结构化任务仍会优先保证稳定输出。
- AI 实况会把厂商返回的思考过程与生成内容分开完整展示；关闭模型思考后，厂商仍然返回的思考内容也会保留，方便判断等待时间来自模型推理还是正式输出。
- AI 实况会实时计时本次调用的总耗时，并记录模型首个思考或正文片段的返回耗时；调用结束后会列出输入、输出、思考与合计 Token，便于识别响应慢和消耗异常，厂商未提供的统计不会用估算值代替。近期实况会缓存在本机，刷新或服务重启后仍可查看；客户端不支持缓存时不影响正常生成。
- “模型路由管理”从模型与厂商页面中独立出来；模型连接、模型目录和任务路由各有清晰入口，调整任务模型时无需在长页面中查找。
- DeepSeek 内置候选聚焦 V4 Flash 与 V4 Pro，旧模型只在已有配置或历史路由需要时保留。
- 从已有项目继续自动导演时，会直接展示推荐接续方式；主要动作更集中，模型、写法和接续位置等调整可按需展开。
- 接续目标超过当前卷的章节范围时，AI 会先补齐后续卷规划与近期拆章，再持续推进到选定章节。
- 调整卷规划的审查或其他局部内容时，会保留已有卷和章节规划，不会因一次局部保存误清空整本结构。
- 30 至 59 章的紧凑全书会按开局、转向、终局规划为三卷；终章会按全书章节位置识别，第三卷末将完成整书收尾而不是继续开启下一阶段。
- 新小说初始化时的空白占位卷不会再被当成作者选定的一卷结构；自动导演会按整书篇幅生成正确卷数，用户明确保留的手工分卷仍会受到保护。
- 从热门题材雷达开书时，AI 会只保留题材趋势、读者满足点和结构机会；开书种子不再提前生成具体人名或其他专属称呼，角色与世界命名会在后续规划中完成。
- 章节场景拆解缺少目标字数或有效场景不足时，会把明确的中文原因交给 AI 自动修正一次，不再直接显示内部英文错误。
- 章节完成审校且有待处理建议时，右侧 AI 执行台会提供修复入口，与正文区域的操作保持一致。
- 自定义模型厂商可选择 Bearer、x-api-key 或无需鉴权；获取模型列表、测试连接与正式生成会使用同一鉴权方式。
- 模型厂商地址已包含 `/models` 时不会重复追加路径，避免 Gemini 和第三方网关因错误目录无法获取模型。
- Windows 桌面版更新至 0.4.19，包含章节执行保护、模型路由管理以及 AI 实况耗时与 Token 统计。

### 2026-09-02（章节执行链路治理）

- 继续创作时可以在预计章节数以内选择较小的推进目标；保留预计章节数则继续使用自动补卷和滚动拆章。
- 自动导演从已有项目继续创作时，会以预计章节数作为整本书目标，不再被当前已拆章节数限制。先完成手头章节；接近当前范围末尾时，AI 会自动补后续卷骨架和近期拆章，再继续写作，不会一次生成整本书的远期章节任务。
- 自动导演会复用已经完整生成的章节任务，不会因为事实条目增加而重复刷新，也不会在“提前准备下一章”和“正式执行下一章”两个入口重复生成同一份任务，减少异常模型调用与 Token 消耗。
- 同一章在多个入口同时准备时会合并为一次生成；全书自动创作中的任务单语义提示会保留给后续验收，不会再重写整份任务单，减少异常模型调用与 Token 消耗。
- 章节任务生成只会针对结构接收失败进行一次定向重试；连接中断或空响应会按真实原因结束当前调用，不再触发整套章节任务的多层重复生成。
- 正文会在适合章节长度的额度内生成，并关闭不必要的推理过程；验收、局部修文和章节事实回灌各自使用独立的小额度，减少隐藏推理、过长结构结果和修复重试带来的无效 Token 消耗。
- 选定范围的章节都已保存完成时，系统会正常完成本次创作；异常用量仍会阻止尚未开始的后续章节继续消耗，不会把已完成的范围误显示为等待恢复。
- 第三方或自定义兼容接口不会再因模型名称被强制使用原生 JSON；系统先采用更稳的 JSON 提示词格式，连接检测确认支持后才保留对应格式，减少截断和无效修复调用。
- AI 创作实况连接因服务重启或网络波动中断后会自动恢复，并重新读取近期生成快照，不需要刷新页面，也不会重复触发模型任务。
- 自动导演开书时即可确认本书的问题处理方式；不调整时沿用默认规则，调整后的规则会随本书任务保存并在执行中保持一致。
- 正文生成与验收使用同一套退化检查边界，减少“生成后立刻被自身审核判为不合格”的无效修复。
- Windows 桌面版更新至 0.4.17，包含本次章节 Token 用量控制与完成状态收束改进。
- 首页聚焦当前最需要推进的作品、全局状态和创作资源，减少与创作主链重复的信息区块。
