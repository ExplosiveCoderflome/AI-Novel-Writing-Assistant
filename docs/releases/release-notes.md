...
- 模型路由设置里现在新增了单独的“短文本处理”通道，便于把提示短、上下文轻的规划类请求单独分流，不再和正文写作、审阅、修复共用同一条默认说明口径。
- `small_text` 路由现在只会接住真正短的规划提示。像角色动态重建、伏笔账本同步这类输入已经很长的后台维护请求，不会再被硬塞进短文本通道，而是继续留在主规划路由，模型选择与排障口径会更贴近真实负载。
- 章节审阅后如果系统判断需要自动重规划，重规划现在会改成后台补跑，不再阻塞当前审阅结果返回。你可以先拿到本章审阅结论和问题列表，再决定是否继续等后续规划补写完成。
- LLM 默认超时上限也已经拉长到 10 分钟。遇到卷级规划、长上下文分析或代理链路波动时，不容易再因为默认超时过短而过早中断。
