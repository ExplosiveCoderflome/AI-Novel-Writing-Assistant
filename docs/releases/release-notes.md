...
正文生成最终 message 的上下文也更接近写作者和模型实际需要阅读的资料：必需上下文保底、书级合约、章节任务、当前局面和风格合约会尽量使用中文标题与中文字段，减少内部 key、调试字段和角色 ID 泄漏。

- 高级模板编辑器会把书级合约、章节任务、时间线、角色硬事实、章节标题、语气与节奏等引用渲染为标签；源码视图仍可查看和排查原始 `{{...}}` token。
- `@` 引用菜单插入的是可视化标签，点击或悬停标签可以理解引用含义；未知 token 会保留原文并以异常状态提示，避免保存时丢失模板内容。
- 正文生成缺少必需上下文时，保底标题会显示“时间线”等中文名称，而不是 `timeline_context` 这类内部分组 key。
- 书级合约、章节任务、义务合约、当前局面和风格合约的正文上下文减少英文 schema 字段、调试键和内部角色 ID，让最终 prompt 更适合直接交给模型写作。
- 任务单质量门禁、章节接收闸门和卷级章节列表提示词补齐枚举表、JSON skeleton、输出归一化和重试诊断，降低结构化输出因为 `pass`、`acceptable`、百分制置信度或标题重试说明不清而反复修复的概率。
- 章节列表质量重试会把失败原因按标题结构、标题基础质量、章节功能、摘要推进和结尾牵引分组说明，并要求重排整组标题骨架或章节功能，而不是只替换触发项。
