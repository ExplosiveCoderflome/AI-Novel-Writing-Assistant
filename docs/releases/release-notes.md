...
这次更新把桌面版自动导演里一批最容易把新手卡死的结构化调用问题收束到了同一套共享治理里。桌面版在 OpenAI 兼容模型上的自动开书流程更容易从候选方向、故事宏观规划、角色准备一直推进到分卷战略，而不会因为结构化协议错配或中途的传输失败过早停住。

- 模型路由会更稳定地把 OpenAI 兼容模型按正确协议执行，减少“明明在用 OpenAI 模型，却按别的协议发送结构化请求”导致的桌面版自动导演中断。
- 结构化输出调用新增统一的超时和降级治理。遇到原生结构化请求不稳定时，系统会在共享层尝试更适合当前模型端点的结构化策略，而不是要求每条业务链分别补一层独立兜底。
- 自动导演里的角色阵容生成不再停在旧的首层传输错误。恢复后的桌面版真实运行已经能从角色准备继续推进到角色应用，并进入卷战略与卷骨架生成阶段。
- 书级候选和 Book Contract 的本地兜底辅助逻辑做了减法：重复的共享重试层被移除，只保留仍然需要的最终直连兜底路径，让后续排查更集中在共享结构化治理本身。
- 桌面版问题排查经验被写回项目协作规则。后续处理 packaged runtime 问题时，会优先用 fresh runtime 的启动时间、端口、实时路由状态和任务快照来确认当前真实故障层，减少被旧日志误导。
- 书级 framing、故事宏观规划、工作区分析和手动改文影响分析这几条常见开书链路也补上了更稳的兼容处理：旧库存量缺字段时不再轻易崩掉，结构化修复失败时也更容易回到可诊断、可继续的错误层。
- 桌面打包链补齐了 Prisma 运行时版本对齐和 electron-builder 兼容补丁，重新打 fresh 包时更不容易因为依赖装配或构建器兼容性问题卡在发包前。
...
- 模型路由现在支持按任务补充自定义请求头。你可以在设置页为不同任务单独填写额外请求头，适配对 `User-Agent` 或网关标识更敏感的中转渠道。
- 模型路由连通性检测也会带上这组请求头一起测试，保存路由后可手动重新检测，确认当前渠道是否已经按新的请求协议和请求头真正生效。
