...
- 模型路由现在支持按任务补充自定义请求头。通过设置页可以为不同任务单独填写额外请求头，适配对 `User-Agent` 或网关标识敏感的中转服务，减少某些兼容渠道因为请求指纹不匹配而直接失败的情况。
- 模型路由连通性检测也会带上这组请求头一起测试，保存路由后可以重新检测当前渠道是否已经按新协议和请求头真正生效。
- 任务执行日志接口路径修正后，任务中心读取导演执行日志时不会再被通用任务详情路由抢先匹配，旧任务查看执行日志更稳定。

### 2026-04-30

- 自动导演在章节资源准备完成后，点击继续自动执行会直接推进到章节写作链路。需要补齐章节任务单或场景拆解时，系统会先补齐资源，再继续进入章节执行，减少任务停在 92% 反复等待的情况。
