...
- 首页顶部以当前小说、推荐动作和整本创作旅程组织续写入口，封面、真实进度和下一步操作更容易识别。
- 首页摘要聚焦正在创作、等待确认、可以开始写和已沉淀章节，历史失败记录保留在创作提醒与运行记录中。
- 创作中枢聚焦小说状态查询、问题诊断、执行记录和下一步建议；完整 Agent 驱动创作可从页面进入独立项目。
- 创作中枢会阻止写入和执行类计划，小说创建、整本生产、正文写入与自动导演操作统一从正式工作台进入。
- 创作中枢只负责状态查询、问题诊断、执行记录和下一步导航，不承担小说产物创建或任务执行。
- 运行记录和导演跟进入口收进系统区域，创作导航更聚焦小说内容与当前创作状态。
