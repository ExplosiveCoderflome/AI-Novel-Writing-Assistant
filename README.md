...
自动导演接管已有项目时，角色准备和卷战略重跑链路更稳定：从故事宏观或角色准备重新生成，会先安全清空后续卷战略、节奏拆章和章节细化资源；角色资产补齐后，也可以直接从卷战略重新生成，不会再被临时空卷工作区误拦。

- 重新生成角色准备时，系统可以合法清空依赖角色的卷战略和拆章资产，避免出现 `volumes` 至少需要 1 项这类不符合当前操作语义的提示。
- 重新生成故事宏观规划时，后续角色、卷战略和拆章资源也会按链路清理，让下一轮自动导演从干净状态继续。
- 当角色资产已经存在时，从卷战略重新生成会保留前置书级规划和角色资产，只重跑卷战略与后续拆章资源。
