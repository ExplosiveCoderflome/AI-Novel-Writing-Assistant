...
稳定性更新：PostgreSQL 线上发布链路、自动导演阶段同步、任务中心列表、卷规划生成、结构化大纲恢复，以及历史卷工作区更新链路都补上了热修复和回归校准。
...
- 自动导演结构化大纲的恢复与继续执行重新对齐到了完整执行合同，不会再把缺少场景卡或细化未完成的章节误判成“已准备完成”。
- 卷工作区、章节同步和历史更新相关能力已做回归校准；重新保存卷规划、恢复章节细化或继续自动导演时，更不容易把旧的更新链路带坏。
