...
- 角色页现在可以让 AI 准备 2–3 个“下一步影响方向”，对比每个方向的推荐理由、读者回报、风险和适用章节后再选择。
- 确认方向时可以补充一句创作意图，AI 会将它整理成角色软性行为引导；它不会直接改写角色正史，也不会强制正文发生指定剧情。
- 正文明确承接已确认方向时会记录依据；未承接的方向在适用章节结束后自动失效，不会阻断继续写作或自动导演。
