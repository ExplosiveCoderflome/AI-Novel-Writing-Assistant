...
- 自动导演的问题处置统一由“问题处理规则”控制；独立风险阈值设置已移除，风险分只用于解释问题严重程度。
- 后台任务失去响应或触发熔断时，会真正执行本书设定的重试、继续、暂停或结束动作，并在动作完成后更新运行记录。
- 完成优先会保留已有正文并记录质量问题后继续；质量优先可在人工分阶段创作时停在章节边界等待确认，整本自动创作仍会记录质量债并继续。
- 已识别的问题会直接执行处理规则，只有无法分类的异常才交给 AI 辅助判断，减少无效等待和重复模型调用。
- 章节运行异常重试和质量修复共享最多一次自动处理机会，避免同一问题在不同阶段叠加重试。
- 章节问题只执行一次处理规则判断，自动导演不会为同一质量提醒重复调用模型评估。
- 手动修复会只读识别问题并与自动创作共用最终验收；通过后统一确认章节，未通过时保留修复稿并记录待处理项。
- 接收检查暂时不可用不会误占自动修复机会，正文会保留并等待复查。
- 每个最终正文版本会先收束时间线状态再继续；即使局部质量问题暂时保留，下一章仍能读取最小承接信息，不会因跳过修复而失去上下文。
- 单次章节自动修复最多执行一次局部补丁和一次整章修复，不会隐藏追加第二轮补丁调用，实际处理次数与“最多一次重试”保持一致。
- 章节生成、审校和修复统一保存正文与进度状态，降低修复已完成但界面仍待处理、已完成章节被重复执行等状态漂移。
