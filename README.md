...
- 自动导演发现章节计划局部失配时，会自动调整尚未生成的后续章节安排并继续创作，已保存正文和手动内容保持不变；历史重规划任务继续前会先补齐后续安排，避免在相邻章节反复暂停。
...
- 分段生成章节标题时会主动避开本卷已有标题；发现重复会自动重试，无法消除时停止保存，避免重复标题进入后续正文生产。
- 已有作品可逐段重新生成重复标题并同步目录，已完成正文会继续保留。
