...
重大更新：写法创建入口、知识库原文提取和自动导演恢复链路进一步收口，长篇素材和继续任务会更贴近真实资产。

- 写法创建入口收进同一个弹窗了。可以在 `模板起步 / 空白-AI / 从素材提取` 之间选择，并在同一处使用直接粘贴文本、知识库原文或拆书结果创建写法。
- 知识库原文创建写法会冻结活动版本作为来源快照；长篇原文默认使用智能抽样，只把代表性样本交给模型学习写法，减少整本书一次性塞进模型导致的超时和不稳定。
- 写法提取超时可以在系统设置里调整了。长文本或慢模型可以直接调高等待时间后重试，不需要再改 `.env` 或重启服务。
- 写法列表和编辑区新增“知识库原文”来源识别，历史文本提取、拆书生成和新知识库来源会按各自来源显示，后续追溯更清楚。
- 知识库上传长篇小说后的索引状态现在会更贴近真实执行进度。即使历史 RAG 任务很多，知识库页面也会优先看到最近仍在更新的任务，不再容易把已经开始处理的文档长期误显示成“排队中”。
- 上传新文档、重建索引或等待向量生成时，页面会更快跟上最新运行状态，减少“后台已经在跑、前台还像没开始”的错位感。
- 自动导演在服务中断、页面刷新或重新继续任务后，会优先按已经生成好的卷工作区、章节细化和章节执行资产恢复，不再轻易回退到卷战略或重跑卷骨架，减少已细化章节被覆盖的风险。
- 自动导演在“先生成两套书级设定，再确认其中一套”后不再误跳到节奏板。若当前只有占位卷工作区、还没有真正的卷战略，系统会先回到卷战略 / 卷骨架继续推进，避免一确认方案就立刻报“请先生成卷战略建议”的中断。
- 前 10 章、指定章节范围和按卷继续执行现在统一按同一套章节范围规则推进。继续某一卷时，会按这卷当前真实章节范围接着跑，前 10 章也会明确按 `1-10` 章处理，范围判断更稳定。
- `节奏 / 拆章` 和 `章节执行` 之间的衔接也收顺了。AI 生成章节列表、整卷拆章或章节细化后，会自动同步到章节执行区，不需要再额外手动点一次同步。
- 角色阵容方案在应用前会先提示明显的设定风险。系统会把需要确认的问题先展示出来，你可以决定先应用再微调，角色准备阶段的控制感会更强。
