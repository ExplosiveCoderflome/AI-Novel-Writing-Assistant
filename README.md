...
稳定性更新：节奏 / 拆章、整本重规划章数预算、章节标题、PostgreSQL 线上发布链路、自动导演阶段同步、任务中心列表、卷规划生成、结构化大纲恢复、现有项目接管重跑，以及历史卷工作区更新链路都补上了热修复和回归校准。

- 节奏 / 拆章现在会严格校验单卷节奏板的章节跨度，要求连续覆盖该卷目标章数；3-4 章短卷会按实际章数压缩节奏段，不会再被 5-8 个节奏段的常规规则卡住。
- 拆章标题会检查句法是否过度集中；如果一批章节大量套用同一种“X 的 Y”或“A，B”结构，系统会让 AI 自动重写，减少整卷章名模板化。
- PostgreSQL 部署现在会正确接管已存在但迁移记录异常的历史结构，发布后不会再因为重复补列把 API 卡在启动阶段。
- 自动导演与小说主工作流的阶段同步恢复正常，`sync-stage` 这类阶段回写请求不再因为服务端任务更新链路缺失而直接失败。
- 任务中心的全局任务列表可以重新稳定打开；包含写法提取任务时，`/api/tasks` 不会再因为适配器未初始化而整页 500。
- 卷规划与拆章生成里的结构化输出超时封装已恢复，`/api/novels/:id/volumes/generate` 不会再因为 `runWithEnforcedTimeout` 缺失而直接报 `transport_error`。
- 角色动态卷级投影重建补齐了重复职责合并热修；模型偶发给出重复角色-卷职责时，不会再把自动导演后的动态重建直接打断，也更不容易在同一条链路上重复消耗模型调用。
- 自动导演的结构化大纲恢复与继续执行会把章节执行合同和场景卡一起纳入完成判定，不会再把只生成了部分细化结果的章节误当作已完成。
- 整本重规划会以预计总章数重新推导卷数与每卷章节预算；旧草稿里的少量种子章节或异常短节奏板，不会再把长篇目标压缩成第一卷只有几章。
- 现有项目接管时，从“卷战略 / 卷骨架”重新生成会先停在卷战略审核点，不会在卷上下文尚未确认时继续进入拆章链路并报缺少 `volume`。
- 重新生成章节执行批次会按本次选择的执行范围重新启动，不再默认继承上一次前 10 章或旧批次状态。
- 自动导演任务中断后再继续时，会回到中断时所在的真实环节，不会因为当前环节已有部分草稿就直接跳到下一环。
- 卷工作区、章节同步和历史更新链路的关键回归已重新跑通；重新保存卷规划、恢复章节细化、继续自动导演或检查历史工作区版本时更稳。
