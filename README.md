...
- 热门题材雷达新增独立的热门题材列表，可从 AI 分析卡片快捷收藏选题，并直接用单个收藏题材进入自动导演开书。
- 自动导演起始设置支持选择战力体系，可交给 AI 判断，也可明确选择不需要、软性强弱或等级体系；后续世界规则、角色成长与章节写作会保持同一设定。
- 模型路由管理支持配置结构化调用的重试次数与备用模型，在服务繁忙、超时或连接中断时按统一规则恢复。
- AI 实况可查看正文生成实际发送的 Prompt，并同时显示调用用途、厂商、模型、首个返回耗时及输入、输出、思考 Token。

#### 优化

- 官网首页主标题聚焦“说出你的故事”，让第一次访问的人更快理解产品从故事想法出发的创作入口。
- 热门题材雷达通过“当前榜单、AI 分析结果、热门题材列表”三个页签切换，选题浏览和开书操作不再挤在同一长页面；市场收藏与通用题材、推进模式资源彼此独立。
- 章节生成、验收、局部修复和资料整理采用独立额度，并复用同一正文版本的验收与资产抽取结果；并发入口会合并执行，明显减少重复调用与异常 Token 消耗。
- 章节资料整理聚焦影响后续写作的核心变化，兼容字段缺省和常见表达差异；局部修复优先输出少量高价值补丁，减少结构结果截断后的无效重试。
- 自动创作会从已完成的章节规划、正文、审校与资料同步边界继续，只补齐缺失步骤；章节上下文会及时采用最新基础设置、卷规划和正文事实。
- 自动修复会比较原稿与候选稿，只在确有改善时采用修复稿；局部质量问题作为可见的章节待优化项处理，不会无故中断整本创作。
- 热门题材雷达优先展示最近结果并在后台更新，刷新期间保留 AI 报告并显示真实进度；模型设置操作完成后会及时恢复可用状态。

#### 修复

- 修复重新进入热门题材雷达时重复扫榜并长期停留在 99% 的问题；已有榜单会直接展示，用户可按需重新扫榜。
- 修复服务重启、多进程调度或页面恢复时重复执行章节、验收、修复及资料同步的问题，并阻止失去执行权的旧任务覆盖新结果。
- 修复取消请求被完成状态覆盖、人工恢复位置被后台清除，以及原章节任务丢失后错误新建任务的问题。
- 修复正文换行、手动编辑或规划变化导致资料版本误判、旧资料覆盖新正文、章节过早计为完成的问题。
- 修复自动导演首次规划重试丢失开书上下文，以及已完成章节范围被误报为等待恢复的问题。
- 修复桌面版提示词槽位保存时误报数据库未就绪的问题，升级后会自动补齐所需数据表，无需手动迁移。
- 修复 GLM 4.5 及以上模型执行卷战略等结构化任务时因思考阶段过长而返回空内容的问题，此类任务会优先关闭思考并直接生成结构结果。
- 修复 Kimi K3 忽略顶部温度设置并持续发送不兼容数值的问题，调用时会自动采用模型要求的固定温度。
- 删除小说时会同步清理与该作品关联的失败运行记录，避免运行记录中残留无法返回原作品的失败项。
- Windows 桌面版更新至 0.4.21，包含本次章节生产可靠性、Token 控制、AI 实况、自动导演设置与热门题材收藏改进。
