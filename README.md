...
- 本书层恢复官方当前版时，会明确遮蔽全局覆盖；即使全局规则被改坏，本书预览和生成也会使用官方默认值。
- 对官方文案更新、槽位移除等情况，可以选择“恢复官方当前版”，也可以“保留我的设置”并消除版本提醒。
- 槽位卡片会显示当前来源：官方默认、全局覆盖、本书覆盖或本书使用官方默认。
- “清除本书覆盖”和“恢复官方当前版”区分为不同语义，避免误把恢复官方默认操作变成重新继承全局坏值。
- 生成预览会为可编辑槽位提示词提供完整样例输入，审校、正文改写和封面 brief 等提示词不再因为缺字段弹出底层报错。
- 完整审校提示词预览会带上示例章节边界和结构义务上下文，右侧上下文面板能看到本次注入的审校资料块；缺少必需上下文时也会用中文说明缺失组。
- 本书范围可以选择预览章节；完整审校预览会优先读取所选小说章节的正文、章节任务、章节边界和结构义务，不再把示例上下文当成当前书籍资料。
- 正文写作提示词 `novel.chapter.writer` 在本书范围生成预览时，会读取所选小说章节的正文写作上下文，右侧可看到书级合约、章节任务、角色硬事实、义务合约、卷级窗口、参与角色、当前局面和风格合约。
- 正文写作预览的书名、章节序号、章节标题和目标字数会跟随所选小说章节，方便确认预览内容来自当前书籍。
- 正文写作提示词在本书范围可切换“安全槽位”和“高级模板”；高级模板支持 `@` 插入上下文、运行变量和槽位引用。
- 高级模板预览会显示最终 messages、显式引用的上下文和系统自动追加的必需上下文保底。
- 每次保存高级模板都会生成本书版本历史，可以查看、回滚或恢复官方模板；恢复官方不会删除历史版本。
