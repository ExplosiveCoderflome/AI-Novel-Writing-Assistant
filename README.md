...
- 首次使用自动导演且没有世界样本时，可以直接采用默认方案继续开书，不会因打开世界样本选择而白屏；填写中的起始想法和各步设置会保存在本机，刷新、重新打开或页面异常后可以继续。
...
- 自动导演候选方案生成失败时，可直接在当前页面快速重试；运行记录专注展示状态、错误和来源，继续、恢复等操作会回到对应工作页面完成。
- 自动导演会按真实模型或运行错误进入恢复，不再把一般失败误记成章节轻修失败；局部质量问题也不会因为数量、影响范围或旧质量预算单独停止整本创作。
- 重复审校同一个局部问题不会再按历史次数升级成整章重写或强制停机；修改后重新审校通过即可关闭待优化状态。
- 服务重启时会先整理待恢复任务，再启动自动导演 Worker；等待人工恢复的任务不会被后台抢先重新执行。
- AI 创作实况会优先显示中文任务名称，并保留提示词 ID 与版本，方便判断当前生成步骤和定位问题。
