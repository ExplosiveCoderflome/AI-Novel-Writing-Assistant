...
- Windows 桌面版发布 `0.4.10`，包含自动导演风险治理、简易创作入口与工作区导航优化。
- 自动导演完成开写准备后会先让你选择“简易创作”或“专业创作”，不会替你默认进入简易模式。
- 简易创作会让 AI 继续自动完成整本书；专业创作会进入完整工作台，由你检查、调整并安排正文生产。
- 后续自动重规划会沿用已确认的生产方式，不会重复询问。
- 点击继续生成后，按钮位置会持续显示排队或当前章节动作，可以直接确认 AI 是否正在工作。
- 局部章节批次结束后可以按全书目标继续生成，系统会从真实章节进度接续并保留已有正文。
- 从小说列表打开作品会直接进入对应的简易书架或专业工作台，不会在两个页面之间来回跳转。
- 自动导演页面聚焦资源进度与恢复操作；AI 实况统一从顶部入口查看，已完成资源可随时打开。
- 小说项目建立后可以立即进入简易创作书架查看准备成果；AI 会继续后台准备，并在资源具备后自动开始正文。
