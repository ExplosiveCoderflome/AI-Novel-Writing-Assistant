...
- Windows 客户端的资料检索会按保存的检索设置正常启用；历史版本自动留下的关闭状态会恢复默认开启。点击“重建索引”后，资料会进入切片和索引队列；用户主动暂停检索的选择仍会保留。
- AI 修正章节目标时会拒绝与章节摘要或原有目标完全相同的结果，并要求模型重新生成；持续不合格时会保留原有内容，避免被重复文本覆盖。
