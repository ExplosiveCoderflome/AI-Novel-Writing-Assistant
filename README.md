...
- 自动导演因模型余额或配额不可用而暂停时，会保留已完成正文与下一未完成章节的位置，优先展示真实错误和恢复入口，不再被通用步骤错误覆盖。
- 模型服务明确返回余额不足时，会按本书的问题处理规则决定重试或人工暂停；恢复后不会重新生成已经保存的章节。
- 模型配置可新增独立的第三方兼容厂商，不会覆盖已有的 DeepSeek 等内置厂商配置。
- 本书问题管理的两套预设会完整设置所有问题动作，避免部分条目仍受全局规则影响。
