...
- 所有页面顶部都可以打开“AI 实况”，随时查看模型正在生成、检查或修复的内容片段；正式内容仍会在检查完成后安全保存。
- “AI 实况”会以可拖动、无背景遮罩的终端日志窗口显示；查看旧内容时不会被新输出打断，可以随时回到最新位置，关闭操作也不会被拖动手势干扰。
- “AI 实况”按每次调用分组：新调用会自动聚焦，已结束调用自动收起；可以清空当前窗口内容，后台生成和运行记录不受影响。
- “运行记录”用于按需查询执行历史、异常和恢复位置，正常创作时不需要离开当前页面确认 AI 进度。
- 小说工作台新增“世界观准备”步骤：可以先整理或生成本书的世界规则、地点与势力，再进入角色与剧情规划。
- 自动导演恢复创作时会检查世界观准备度，缺少资产会先引导补齐，已有书级规划、角色和正文保持安全。
- 角色准备、卷战略和节奏拆章会使用已准备的世界观作为前置约束；首页与工作台的信息层级也更便于聚焦下一步。
- 在小说内生成或新建世界时，会自动保存到世界库并绑定当前小说；后续可按需复用或手动同步，无需额外保存。
- 已生成的本书世界会正确标记为“世界观准备”完成。
