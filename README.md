...
- 服务重启后若章节资料仍显示同步中，自动创作会保留正文并继续后续章节，不会重新执行已经完成的章节修复；质量记录同时展示用于判断的综合分。
- 局部修复计划提供更充足的结构化输出空间，并要求优先输出少量高价值补丁，减少修复计划被截断后反复重试。
- AI 实况会显示正文生成实际发送给模型的 Prompt，便于核对章节上下文、写作要求和模型输出之间的关系。
- 同一章节版本同时触发多条资料同步时，会复用正在进行的资产抽取结果，避免并发入口重复调用模型。
- 章节恢复或重复进入审校时，会复用同一正文版本的已有验收结果；验收报告空间扩大，减少同一正文重复调用和因输出截断产生的重试。
- 章节资料整理会兼容模型偶发的字段缺省或枚举表达差异，保留有效内容并继续同步，减少因格式偏差产生的重复调用。
- 章节生成后的资料整理会优先提取影响后续写作的核心变化，限制重复重试；偶发的结构化输出偏差不会反复消耗模型额度。
- 自动导演起始设置新增“战力体系”，默认由 AI 判断本书是否需要；也可以明确选择不需要、软性强弱或明确等级。
- 不需要战力体系的小说不会被强行加入境界、等级和升级线；采用战力体系时，世界规则、人物成长和后续章节会保持同一套强弱边界。
