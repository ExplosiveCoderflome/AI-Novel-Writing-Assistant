...
重大更新：模型路由新增 `small_text` 短文本通道并默认走 `DeepSeek`，短规划与后台维护型 prompt 会单独分流；同时默认 LLM 超时和线上 Ingress 超时一并提升到 600 秒，长思考请求更不容易被接口层过早截断。
- `small_text` 会承接短提示规划请求，以及伏笔账本同步、角色动态卷投影这类后台维护 prompt，减少小请求继续挤占主规划模型。
- 模型路由页现在会直接显示 `small_text`，可以单独查看、调整和做连通性检测，后台短文本链路也不会再被主请求模型意外覆盖。
- 服务端会记录实际生效的任务类型、渲染后提示长度和近似 token 数，后续排查“为什么被路由到短文本”会更直接。
- 默认 LLM 超时已提升到 600 秒，线上 Ingress 也同步补齐了 600 秒超时配置，更适合需要长时间推理的请求。
