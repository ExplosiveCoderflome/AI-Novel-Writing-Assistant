...
- 自动导演创建流程改成分步页面：从一句灵感开始，逐步确认作品定位、世界与写法、模型和运行方式，再选择方向方案。
- 首页、小说列表和小说工作台更聚焦下一步创作，减少卡片和边框噪音，让继续写作、处理待办和查看当前进度更直接。
- 节奏拆章新增紧张度曲线和冲突强度锚定，手动指定的章节强度会影响后续拆章、细化和重规划。
- 章节正文增加自然度检查，能发现 AI 自述、占位符、提示词泄漏、截断、复读和模板化句式，并尽量在本章修复或记录为质量债。
- Prompt Workbench 改成可视化工作台，正文生成提示词支持安全槽位和本书高级模板；常用引用会显示成中文标签，输入 `@` 就能插入资料、章节参数或写作规则。
- 提示词编辑器新增“测试产出”：可以在保存前打开测试弹窗选择模型，用当前草稿真实生成一次结果，对比最终 messages、上下文和模型输出。
- 公开文档站改为真实页面路径并支持预渲染，文档可以直接分享、被搜索引擎收录，也能在站内搜索。
- 本次包含新的前端可视化依赖和数据库迁移；源码运行用户拉取后需要执行一次 `pnpm install`，并按现有流程完成数据库迁移。
