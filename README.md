...
- 自动导演恢复创作时，章节规划只有部分执行字段也能继续推进；系统会在写作前自动补齐并校验当前章，不再反复卡在章节合同同步阶段。
- 简易模式与专业模式改为两种创作界面：阅读书架优先呈现正文和进度，完整工作台呈现完整创作资料。两者共享自动创作、审校、恢复、编辑、导出和删除能力，切换界面不会暂停正在进行的章节生产。
- 自动导演标题区可直接打开简易模式或专业模式；失败任务会显示实际卡住的检查点，不再误显示为仍在实时推进。
- “继续创作”列表中的作品也可直接删除，与“我的作品”保持一致。
- 自动导演的问题管理新增“优先完成整本书”和“质量优先”两套方案，自动重试最多一次；可用正文的局部问题可以留到后续优化，也可以暂停等待确认。
...
- 自动导演的预计章节数可先清空再输入新数值，例如直接改为 30 章；输入中途不会自动跳回默认值。
- 自动导演在标题右侧提供清晰的“进入创作”入口，便于切换简易模式或专业模式；手动创建保持为独立的次要入口。
- 专业创作的导出菜单提供整本 TXT 正文下载，方便直接保存或阅读已完成章节。
