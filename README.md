...
- 自动导演会统一记录规划、正文、质量和后台运行中的问题，并在任务详情中显示问题类型、风险分、对应章节和实际处理方式。
- 可以为已接入自动创作链的问题选择自动重试、提醒后继续、暂停处理或结束任务；规则会真正控制章节流水线、安全暂停和后台任务恢复。
- 后台执行失去响应时，会按本次任务规则自动续跑、停在恢复入口或结束任务；运行记录只在处理动作成功生效后标记为已执行。
- 有可用正文的局部质量问题会作为可追踪提醒继续全书；明确重规划、异常用量、受保护内容和数据完整性风险会停在可恢复位置等待处理。
- 简易创作书架可以直接查看问题处理分数与最近记录，并就地打开本书的问题管理。
- 简易创作完成一个局部章节批次后，可以直接从书架继续生成剩余章节；系统会从首个没有正文的章节接续并保留已有内容。
- 全书进度按已保存正文与目标章节计算，局部批次完成不会再被误认为整本书完成。
