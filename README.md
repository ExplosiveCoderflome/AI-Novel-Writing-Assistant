...
- 模型连接测试、刷新模型、刷新余额、调整思考设置或移除厂商完成后，按钮会立即恢复可操作状态，不会继续显示处理中。
- 热门题材雷达进入页面后会先显示最近一次已保存的榜单结果，并在后台继续更新；新榜单有可展示内容后自动替换，避免等待期间出现空白。
- 热门题材雷达更新期间会显示当前任务的真实进度；任务尚未完成时不会再误显示为 100%。
- 热门题材雷达刷新公开榜单时，仍可直接查看已经生成的 AI 分析报告，不会被后台更新阻塞。
- AI 实况会直接说明市场雷达和自动导演每次规划的用途，方便辨认榜单归纳、市场判断、开书简报、资源匹配和书级方向候选。
- 模型路由管理新增结构化调用容错设置。服务端繁忙、超时或连接中断时默认额外重试 1 次；可以调整为 0 至 3 次，并在仍失败后使用已启用的备用模型。
- 自动导演首次生成书级方向时遇到临时模型服务错误，快速重试会沿用已保存的开书信息和市场简报重新生成，不会错误提示缺少恢复上下文。
