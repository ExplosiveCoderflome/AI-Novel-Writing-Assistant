...
### 2026-07-08（提示词可视化编辑、最终上下文中文化与高发提示词稳定性）

提示词编辑器继续面向正文生成调试收束：高级模板默认用中文可视化标签展示上下文、运行变量和槽位引用，成熟用户可以更直观地调整 `novel.chapter.writer`，不需要记忆底层 key。

正文生成最终 message 的上下文也更接近写作者和模型实际需要阅读的资料；高发结构化提示词补齐枚举表、JSON skeleton、输出归一化和重试诊断，降低反复修复与无效重试。

- 高级模板编辑器会把书级合约、章节任务、时间线、角色硬事实、章节标题、语气与节奏等引用渲染为标签；源码视图仍可查看原始 `{{...}}` token。
- `@` 引用菜单插入的是可视化标签，未知 token 会保留原文并以异常状态提示，避免保存时丢失模板内容。
- 正文生成缺少必需上下文时，保底标题会显示“时间线”等中文名称；书级合约、章节任务、当前局面和风格合约也减少内部 key、英文字段、调试键和角色 ID 泄漏。
- 任务单质量门禁、章节接收闸门和卷级章节列表提示词补齐合法枚举、结构化示例和重试说明，减少 `pass`、`acceptable`、百分制置信度或标题重试说明不清导致的修复压力。
- README 和公开文档站新增提示词编辑器说明与截图，提示词管理文档补充正文生成、高级模板、可视化引用标签、上下文面板、预览保存和治理边界的使用方法。
