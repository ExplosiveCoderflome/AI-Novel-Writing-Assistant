...
重大更新：系统设置、知识库兼容和角色形象图这几条链路一起更适合桌面化安装后的直接使用了，很多原本得回头改 `.env` 或靠猜的地方，现在都能在界面里完成。
- 支持图片生成的模型厂商现在可以直接在设置里配置图片模型、API 地址和密钥；桌面版后续做成安装包时，用户不必再为了出图能力单独改环境变量。
- 知识库相关设置开始兼容旧版本的 `.env` 习惯，旧 Embedding / Qdrant / 检索参数升级后更容易平滑接到当前设置体系，也降低了旧集合名在升级时被意外切换的风险。
- 知识库检索设置页补成了中文，同时下拉选择器针对超长模型名做了宽度和换行优化，模型选择、检索配置这类长选项不再容易显示不全。
- 角色形象图弹窗里的“最终发送 Prompt 预览”现在可以直接手动编辑，不必先点 AI 优化；新增自定义模型厂商时，也会先引导填写默认模型名，再在创建后刷新模型列表，减少首次配置时的误导感。
- 角色列表里的“形象图库”也补上了直接删除图片的能力。现在可以在图库卡片或大图预览里删除不需要的角色图；如果删的是当前主图，系统会自动补一张新的主图，不用再手动重新指定。
