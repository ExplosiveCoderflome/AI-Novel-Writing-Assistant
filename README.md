...
自动导演接管与继续已有进度更稳：重跑链路会正确清理依赖资源，节奏拆章恢复会按卷顺序补齐当前卷和后续卷，进入章节执行前会确认目标章节范围完整。

- 从故事宏观或角色准备重新生成时，会先安全清空后续卷战略、节奏拆章和章节细化资源，避免被临时空卷工作区误拦。
- 当角色资产已经存在时，可以直接从卷战略重新生成，只重跑卷战略与后续拆章资源。
- 前 10 章或指定章节范围跨卷时，系统会先完成当前卷章节列表，再生成后续卷节奏板和章节；缺少目标范围章节时不会直接进入章节执行。
- 整本范围的节奏拆章会按目标总章数重新校验。长篇项目里，如果某卷只生成了少量章节，系统会回到该卷补齐节奏板和章节，不会把短章节误当成整卷完成后直接进入章节执行。
