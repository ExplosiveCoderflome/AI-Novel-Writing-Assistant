...
- 正文生成、章节验收与局部修复会共用同一份读者体验目标，减少写作方向、验收标准和修复结果互相偏离。
- 全书阅读承诺、主角幻想、核心卖点、阶段回报与卷级回报会持续进入每章写作上下文，长篇推进更不容易丢失开书承诺。
- 旧章节计划会兼容读取；新生成的章节计划则必须提供完整体验合同和场景阻力、转折、情绪变化、读者价值，避免空泛场景卡继续进入正文。
- 普通爽点、钩子或情绪强度不足会进入本章修复建议或质量债，不会因此中断整本自动写作；未启用时间线时也不会产生误导性的质量警告。
- Book Contract 的第 3/10/30 章回报会稳定进入现有承诺账本；修改阶段回报后系统会在后台重新对账，避免章节知道承诺、后续规划却丢失承诺。
- 承诺账本同步失败时会保留上次成功结果，保存设定无需等待 AI 对账，重复更新也不会制造重复任务。
- 阶段回报被修改或移除时，旧承诺会退出后续正文义务，同时保留历史证据和已兑现记录，避免新旧承诺并存。
- 承诺逾期会作为质量提醒继续追踪，不会仅因逾期章数或当前章引用暂停整本生产；只有明确的计划失配或用户重规划要求才会停止。
- 知识资料库会显示可检索资料、同步任务和失败状态，并推荐上传、查看进度、修复索引或继续创作。
- 题材基底库会展示分类结构、关联小说和说明完整度；基础角色库会提示可复用角色、形象资料和待补核心信息。
- 知识、题材和角色入口统一为更清晰的创作资产工作台，加载、失败和空态都会说明下一步。
- 任务中心会汇总全局执行、等待操作和可恢复任务；任务列表、详情和导演跟进都提供清楚的失败恢复与重新读取入口。
- 章节级质量提醒、待确认节点和真正阻塞会使用不同等级表达，避免把可继续的局部问题误解成整本写作失败。
- 拆书工作台会直接展示来源、版本、范围、阶段、进度和计划小节完成度；成功有结果、成功无结果、部分完成和预算用尽都会给出对应的唯一下一步。
- 部分失败、取消或原文读取失败不会隐藏已生成的拆书内容，归档结果也仍可阅读和复制。
- 创作中枢会展示当前小说、阶段、线程状态和唯一推荐下一步，创作推进记录保留自由输入、工具结果、审批、编辑与重新生成能力。
- 切换创作线程会立即清空旧内容；读取失败可重试或切换线程，资源绑定和运行细节不会再挤占主创作区。
