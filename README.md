...
重大更新：`tk/*` 这组代理模型现在会按 Anthropic 协议调用，结构化任务和 token 统计的口径开始跟代理真实能力对齐；自动导演新增了独立的导演跟进中心；模型路由新增 `small_text` 短文本通道，真正短的规划提示会按近似 token `<= 2048` 单独分流；章节审阅后的自动重规划改成后台补跑并支持同因去重；默认 LLM 与线上 Ingress 超时统一提升到 600 秒。
- `tk/GLM-5.1`、`tk/Kimi-K2.5`、`tk/Kimi-K2.6` 以及其他 `tk/*` 模型现在统一改走 Anthropic `messages` 请求，不再继续误套 OpenAI 兼容语义。
- 这组模型在结构化生成里会同步退回更稳的 `prompt_json` 路线，减少节奏规划、结构化分析等任务里因为协议和能力口径不一致导致的异常。
- 服务端会把 Anthropic 返回的输入 token、输出 token 和缓存读取量映射回统一用量追踪，后续在代理观测和服务日志里更容易看清真实消耗。
- 如果模型返回正文正常、但输出 token 异常偏小甚至直接为 `0`，服务端现在会强制把该次请求的输入、输出、token 使用量和停止原因单独写入异常日志，排查中转或上游 usage 回传异常时不需要再临时开全量 debug。
- 角色动态投影在写回数据库前会先合并同角色、同卷的重复 assignment，重复主键导致的动态重建失败现在会被提前消掉，不再把整条链路卡死。
- 自动导演待处理任务现在可以在“导演跟进”页面集中查看，支持按原因筛选、看详情、继续自动执行、按任务模型重试，以及批量继续 / 批量重试，不用再在任务中心和小说编辑页之间来回找入口。
- 写法引擎提取出来的写法资产现在会把三套提取预设、每个特征的保留决策，以及关联的反 AI 规则一起保存下来；旧资产读回时也会自动补齐规则推导，继续编辑、绑定和复用时更不容易丢失原本的写法取舍。
- 模型路由设置现在新增了“短文本处理”通道，默认走 `DeepSeek`，可以单独查看、调整和做连通性检测。
- 服务端会记录实际生效的任务类型、渲染后提示长度和近似 token 数，后续排查“为什么被路由到短文本”会更直接。
- `small_text` 现在只按提示规模分流，不再对伏笔账本同步、角色动态卷投影这类后台 prompt 做写死特殊分配；这些请求只有在自身也足够短时才会走短文本通道。
- 导演跟进中心里的钉钉 / 企微通道能力现在会按系统真实配置判断。只有对应 webhook 真正配好了，页面和接口才会把该通道标成可用，避免未配置时误以为已经能直接走外部通道。
- 当模型按约定返回 ````json` fenced JSON，且前面还带有解释性文本、占位说明或类似 `[json]` / `{json}` 标记时，系统现在会优先提取真正的 JSON 体；结构化大纲、标题、世界观、规划与其他结构化链路在这类返回格式下更不容易因为误截到说明文本而解析失败。
- 系统设置菜单里现在补上了导演跟进通道配置，钉钉和企微的 webhook、回调 token、人员映射、订阅事件以及站内访问地址都可以直接在页面里维护，不需要再只靠环境变量硬配。
- 导演跟进中心现在可以把自动导演的审核、异常、恢复和完成状态直接推送到钉钉或企微；收到提醒后，可以从消息卡片里直接继续自动执行，或按任务当前模型发起低风险重试，不必再先回到站内找入口。
- 企微提醒消息现在改成官方 `markdown` 格式。配好回调 token 后，消息里会直接带上“继续自动执行”“按当前模型重试”“查看详情”“打开跟进中心”等可点击链接，不需要再面对一段自定义 JSON 或回站内手动找入口。
- 系统设置里的“订阅事件”现在改成中文说明的下拉多选，不再要求手动输入逗号分隔的事件码；选择时看中文即可，保存时仍会沿用原始事件编码，旧配置和后端能力不用额外迁移。
- GLM 兼容端点的结构化调用现在不再误判成 OpenAI 原生 `json_schema/json_object`。像 `glm-5` 这类模型即使返回的是 ```json 包裹的内容，也会走系统自己的 `prompt_json` 解析链路，不会再因为 SDK 过早按“纯 JSON”解析而直接报 `Unexpected token '` 这类错误。
- 钉钉卡片动作会继续走统一的跟进执行链路，并带上回调幂等键；同一个回调被重复触发时，系统会按同一轮动作识别，减少消息平台重放或连点导致的重复异步执行。
- 跟进详情页会同步展示最近一次钉钉 / 企微通道触达状态，包括是否送达、响应码和时间；当提醒没发出去或被回调网关拦住时，排查入口会更直接。
- 跟进消息里的站内链接现在支持单独配置访问地址，同时兼容旧版 `/auto-director/follow-up-center` 和 `/auto-director/followup-center` 入口；之前已经发出的旧消息链接，现在也更容易直接打开到新的导演跟进页。
- 导演跟进通道配置现在支持显式清空；如果数据库表暂时还没初始化好，系统也会自动回退到原来的环境变量配置，升级时不容易把已有提醒链路直接配断。
- 章节人工审阅如果命中“需要自动重规划”，接口现在会先把审阅结果返回，再在后台补跑重规划；同一章节短时间内重复触发相同原因时，也会自动去重，避免因为连点或重复请求把后台重规划并发打重复。
- LLM 默认超时上限已拉长到 10 分钟，卷级规划、长上下文结构化生成和代理链路波动时更不容易过早超时。
