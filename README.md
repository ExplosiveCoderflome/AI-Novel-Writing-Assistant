...
移动端创作体验、全站手机导航和主链稳定性一起更新：手机端进入主要页面会使用专属页面壳，首页、小说、创作、任务、资产、世界观和设置类工作区都补齐窄屏布局；自动导演、卷骨架、节奏 / 拆章、章节执行、整本重规划、任务中心和 PostgreSQL 发布链路也继续补齐体验与稳定性修复。

- 手机端全站使用轻量顶部栏和底部导航，首页、小说、创作、任务和更多入口可以直接切换；更多菜单集中放置拆书、导演跟进、知识库、题材、推进模式、标题工坊、写法引擎、基础角色、世界观和系统设置。
- 手机端导演跟进页的原因统计改为 2×2 排布，待跟进总数单独展示，让四类跟进入口在窄屏上更容易扫读和点击。
- 首页、小说列表、创建小说、创作中枢、拆书、任务中心、导演跟进、知识库、世界观、写法引擎和设置类页面都补齐手机端布局，不会再被桌面侧栏挤压。
- 手机端页面统一减少深层卡片嵌套，复杂工作台优先使用单列模块、轻量分区、横向可扫标签和全宽操作按钮，长表单、大弹窗和列表也会更适合窄屏阅读。
- 手机端小说编辑页会显示更适合窄屏的创作工作区，支持横向切换项目设定、故事宏观规划、角色准备、卷骨架、节奏拆章、章节执行、质量修复和版本历史。
- 手机端章节正文编辑页会优先展示正文区域，并保留返回、保存、AI 修改和诊断相关入口，让手机上继续查看和处理章节更顺手。
- 手机端「节奏 / 拆章」会按模块纵向展开，当前处理卷使用下拉选择，当前卷节奏、章节导航和章节细化减少卡片套卡片与双向滚动。
- 手机端自动导演状态从大蓝框改成上方独立轻卡片，并在向下浏览时保留一行不遮挡顶部栏的悬浮标识；卷骨架、节奏拆章和章节相关区域继续弱化最外层大卡片。
- 手机端保存入口改为可拖动悬浮按钮，减少顶部操作按钮占用内容区，也方便在长页面中随时保存当前工作区。
- 桌面端仍保持原有顶部栏、侧边导航和工作区布局；手机端支持集中在专属移动端组件和移动端样式中，避免影响桌面使用习惯。
- 节奏 / 拆章会严格校验单卷节奏板的章节跨度，要求连续覆盖该卷目标章数；3-4 章短卷会按实际章数压缩节奏段，不会被 5-8 个节奏段的常规规则卡住。
- 拆章标题会检查句法是否过度集中；如果一批章节大量套用同一种“X 的 Y”或“A，B”结构，系统会让 AI 自动重写，减少整卷章名模板化。
- PostgreSQL 部署会正确接管已存在但迁移记录异常的历史结构，发布后不会因为重复补列把 API 卡在启动阶段。
- 自动导演与小说主工作流的阶段同步恢复正常，任务中断后再继续时也会回到中断时所在的真实环节。
- 任务中心的全局任务列表可以稳定打开；卷规划与拆章生成里的结构化输出超时封装也已恢复。
- 整本重规划会以预计总章数重新推导卷数与每卷章节预算，重新生成章节执行批次也会按本次选择的执行范围重新启动。
- 自动导演接管已有项目时，重新生成故事宏观、角色准备或卷战略会同步清理后续规划产物；旧节奏板、旧拆章和旧章节细化不会再被恢复游标误用，减少重跑后出现重复标题或旧结构干扰。
- 自动导演继续已有进度时，会先核对卷规划、节奏拆章、章节细化和章节正文完成度；缺项会回到对应环节补齐，避免章节执行与实际章节计划不同步。
