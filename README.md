...
- 继续创作时可以在预计章节数以内选择较小的推进目标；保留预计章节数则继续使用自动补卷和滚动拆章。
- 自动导演继续创作会以预计章节数为整本书目标：先完成当前章节窗口，接近末尾时自动补后续卷骨架和近期拆章，再继续写作，不会要求手动回到拆章页接力，也不会提前生成整本书的远期章节任务。
- 自动导演会复用完整的章节任务；同一章在多个入口同时准备时只生成一次，减少异常模型调用与 Token 消耗。
- 全书自动创作中的任务单语义提示会保留给后续验收，不会再重写整份任务单；连接中断、空响应和结构接收失败会保留真实原因，不再触发多层重复生成。
- 正文会在适合章节长度的额度内生成，并关闭不必要的推理过程；验收、局部修文和章节事实回灌各自使用独立的小额度，减少无效 Token 消耗。
- 选定范围的章节都已保存完成时，系统会正常完成本次创作；异常用量仍会阻止尚未开始的后续章节继续消耗。
- 第三方或自定义兼容接口会先采用稳妥的 JSON 输出方式；连接检测确认支持后才使用对应的原生格式，减少截断和无效修复调用。
- AI 创作实况连接中断后会自动恢复并读取近期生成内容，不需要刷新页面，也不会重复触发模型任务。
- 自动导演开书时即可确认本书的问题处理方式；不调整时沿用默认规则，调整后的规则会随本书任务保存并在执行中保持一致。
- 正文生成与验收使用同一套退化检查边界，减少无效修复。
