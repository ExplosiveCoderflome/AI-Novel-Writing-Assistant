...
- 角色对话扩展到小说角色、基础角色库与拆书角色：从任一入口都能进入同一谈话工作台，并清楚看到回应依据和来源边界。
- 基础角色可进行只读访谈；拆书角色会锁定截至章节、依据原文回应；只有小说角色能在你确认后把一条谈话倾向带入后续创作。
- 基础角色库的对话区与右侧资料区会保持同高；长设定在资料区内滚动阅读，减少空白占用。
- 从基础角色库点击“与角色交谈”会直接进入全屏对话，退出后回到角色列表。
- 拆书角色会正确读取所选的截至章节，可以正常进入基于原文证据的访谈。
- 从拆书角色档案点击“基于原文访谈”会直接进入全屏对话，退出后回到角色档案。
- 长篇拆书的形象演变会优先显示关键章节，并支持分页或按需查看全部快照。
- 拆书角色访谈会复用逐章形象快照中的原文证据，让资料丰富的角色也能安全开始访谈。
- 三类角色的对话工作台采用更专注的创作阅读界面：谈话成为视觉中心，回应依据收进轻量场景分析侧栏，减少卡片、阴影和边框带来的干扰。
