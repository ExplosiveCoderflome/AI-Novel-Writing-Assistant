...
- 滚动规划只负责先准备下一章大纲，完整执行信息会在真正写作前补齐；章节生成、审核、修复和恢复共用同一套问题处理动作，暂停会保留在已保存章节边界，只有明确恢复后才继续。
- 从热门题材雷达开书时，最终勾选的每条信号会直接生成并回填起始想法，同时带入金手指或核心优势、核心卖点和前 30 章承诺；题材基底与推进模式会按这组信号重新匹配，开书页会用紧凑摘要呈现雷达方向。
