...
- 新增“热门题材雷达”，进入页面会先自动完成榜单采集并展示番茄、起点和晋江的公开数据；平台有新书或新晋榜单时，AI 只分析这些当前开书样本，缺失时才参考成熟榜单，分析结果也会主动收敛数量，减少结果过长中断。番茄标题会校正为正常文字，选中的市场信号会在第一次生成时直接参与自动导演的故事星图、整书方向和书名。
- 自动导演“故事星图”会按当前题材生成 35 项具体开书素材，覆盖主角、舞台、金手指、首章爆点、前期目标、核心阻力和关键关系；完整短句标签可以直接确认，桌面词条会自动换行并相互避让，再由 AI 整理成可直接开书的起始想法。
- 自动导演会根据故事卖点、推荐平台和目标读者直接确定每套方向的主书名，减少只有抽象情绪、看不出网文题材的命名结果。
- 模型配置优先展示推荐方案，同时支持管理内置厂商、自定义兼容服务以及独立的知识库向量连接。
- 章节细化中途失败时会保留已完成结果，并支持从失败章节继续；正文效果实验室可从章节和写法引擎直接进入，带入本书模板进行安全试写，文本试写会边生成边显示；提示词工作台在深色与夜航主题下保持清晰可读，深色暖纸的沉浸正文区不会再出现大块棕色工作底色。
