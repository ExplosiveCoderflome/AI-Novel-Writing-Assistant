...
提示词编辑器更适合调试正文生成。高级模板里的书级合约、章节任务、时间线、角色硬事实、章节标题、语气与节奏等引用会显示成中文标签，调整提示词时更容易看懂模型会读取哪些资料。

正文生成预览也更接近真实写作资料；关键背景、章节任务、角色状态和风格要求会用更清晰的中文标题展示。章节规划和评估类任务的重试方向也更明确，减少标题、摘要或章节功能反复生成仍改不到重点的情况。

- 输入 `@` 可以插入资料、章节参数或写作规则，插入后以中文标签显示。
- 源码视图仍保留给高级用户排查模板，日常编辑可以优先使用标签视图。
- 缺少关键写作资料时，预览会用中文名称说明缺少什么，方便回到对应模块补齐资料。
- README 和公开文档站补充提示词编辑器截图与使用方法，说明如何编辑高级模板、查看上下文、生成预览、保存修改和恢复官方模板。
