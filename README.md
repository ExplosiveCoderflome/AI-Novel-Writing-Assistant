...
- 势力图谱和地理地图支持直接拖动节点整理布局，并保留画布移动、缩放、完整视口和一键重置。
- 势力卡片支持双行名称，悬停或键盘聚焦时可以查看完整名称和势力类型。
- 关系文字会优先显示不遮挡的简短名称；悬停关系线可查看双方与完整关系，点击后可以固定详情。
- 关系线与详情浮层之间的悬停保持稳定，查看信息时不会反复闪动。
- 查看关系时会突出当前连线和两端节点；地理路线详情还会展示路线类型、距离和风险。
- 自动布局保持稳定，重置不会修改世界手册中保存的地点坐标或关系数据。
- 世界时间线使用事件节点和推进轨道呈现世界局势变化；桌面端横向浏览，窄屏自动切换纵向时间线。
- 写法资产列表继续使用卡片内展开的阅读方式，选中后可在当前卡片查看读感、规则、样本和操作。
- 写法应用与测试采用清晰的双步流程：先绑定到作品或章节，再生成试写结果对照读感；已有正文的修订仍从独立的去 AI 味入口完成。
- 新建写法从“用模板开始、说一句想法、从素材学习”三个创作起点进入，AI 起草默认优先呈现，熟悉规则时仍可从空白创建。
- 写法档案保持原有信息与展开方式，并用更轻的卡片层次、细色选中态和摘录式原文样本提升阅读舒适度。
- 顶栏提供模型设置入口；首次缺少可用模型时自动打开快捷配置，之后也可以随时更换厂商、密钥、地址和默认模型，高级模型路由仍集中在系统设置中。
- 第一次完成模型检测后，可直接用一句灵感开始第一本小说，并查看“说想法、选择方向、阅读首章”的创作路径。
- 开篇项目设定使用居中的单一创作画布，读者可连续完成故事开场与读者期待，封面视觉按需展开。
