...
- 旧版自动导演任务也能按本书当前的问题处理规则安全恢复；后台执行失去响应时只使用统一的一次自动恢复机会，避免整本任务在独立重试阈值下反复运行。
- 简易书架与专业工作台会统一显示章节待优化原因、来源和真实自动修复次数，并提供同一个修改与重新审校入口。
- 重新审校通过后会关闭章节待优化项；仍是局部问题时保留正文继续创作，只有明确需要整书重规划时才阻断。
- 自动导演的问题动作统一由问题处理规则决定，不再由独立风险评分改变流程；每章自动修复最多一次，轻修失败不会在恢复时自动升级为整章重写。
- 章节接收检查与时间线收束使用唯一执行入口，最终正文不会再经过重复检查或旧重规划循环。
