...
重大更新：`tk/*` 这组代理模型现在会按 Anthropic 协议调用，结构化任务和 token 统计的口径开始跟代理真实能力对齐；自动导演新增了独立的导演跟进中心；模型路由里也补上了“短文本处理”通道，并把超长维护请求从 `small_text` 里移出，审阅后的自动重规划改成后台补跑。
...
- 模型路由设置现在新增了“短文本处理”通道，短规划请求可以单独分流；像角色动态重建、伏笔账本同步这类超长后台维护请求，不会再被硬塞进 `small_text`。
- 章节审阅后若需要自动重规划，系统会先把当前审阅结果返回，再在后台补跑重规划，不再把整次审阅卡在后续规划上。
