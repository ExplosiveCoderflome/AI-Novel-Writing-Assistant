...
- 已有本地作品库升级后可继续生成章节，事实账本所需的数据会在启动时自动补齐。
- 自动导演遇到重规划提醒后可从任意继续入口稳定恢复，不会反复停在同一检查点。
- 关闭知识库检索时，资料不会再显示没有执行者的长期排队状态。
- 世界骨架生成过长、截断或超时时会给出可重试提示，不会持续卡住或保存半成品。
- 根据本书主题生成世界会先给出可直接开书的紧凑设定，降低本地模型因输出过长而中断的概率。
- 根据本书主题生成世界时会沿用当前选择的模型；使用本地 Ollama 等模型不再被要求配置 DeepSeek。
- 小说预览页可下载整本正文或当前章节，目录开关也会同步调整阅读区宽度。
