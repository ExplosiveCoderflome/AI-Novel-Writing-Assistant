...
角色资源账本开始进入长篇主链，自动导演和模型结构化输出也补上了更稳的恢复与排查能力。系统会帮你追踪角色手里的道具、线索、身份凭证、底牌和代价，并在继续任务、接管当前步骤、按卷或按章节范围推进时更贴近真实进度。

- 角色工作台新增关键资源视角，主角、长期角色和临时角色会按不同重点展示可用资源、隐藏底牌、已转交或已消耗的资源。
- 章节执行后的状态沉淀会同步识别角色资源变化。低风险变更可以自动进入账本，高风险或不确定变更会保留为待确认项。
- 后续章节的写作、审阅和修复会参考本章相关资源窗口，让 AI 更容易遵守谁拥有、谁知情、还能不能用、什么时候该兑现。
- 自动导演恢复和接管会更重视当前真实进度和用户选择的范围，减少误回前 10 章、跳错阶段或覆盖已有拆章成果。
- 模型路由和结构化输出容错补强了，小文本任务更容易走合适路由，极低 token 或疑似空响应也会更早暴露在日志里。
- 角色动态和卷规划预算更稳，重复角色-卷投影会被合并，章节预算不容易被少量种子章节或短卷节奏板带偏。
- 发布流程新增 `beta` 预发验证规则，功能稳定后再进入正式发布和桌面打包链路。
