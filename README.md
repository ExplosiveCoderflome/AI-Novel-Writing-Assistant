...
自动导演按卷拆章节列表时，不再会把旧工作区里少量开篇占位章节误当成整卷可信章节数；即使先前只落了几章 seed chapters，系统也会继续按原本的章节预算和节奏板推进，不会再因为章节数被压小而直接报“章节跨度异常”。
修复了“导演跟进”页面在部分待处理任务场景下打开直接白屏的问题。现在进入导演跟进中心时，列表、详情和批量操作区域都能稳定渲染，不会再因为前端状态同步异常而整页空白。
角色动态系统在自动导演拆章和卷规划连续保存时，也不会再短时间内重复触发整本角色卷投影重建；卷级职责和关系阶段同步会更稳，模型调用风暴风险明显降低。
自动导演继续或重跑指定章节范围时，现在会严格按你选定的区间推进。像 `11-190` 这类范围，不会再误清空 `1-10` 的已有内容，也不会因为旧批次的质检完成状态被直接带进人工处理。
当卷规划已经扩展到更大的章节数后，自动导演继续执行也会继承这次扩展后的范围，不会再因为请求里没重新带上章节计划，就退回默认前 `10` 章并过早结束。

当前版本也已经可以同时兼容原有本地 / 桌面 SQLite 数据库和 PostgreSQL 部署。桌面运行时会继续沿用原来的本地数据库路径；如果你已经在服务端或本地接入 PostgreSQL，也可以继续直接使用，不需要为了同步主干能力在两套数据库之间切换。

这次同步还把主干里已经稳定的桌面运行与生产主链能力合入了当前导演跟进版本，同时保留 lucky 分支原有的导演跟进、自动恢复和相关增强能力。
