...
### 9. 美股投研 Agent 与 MooMoo OpenD 直连策略引擎
- **MooMoo OpenD 本地 TCP 直连**：基于 44 字节二进制 Native Header 报文协议连通本地 OpenD (`127.0.0.1:11111`)，通过专用 Python SDK 桥接器解析 64 位 uint64 账号 ID，实现真实持仓与闲置资金零延迟同步；
- **三大操盘指南看板**：
  - **【指南一：已有仓位增减】**：评估现有持仓集中度风险（>30% 风控警报）与盈亏防线，给出加减仓与止盈止损策略；
  - **【指南二：新仓位建立】**：结合闲置现金与新增预算，优先调取 MooMoo 自选关注池与隔夜催化剂标的建立全新底仓；
  - **【指南三：昨日指南复盘与沉淀优化】**：对比历史操盘推荐与最新实盘成交，归因避险/收益并沉淀为长效交易纪律；
- **Prisma 数据库持久化 2D 可交互股票知识图谱与多源融合**：
  - **四源数据融合引擎**：融合 OpenD 实时盘口现价、网络新闻快讯催化剂、实盘资产持仓及用户人工研判；
  - **严格语义三元组 $(E_1 \rightarrow R \rightarrow E_2)$**：支持实体分类（`ROOT_STOCK`, `SUPPLIER`, `CLIENT`, `COMPETITOR`, `MACRO`, `CONCEPT`）与方向矢量边，搭配 2D SVG 拓扑网络与节点交互透视抽屉；
  - **Prisma 数据库持久化 (`StockKnowledgeGraphStore`) 与接口 (`/api/stock/knowledge-graph/update`)**：每只股票拥有数据库专属图谱，支持前端【人工修改图谱】一键落库，跨服务重启与每日更新无缝保留。
- **确定性风控数学校准层 (Guardrail Layer)**：100% 依赖纯数学公式计算买入资金上限与实盘现价硬替换，彻底杜绝 AI 假股价与假数字。
