...
test("chapter title diversity keeps frame detection while new hard validation is disabled", () => {
...
test("chapter title diversity no longer blocks repeated comma framing during generation", () => {
  const issue = getChapterTitleDiversityIssue([
    "签下合同，甜蜜同居",
    "房租超支，紧急筹钱",
    "林晓求职，首战告败",
...
test("volume chapter list prompt accepts structurally repetitive titles while hard validation is disabled", async () => {
  const calls = [];
...
test("volume chapter list prompt does not throw solely for repetitive title structure", async () => {
  const calls = [];
