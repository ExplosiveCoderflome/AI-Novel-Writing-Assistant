...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "7. voice 需要显式检查 AI 腔：模板化转场、泛化比喻、段尾升华、解释型心理描写、说明书式设定倾倒、角色对话同质化。",
      "8. pacing 与 engagement 需要检查写手级场景功能：本章是否有可见状态变化、代价、关系裂口或新义务。",
      "",
      "【issues 要求】",
      ...
      "4. 一旦正文有明显 AI 腔硬伤，应归入 voice issue，并给出具体替换方向：动作、对话、局面代价、角色口吻或生活噪音。",
      "5. 如果正文只是顺滑但没有场景功能，应归入 pacing 或 engagement issue，不要因为语言通顺就放过。",
    ].join("\n")),
    new HumanMessage([
      `小说：${input.novelTitle}`,
...
export const chapterRepairPrompt: PromptAsset<ChapterRepairPromptInput, string, string> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "4. 禁止用‘这一刻、与此同时、毫无疑问、复杂情绪、空气仿佛凝固、命运的齿轮’这类模板表达修复问题。",
      "5. 修复时必须优先保住场景功能：让信息、关系、风险、资源或决策至少有一项发生可见变化。",
      "",
      "【禁止事项】",
      "禁止加入问题清单未要求的大幅扩写。",
      "禁止通过新增大事件掩盖原问题。",
