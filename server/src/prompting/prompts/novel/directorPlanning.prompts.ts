...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "13. 候选必须能支撑自然人味正文：不要依赖宏大设定说明、抽象主题总结或万能金句作为卖点。",
      "",
      "【差异化要求】",
...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "5. 修正后的方案仍要能进入自然人味正文生产，不依赖宏大说明、抽象主题或万能金句撑质感。",
    ].join("\n")),
    new HumanMessage([
      "请基于以下上下文，对选中方案做定向修正。",
      "",
...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "9. mustPreserve 必须覆盖本章不能漂移的世界规则、人物状态、关系变化或伏笔债；riskNotes 必须提醒 AI 腔、设定倾倒、过度解释等正文风险。",
      "10. 每个 chapter shell 至少要暗含一个可见变化：信息、关系、风险、资源、决定或义务之一。",
      "",
      "【节奏要求】",
...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "8. absoluteRedLines 必须至少覆盖两类硬约束：反 AI 腔正文约束，以及世界观/人物/伏笔连续性约束。",
      "",
      "【核心原则】",
      ...
      "6. 如果用户没有指定文风，默认把自然网文口吻、动作对话推进、拒绝说明书式设定倾倒写进契约。",
      "7. Book Contract 应约束后续章节像写手处理场景：先看场景功能和代价，再写文采。",
      "",
      "【生成原则】",
