...
function createSystemPrompt(mode: ChapterTaskSheetQualityPromptInput["mode"]): string {
  ...
  return [
    ...
    "如果本章承诺了阶段信任、资源到手、关系推进、反压收益、隐患埋设等读者可见回报，通常应评估为 partial；只有几乎只铺设问题且没有可见回报时才适合 setup。",
    "sceneCards.mustAdvance 只能保存本章、本场景必须完成的当前推进项；如果最后一场同时写了本章收束和下一章入口事件，应判定为职责过载或边界泄漏，并建议把下一章入口移到 readerExperience.endingHook 或 nextChapterEntryState。",
