...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      "8c. 如果重要角色对白互换后仍然成立，或所有人都像同一个旁白在解释剧情，应归入 voice，并要求补角色专属节奏、避讳、试探、嘴硬、误读方式或小动作。",
      "8d. 如果段落只是把任务单内容顺滑翻译成正文，缺少现场物件、身体反应、打断、交易、失败后果或具体代价，应归入 plot 或 voice，不得 accepted。",
      "8e. 如果开头复述上一章、中段解释设定、结尾抽象升华三者出现任意一项且影响读感，应至少 repairable；严重时 rewrite_needed。",
      "9. assetSyncRecommendation 只判断资产同步优先级和是否需要全量伏笔对账，不要输出落库细节；如果本章产生新承诺、新资源、新信息差、新关系阶段或伏笔状态变化，priority 通常应为 high。",
