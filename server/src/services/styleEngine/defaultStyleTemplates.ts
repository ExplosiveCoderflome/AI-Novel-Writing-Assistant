import type { DefaultTemplateDefinition } from "./defaultTypes";
...
export const DEFAULT_STYLE_TEMPLATES: DefaultTemplateDefinition[] = [
  ...
  },
  {
    ...
    name: "网文自然人味连续流",
    description: "默认给长篇网文使用：按写手级四轮过闸，强化现场推进、人物口吻、场景代价和连续性。",
    category: "长篇网文",
    tags: ["去AI味", "连续性", "人味", "网文"],
    applicableGenres: ["玄幻", "都市", "悬疑", "科幻", "言情", "历史"],
    analysisMarkdown: "先按优秀写手的判断方式看每章是否有场景功能、角色选择、代价变化和连续性承接，再处理去 AI 味。正文必须像真实现场，而不是像模型在总结设定。",
    narrativeRules: {
      progressionMode: "chapter_chain",
      sceneUnitPattern: ["场景功能", "当前压力", "角色选择", "代价变化", "章末钩子"],
      multiPov: false,
      looping: false,
      endingStyle: "hook",
      summary: "每章只解决局部问题，同时带来新的代价、压力、选择或误解。",
    },
    characterRules: {
      allowSelfReflection: false,
      emotionExpression: "dialogue_and_action",
      ...
      facePriority: true,
      dialogueStyle: "distinct_by_role",
      summary: "人物通过动作、口头禅、回避、潜台词和选择暴露状态，不替他们解释。",
    },
    languageRules: {
      register: "colloquial",
      roughness: 0.55,
    ...
    defaultAntiAiRuleKeys: [
      ...
      "forbid-theme-summary",
      "risk-dialogue-too-functional",
      "risk-scene-without-state-change",
      "risk-dialogue-voice-swappable",
      "risk-task-sheet-transcription",
      "risk-no-scene-cost",
      "risk-repeated-sentence-structure",
      "risk-even-paragraph-length",
      "encourage-useless-action",
      ...
      "encourage-subtext-dialogue",
      "encourage-scene-cost",
    ],
  },
];
