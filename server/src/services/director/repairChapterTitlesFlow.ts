...
import { buildDirectorSessionState, getDirectorInputFromSeedPayload, getDirectorLlmOptionsFromSeedPayload, type DirectorWorkflowSeedPayload } from "./helpers";
...
export interface DirectorRepairChapterTitlesFlowDeps {
...
export async function repairChapterTitles(
  deps: DirectorRepairChapterTitlesFlowDeps,
  taskId: string,
  input?: {
    volumeId?: string | null;
  },
  ...
  const workspace = await deps.volumeService.getVolumes(novelId);
  ...
  await deps.workflowService.bootstrapTask({
    ...
    seedPayload: deps.buildDirectorSeedPayload(repairRequest, novelId, {
      directorSession,
  ...
  deps.scheduleBackgroundRun(taskId, async () => {
    await repairDirectorChapterTitles({
      ...
      buildDirectorSeedPayload: (request, targetNovelId, extra) => (
        deps.buildDirectorSeedPayload(request, targetNovelId, extra)
      ),
    });
  });
}
