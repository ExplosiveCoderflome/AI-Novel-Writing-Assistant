...
export interface DirectorCandidateStageRuntimeDeps {
...
export function isCandidateSelectionTask(input: {
  novelId?: string | null;
...
export function buildCandidateStageBaseRequest(
  taskId: string,
  seedPayload: DirectorWorkflowSeedPayload,
  ...
  const commercialTags = Array.isArray(seedPayload.commercialTags)
    ? seedPayload.commercialTags.filter((item: unknown): item is string => typeof item === "string" && item.trim().length > 0)
    : undefined;
  const continuationBookAnalysisSections = Array.isArray(seedPayload.continuationBookAnalysisSections)
    ? seedPayload.continuationBookAnalysisSections.filter((item: unknown): item is string => typeof item === "string" && item.trim().length > 0)
    : undefined;
  return {
...
export async function continueCandidateStageTask(
  deps: DirectorCandidateStageRuntimeDeps,
  taskId: string,
  input: {
    novelId?: string | null;
    status: string;
  ...
  const baseRequest = buildCandidateStageBaseRequest(taskId, input.seedPayload);
  if (!baseRequest) {
  ...
  deps.scheduleBackgroundRun(taskId, async () => {
    if (mode === "generate") {
      await deps.candidateStageService.generateCandidates(baseRequest);
      return;
    }
    ...
    if (mode === "refine") {
      await deps.candidateStageService.refineCandidates({
        ...baseRequest,
        previousBatches,
    ...
    if (mode === "patch_candidate") {
      await deps.candidateStageService.patchCandidate({
        ...baseRequest,
        previousBatches,
    ...
    await deps.candidateStageService.refineCandidateTitleOptions({
      ...baseRequest,
      previousBatches,
      batchId: candidateStage.batchId,
      candidateId: candidateStage.candidateId,
...
export async function runCandidateStageWithFailureHandling<T>(
  deps: DirectorCandidateStageRuntimeDeps,
  workflowTaskId: string | null | undefined,
  runner: () => Promise<T>,
): Promise<T> {
  try {
    return await deps.withWorkflowTaskUsage(workflowTaskId, runner);
  } catch (error) {
