...
export function resolveWorkflowContinuationFeedback(
  ...
  const scopeLabel = requestedScopeLabel || taskScopeLabel || i18next.i18next.t("gen.lib.novelWorkflowContinuation.gen_d7432bb5");
    ...
    return {
      ...
      message: task.failureSummary?.trim()
        ...
        || (options?.mode === "auto_execute_range"
          ...
          : i18next.i18next.t("gen.lib.novelWorkflowContinuation.gen_bb8020bb")),
    };
  }
  ...
  return {
    ...
    message: options?.mode === "skip_quality_repair"
      ...
      : options?.mode === "auto_execute_range"
        ? `已继续自动执行${scopeLabel}。`
        : i18next.i18next.t("gen.lib.novelWorkflowContinuation.gen_8c4efb64"),
  };
}
...
> | null | undefined): DirectorContinuationMode {
  ...
  if (
    ...
    || task?.currentStage?.includes(i18next.i18next.t("gen.lib.novelWorkflowContinuation.gen_3a7170b9"))
  ) {
