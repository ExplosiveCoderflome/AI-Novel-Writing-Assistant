      ...
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-sm">
          <div className="rounded-md bg-[#f2f8f6] px-3 py-2">
            <span className="text-xs text-muted-foreground">{t("gen.pages.promptWorkbench.components.PromptRunBar.gen_55b09bd2", "上下文估算")}</span>
          ...
          <div className="rounded-md bg-[#f4f7ff] px-3 py-2">
            <span className="text-xs text-muted-foreground">{t("gen.pages.promptWorkbench.components.PromptRunBar.gen_e857b170", "测试模型")}</span>
            <div className="font-semibold text-[#344d7a]">{t("gen.pages.promptWorkbench.components.PromptRunBar.gen_1399b5a4", "可选覆盖")}</div>
          </div>
          <div className="rounded-md bg-[#fff7e8] px-3 py-2">
            <span className="text-xs text-muted-foreground">{t("gen.pages.promptWorkbench.components.PromptRunBar.gen_8e7e4efb", "保存状态")}</span>
            <div className={cn(
              "font-semibold",
        ...
        <div className="flex flex-wrap justify-start gap-2 xl:justify-end">
          ...
          >
            <RotateCcw className="mr-2 h-4 w-4" />{t("gen.pages.promptWorkbench.components.PromptRunBar.gen_07246dad", "重置修改")}</Button>
        </div>
      </div>
      <Dialog open={testDialogOpen} onOpenChange={setTestDialogOpen}>
        <AppDialogContent
          title={t("gen.pages.promptWorkbench.components.PromptRunBar.gen_8a0b352a", "测试产出")}
          description={t("gen.pages.promptWorkbench.components.PromptRunBar.gen_563f9840", "选择本次测试使用的模型参数，系统会用当前未保存草稿生成一次结果。")}
          className="max-w-2xl"
          bodyClassName="bg-[#fbfdfb]"
          footer={(
            <>
              <Button type="button" variant="ghost" onClick={() => setTestDialogOpen(false)}>{t("gen.pages.promptWorkbench.components.PromptRunBar.gen_625fb26b", "取消")}</Button>
              <Button
                type="button"
        ...
        >
          <div className="space-y-4">
            ...
            <div className="rounded-md bg-[#fff7e8] px-3 py-2 text-xs leading-relaxed text-[#7a5620]">{t("gen.pages.promptWorkbench.components.PromptRunBar.gen_f6435f06", "测试产出会调用真实模型并消耗额度；结果只用于调试，不会保存为章节正文。")}</div>
          </div>
        </AppDialogContent>
      </Dialog>
    </div>
