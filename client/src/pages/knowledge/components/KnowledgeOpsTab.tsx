...
export default function KnowledgeOpsTab({
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[360px_1fr]">
      <Card>
        <CardHeader>
          <CardTitle>{i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_49e6a2ab")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2 text-sm">
          <div>{i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_9811f598")}</div>
          <div>{i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_0bd3819b")}</div>
          <div>{i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_cc3dfd03")}</div>
          <div>
            RAG 健康：
            <Badge variant="outline" className="ml-2">
              {ragHealth?.ok ? i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_fd6e80f1") : i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_c195df63")}
            </Badge>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="space-y-4">
        <Card>
          <CardHeader>
            <CardTitle>{i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_fb844b8b")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2 text-sm">
            {ragHealthNotice ? (
        ...
        <Card>
          <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            >
              ...
              {isClearingJobs ? i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_e329328e") : `清理已结束 ${finishedJobCount}`}
            </Button>
          </CardHeader>
          <CardContent className="space-y-2">
            ...
            {jobs.length === 0 ? (
              <div className="text-sm text-muted-foreground">{i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_360f80e4")}</div>
            ) : null}
            {jobs.map((job) => (
                  ...
                  <div className="flex shrink-0 flex-wrap items-center gap-2">
                    ...
                    {canDeleteRagJob(job) ? (
                      <Button
                        ...
                        aria-label={i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_64b1d61a")}
                      >
                        <Trash2 className="h-4 w-4" />
                        {deletingJobId === job.id ? i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_09f2fb82") : i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_2f4aaddd")}
                      </Button>
                    ) : null}
                  </div>
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_6d57a1c1")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            {failedJobs.length === 0 ? (
              <div className="text-sm text-muted-foreground">{i18next.t("gen.pages.knowledge.components.KnowledgeOpsTab.gen_b1652934")}</div>
            ) : null}
            {failedJobs.map((job) => (
              <div key={job.id} className="rounded-md border p-2 text-sm">
                <div className="font-medium">
