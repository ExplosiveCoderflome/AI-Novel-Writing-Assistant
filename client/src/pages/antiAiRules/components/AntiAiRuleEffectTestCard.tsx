    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-xl">
          <FlaskConical className="h-5 w-5" />{i18next.t("gen.pages.antiAiRules.components.AntiAiRuleEffectTestCard.gen_0ba12df3", "效果测试")}</CardTitle>
        <CardDescription>{i18next.t("gen.pages.antiAiRules.components.AntiAiRuleEffectTestCard.gen_88992628", "粘贴一段正文，检查规则会怎样判断 AI 味，并生成一版只用于预览的修订稿。")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-2 text-sm sm:grid-cols-2">
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.antiAiRules.components.AntiAiRuleEffectTestCard.gen_3455fcca", "测试规则")}</div>
            <div className="mt-1 font-semibold">{totalRuleCount}</div>
          </div>
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.antiAiRules.components.AntiAiRuleEffectTestCard.gen_f17f0a85", "临时加入")}</div>
        ...
        {props.previewRules.length > 0 ? (
          <div className="space-y-2 rounded-md border bg-muted/20 p-3">
            <div className="flex items-center justify-between gap-2">
              <div className="text-sm font-medium text-foreground">{i18next.t("gen.pages.antiAiRules.components.AntiAiRuleEffectTestCard.gen_2fb2fe27", "临时测试规则")}</div>
              <Button type="button" variant="ghost" size="sm" onClick={props.onClearPreviewRules}>{i18next.t("gen.pages.antiAiRules.components.AntiAiRuleEffectTestCard.gen_288f0c40", "清空")}</Button>
            </div>
            <div className="flex flex-wrap gap-2">
              {props.previewRules.map((rule) => (
                <button
                  ...
                  title={i18next.t("gen.pages.antiAiRules.components.AntiAiRuleEffectTestCard.gen_b9016d5f", "移出测试")}
                >
                  {rule.name}
                  <X className="h-3 w-3" />
                </button>
          ...
          <div className="rounded-md border border-dashed p-3 text-sm leading-6 text-muted-foreground">{i18next.t("gen.pages.antiAiRules.components.AntiAiRuleEffectTestCard.gen_b25fa53c", "从左侧规则列表加入测试，可以在不改变规则状态的情况下比较效果。")}</div>
        )}

        <textarea
          className="min-h-[180px] w-full rounded-md border bg-background p-3 text-sm leading-7"
          value={props.content}
          placeholder={i18next.t("gen.pages.antiAiRules.components.AntiAiRuleEffectTestCard.gen_7e51fbc8", "粘贴待检测正文。建议输入一段完整场景，便于判断总结腔、解释腔和模板感。")}
        ...
        {props.report ? (
          <div className="space-y-3 rounded-md border p-4">
            ...
            {props.report.violations.length > 0 ? (
              ...
              <div className="rounded-md border border-dashed p-3 text-sm text-muted-foreground">{i18next.t("gen.pages.antiAiRules.components.AntiAiRuleEffectTestCard.gen_91c34675", "没有发现值得进入修正流程的问题。")}</div>
            )}
          </div>
        ) : null}
        ...
        {props.rewritePreview ? (
          <div className="space-y-2">
            <div className="text-sm font-medium text-foreground">{i18next.t("gen.pages.antiAiRules.components.AntiAiRuleEffectTestCard.gen_b4d3f1ac", "修订稿预览")}</div>
