...
export default function TaskCenterMilestoneHistory({
  ...
  return (
    <div className="space-y-2">
      <div className="font-medium">{i18next.t("gen.pages.tasks.components.TaskCenterMilestoneHistory.gen_8e910f68")}</div>
      {milestones.map((item) => (
        <div key={`${item.checkpointType}:${item.createdAt}`} className="rounded-md border p-2 text-muted-foreground">
          ...
          <div className="mt-1 text-xs">{i18next.t("gen.pages.tasks.components.TaskCenterMilestoneHistory.gen_b6387af6")}</div>
        </div>
      ))}
    </div>
  );
