...
const SCENE_TYPE_LABELS: Record<SceneType, string> = {
  interior: i18next.t("gen.pages.comic.project.ScenesPanel.gen_4fd46565"),
  exterior: i18next.t("gen.pages.comic.project.ScenesPanel.gen_bf8f50c7"),
  landscape: i18next.t("gen.pages.comic.project.ScenesPanel.gen_fc30335a"),
  abstract: i18next.t("gen.pages.comic.project.ScenesPanel.gen_d8f11435"),
  other: i18next.t("gen.pages.comic.project.ScenesPanel.gen_0d98c747"),
};

const BIBLE_FIELDS: Array<{ key: keyof SceneBible; label: string; placeholder: string }> = [
  { key: "palette", label: i18next.t("gen.pages.comic.project.ScenesPanel.gen_cd99c486"), placeholder: i18next.t("gen.pages.comic.project.ScenesPanel.gen_e797771e") },
  { key: "keyElements", label: i18next.t("gen.pages.comic.project.ScenesPanel.gen_aa847230"), placeholder: i18next.t("gen.pages.comic.project.ScenesPanel.gen_872d6f35") },
  { key: "materials", label: i18next.t("gen.pages.comic.project.ScenesPanel.gen_e3623348"), placeholder: i18next.t("gen.pages.comic.project.ScenesPanel.gen_1503e10b") },
  { key: "ambiance", label: i18next.t("gen.pages.comic.project.ScenesPanel.gen_852d1479"), placeholder: i18next.t("gen.pages.comic.project.ScenesPanel.gen_9a1252d3") },
  { key: "layout", label: i18next.t("gen.pages.comic.project.ScenesPanel.gen_1d86438b"), placeholder: i18next.t("gen.pages.comic.project.ScenesPanel.gen_dcf37314") },
];
  ...
  return (
    <aside className="overflow-hidden rounded-lg border bg-background">
      <div className="border-b px-3 py-3">
        <p className="text-sm font-semibold">{i18next.t("gen.pages.comic.project.ScenesPanel.gen_b30e5c25")}</p>
        <p className="mt-0.5 text-xs text-muted-foreground">{i18next.t("gen.pages.comic.project.ScenesPanel.gen_925537c0")}</p>
      </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-[10px] text-muted-foreground">
                      ...
                      {hasSheet && <span className="ml-1.5 text-primary">{i18next.t("gen.pages.comic.project.ScenesPanel.gen_6bd9342d")}</span>}
                    </p>
                  </div>
                </div>
              </button>
...
function SceneDetail({
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => { onChanged(); toast.success(i18next.t("gen.pages.comic.project.ScenesPanel.gen_eaf9301f")); },
  ...
  return (
    <>
      ...
      <div className="flex items-start justify-between gap-3 border-b px-4 py-4">
        ...
        <button
          type="button"
          title={i18next.t("gen.pages.comic.project.ScenesPanel.gen_f141ac85")}
      ...
      <div className="grid lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.7fr)]">
        ...
        <div className="min-w-0 space-y-3 border-b p-4 lg:border-b-0 lg:border-r">
          ...
          <p className="text-xs text-muted-foreground">{i18next.t("gen.pages.comic.project.ScenesPanel.gen_e555b92b", "这些视觉约束会在生成该场景下每一格时注入提示词，锁定空间一致性。")}</p>
        ...
        <aside className="min-w-0 p-4">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-sm font-medium">{i18next.t("gen.pages.comic.project.ScenesPanel.gen_e847f2b4")}</p>
            {sheet.origin && hasSheet && (
              <span className="text-[10px] text-muted-foreground">
                {sheet.origin === "uploaded" ? i18next.t("gen.pages.comic.project.ScenesPanel.gen_ba7f57c5") : i18next.t("gen.pages.comic.project.ScenesPanel.aiGenerated")}
              </span>
            )}
          </div>
          <div className="flex min-h-[180px] items-center justify-center overflow-hidden rounded-md border bg-muted/30">
            {hasSheet ? (
              ...
              <div className="flex flex-col items-center gap-2 text-muted-foreground">
                ...
                <span className="text-xs">{i18next.t("gen.pages.comic.project.ScenesPanel.gen_b0b83a3f")}</span>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-1.5 text-muted-foreground">
                <ImageIcon className="h-8 w-8 opacity-30" />
                <span className="text-xs">{i18next.t("gen.pages.comic.project.ScenesPanel.gen_e6b2b290")}</span>
              </div>
            )}
          </div>
          {sheet.status === "error" && (
          ...
          <p className="mt-2 text-[11px] text-muted-foreground">{i18next.t("gen.pages.comic.project.ScenesPanel.gen_e4cdb182", "设定图会作为低权重参考图传给图像模型，只锁定色调/布局/材质，镜头仍按每格自由运镜。建议先保存场景圣经再生成。")}</p>
          <div className="mt-2 flex gap-2">
            ...
            >
              ...
              {hasSheet ? i18next.t("gen.pages.comic.project.ScenesPanel.gen_a7c23201") : i18next.t("gen.pages.comic.project.ScenesPanel.aiGenerated")}
            </Button>
            <Button
              type="button"
              size="sm"
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">{i18next.t("gen.pages.comic.project.ScenesPanel.gen_cc9155a5", "场景在生成分格脚本时自动识别，可在此编辑场景圣经并生成设定图，用于锁定跨格/跨话的空间一致性。")}</p>
        <Button type="button" size="sm" variant="outline" onClick={() => setShowAdd((v) => !v)}>
          <Plus className="h-4 w-4" />{i18next.t("gen.pages.comic.project.ScenesPanel.gen_461211c2", "添加场景")}</Button>
      </div>
      ...
      {showAdd && (
        <div className="flex gap-2 rounded-md border bg-muted/20 p-3">
          <input
            ...
            placeholder={i18next.t("gen.pages.comic.project.ScenesPanel.gen_0c1e726b")}
            value={newName}
            onChange={(e) => setNewName(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter" && newName.trim()) createMut.mutate(); }}
          />
          <Button type="button" size="sm" disabled={!newName.trim() || createMut.isPending} onClick={() => createMut.mutate()}>{i18next.t("gen.pages.comic.project.ScenesPanel.gen_e83a256e", "确认")}</Button>
          <Button type="button" size="sm" variant="outline" onClick={() => { setShowAdd(false); setNewName(""); }}>{i18next.t("gen.pages.comic.project.ScenesPanel.gen_625fb26b", "取消")}</Button>
        </div>
      )}

      {isLoading ? (
        <div className="py-12 text-center text-sm text-muted-foreground">{i18next.t("gen.pages.comic.project.ScenesPanel.gen_26b5bd49")}</div>
      ) : scenes.length === 0 ? (
        <div className="space-y-2 py-12 text-center text-sm text-muted-foreground">
          <MapPin className="mx-auto h-10 w-10 opacity-30" />
          <p>{i18next.t("gen.pages.comic.project.ScenesPanel.gen_28618e83")}</p>
          <p className="text-xs">{i18next.t("gen.pages.comic.project.ScenesPanel.gen_0138d439")}</p>
        </div>
      ) : (
