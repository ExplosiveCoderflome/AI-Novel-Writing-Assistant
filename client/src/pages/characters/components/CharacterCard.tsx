...
export function CharacterCard({
  ...
  const handleDeleteAsset = async (asset: ImageAsset) => {
    const confirmed = window.confirm("Error 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know.");
    if (!confirmed) {
      return;
    }
    try {
  ...
  return (
    <article className="overflow-hidden rounded-md border border-border/80 bg-background">
      ...
      <div className="grid gap-px border-y border-border/70 bg-border/70 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <CharacterFact label="Error 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know." value={character.interests} />
        <CharacterFact label="key experiences" value={character.keyEvents} />
      </div>

      <div className="space-y-3 px-4 py-4">
        <div>
          <div className="text-sm font-semibold text-foreground">Character image</div>
          <div className="mt-1 text-xs text-muted-foreground">After saving the main image, the character recognition can be maintained in subsequent visual generation.</div>
        </div>
      ...
      >
        <AppDialogContent
          ...
          title={previewAsset ? `${character.name} - 图片预览` : "Picture preview"}
