...
const IMAGE_STATUS_TEXT: Record<string, string> = {
  queued: i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_e5ac1d20"),
  running: i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_1ae3a984"),
  succeeded: i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_b6c4a445"),
  failed: i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_7f7de8a2"),
  cancelled: i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_2111ccbb"),
};
...
export function CharacterImageDialog({
  ...
  const [imageForm, setImageForm] = useState({
    stylePreset: i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_7f5c7c44"),
    negativePrompt: i18next.t("gen.pages.characters.components.CharacterImageDialog.lowClarityDistortedExtraneousBodyTextWatermark"),
  ...
  const currentSendModeLabel = promptMode === "direct"
    ? (directPromptSource === "optimized" ? i18next.t("gen.pages.characters.components.CharacterImageDialog.optimizePromptA") : i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_3ec5bafe"))
    : i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_e799da59");
  ...
  const optimizeMutation = useMutation({
    mutationFn: async () => {
      if (!character) {
        throw new Error(i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_05ce9b2e"));
      }
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!imageForm.provider) {
        throw new Error(i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_27649b80"));
      }
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-6 py-5">
          <section className="space-y-2 rounded-2xl border border-slate-200 bg-slate-50/65 p-4">
            <div className="space-y-1">
              ...
              <div className="text-xs leading-5 text-slate-500">{i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_a8a80917", "这里填写角色描述。点击“AI优化Prompt”后，会把这段描述整理成图片生成专用 prompt。")}</div>
            </div>
            <textarea
              ...
              placeholder={i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_8d71845f")}
              value={sourcePrompt}
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              <div className="space-y-2">
                <div className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">{i18next.t("gen.pages.characters.components.CharacterImageDialog.optimizeOutputLanguage")}</div>
                <div className="inline-flex w-full rounded-xl border border-slate-200 bg-slate-50 p-1 sm:w-auto">
                  ...
                  >{i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_a7bac223", "中文")}</Button>
                  <Button
                    type="button"
              ...
              <div className="flex min-w-0 flex-col gap-3 xl:items-end">
                <div className="flex flex-wrap items-center gap-2 xl:flex-nowrap">
                  ...
                  >
                    {optimizeMutation.isPending ? i18next.t("gen.pages.characters.components.CharacterImageDialog.optimizingInProgress") : i18next.t("gen.pages.characters.components.CharacterImageDialog.optimizePromptB")}
                  </Button>
                  <Button
                    type="button"
                    variant="ghost"
                  ...
                  >{i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_780bf992", "恢复原链路")}</Button>
                </div>

                <div className="flex flex-wrap items-center gap-2 text-sm xl:justify-end">
                  <span className="text-slate-500">{i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_ff04aacc")}</span>
          ...
          <section className="space-y-2 rounded-2xl border border-slate-200 bg-slate-50/55 p-4">
            <div className="space-y-1">
              ...
              <div className="text-xs leading-5 text-slate-500">{i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_5e63eeb2", "这里展示最终会发送给图像模型的 prompt。你可以直接手动编辑；AI 优化后，也可以继续在这里修改。")}</div>
            </div>
            <textarea
          ...
          <div className="grid gap-2 md:grid-cols-2">
            <input
              ...
              placeholder={i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_fdbc4d32")}
              value={imageForm.stylePreset}
              onChange={(event) => updateStylePreset(event.target.value)}
            />
            <input
              className="rounded-xl border border-slate-200 bg-white p-3 text-sm shadow-sm outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
              placeholder={i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_2300a232")}
              value={imageForm.negativePrompt}
              onChange={(event) => setImageForm((prev) => ({ ...prev, negativePrompt: event.target.value }))}
            />

            <label className="space-y-1 text-sm">
              <div className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">{i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_b51bd70b")}</div>
              <SelectControl
              ...
              >
                {imageProviderOptions.length === 0 ? (
                  <option value="">{i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_fca97c09")}</option>
                ) : null}
            ...
            <label className="space-y-1 text-sm">
              <div className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">{i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_c8339fd2")}</div>
              <SelectControl
                className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm shadow-sm outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
            ...
            <label className="space-y-1 text-sm md:col-span-2">
              <div className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">{i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_3922a4bb")}</div>
              <SelectControl
                className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm shadow-sm outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
              ...
              >
                <option value="1">{i18next.t("gen.pages.characters.components.CharacterImageDialog.onePage")}</option>
                <option value="2">{i18next.t("gen.pages.characters.components.CharacterImageDialog.twoPages")}</option>
                <option value="3">{i18next.t("gen.pages.characters.components.CharacterImageDialog.threePages")}</option>
                <option value="4">{i18next.t("gen.pages.characters.components.CharacterImageDialog.fourPages")}</option>
              </SelectControl>
            </label>
          </div>
          ...
          {activeTask ? (
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">
              <div>{i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_f33bd525")}</div>
              {activeTask.error ? (
          ...
          >
            {generateMutation.isPending ? i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_c661e656") : i18next.t("gen.pages.characters.components.CharacterImageDialog.gen_dac38a8b")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
