...
export function useAnalysisBudget(input: {
  ...
  const updateBudgetMutation = useMutation({
    ...
    onSuccess: async (response, payload) => {
      ...
      toast.success(i18next.i18next.t("gen.pages.bookAnalysis.hooks.actions.useAnalysisBudget.gen_858e0779"));
      await queryClient.invalidateQueries({ queryKey: ["book-analysis"] });
      await refreshAnalysisData(payload.id);
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : i18next.i18next.t("gen.pages.bookAnalysis.hooks.actions.useAnalysisBudget.gen_8ee0a0a4"));
    },
  });
  ...
  const resumeWithBudgetMutation = useMutation({
    ...
    onSuccess: async (response, payload) => {
      ...
      onAnalysisUpdated(response.data);
      toast.success(i18next.i18next.t("gen.pages.bookAnalysis.hooks.actions.useAnalysisBudget.gen_b0ca6863"));
      await queryClient.invalidateQueries({ queryKey: ["book-analysis"] });
      await refreshAnalysisData(payload.id);
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : i18next.i18next.t("gen.pages.bookAnalysis.hooks.actions.useAnalysisBudget.gen_8538bbd0"));
    },
  });
