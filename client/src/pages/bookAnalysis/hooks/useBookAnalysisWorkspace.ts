...
const DIAGNOSIS_FOCUS_INSTRUCTION = i18next.i18next.t("gen.pages.bookAnalysis.hooks.useBookAnalysisWorkspace.gen_516d055f");
...
export function useBookAnalysisWorkspace(): BookAnalysisWorkspace {
  ...
  const sourceChaptersError = sourceChaptersQuery.error instanceof Error
    ...
    : sourceChaptersQuery.error
      ? i18next.i18next.t("gen.pages.bookAnalysis.hooks.useBookAnalysisWorkspace.gen_9ccd1db5")
      : "";
  ...
  const createDiagnosisMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!document) {
        throw new Error(i18next.i18next.t("gen.pages.bookAnalysis.hooks.useBookAnalysisWorkspace.gen_06f985f9"));
      }
    ...
    onSuccess: async (result) => {
      ...
      await queryClient.invalidateQueries({ queryKey: queryKeys.bookAnalysis.list(listKey) });
      toast.success(i18next.i18next.t("gen.pages.bookAnalysis.hooks.useBookAnalysisWorkspace.gen_bf1d4971"));
    },
  });
