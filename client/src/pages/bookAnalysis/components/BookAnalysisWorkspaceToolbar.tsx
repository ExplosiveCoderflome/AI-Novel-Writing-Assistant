      ...
      <div className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="text-sm font-semibold tracking-normal text-foreground">{t("gen.pages.bookAnalysis.components.BookAnalysisWorkspaceToolbar.gen_b9bb2ae9", "结果工具")}</h2>
            <Badge variant="outline">{formatStatus(selectedAnalysis.status)}</Badge>
            {selectedAnalysis.publishedDocumentId ? <Badge variant="secondary">{t("gen.pages.bookAnalysis.components.BookAnalysisWorkspaceToolbar.gen_dca0c13b", "已发布")}</Badge> : null}
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">{t("gen.pages.bookAnalysis.components.BookAnalysisWorkspaceToolbar.gen_9df93f7a", "阅读结果是当前主任务；发布、导出和维护操作可按需使用。")}</p>
        </div>
        <div className="mobile-full-actions flex flex-wrap gap-2">
          ...
          <Button asChild size="sm" variant="outline">
            <Link to={`/tasks?kind=book_analysis&id=${selectedAnalysis.id}`}>{t("gen.pages.bookAnalysis.components.BookAnalysisWorkspaceToolbar.gen_4a98bf0c", "任务详情")}</Link>
          </Button>
        </div>
      </div>

      <details className="border-t border-border/70 px-4 py-3">
        <summary className="cursor-pointer text-xs font-medium text-muted-foreground">{t("gen.pages.bookAnalysis.components.BookAnalysisWorkspaceToolbar.gen_bf7dbd72", "更多维护操作")}</summary>
        <div className="mobile-full-actions mt-3 flex flex-wrap gap-2">
          <Button size="sm" variant="outline" onClick={onCopy} disabled={pending.copy}>{t("gen.pages.bookAnalysis.components.BookAnalysisWorkspaceToolbar.gen_b12bc66c", "复制分析")}</Button>
          <Button
            size="sm"
            variant="outline"
            onClick={() => onRebuild(selectedAnalysis.id)}
            disabled={pending.rebuild || selectedAnalysis.status === "archived"}
          >{t("gen.pages.bookAnalysis.components.BookAnalysisWorkspaceToolbar.gen_a7c23201", "重新生成")}</Button>
          {canAdjustBudget ? (
            <Button
              type="button"
              size="sm"
            ...
            >
              <Pencil className="mr-1.5 h-3.5 w-3.5" aria-hidden="true" />{t("gen.pages.bookAnalysis.components.BookAnalysisWorkspaceToolbar.gen_c4c506a4", "调整预算")}</Button>
          ) : null}
          <Button size="sm" variant="outline" onClick={() => onDownload("markdown")}>{t("gen.pages.bookAnalysis.components.BookAnalysisWorkspaceToolbar.gen_6f498373", "导出 MD")}</Button>
          <Button size="sm" variant="outline" onClick={() => onDownload("json")}>{t("gen.pages.bookAnalysis.components.BookAnalysisWorkspaceToolbar.gen_e1b05f87", "导出 JSON")}</Button>
          <Button
            size="sm"
            variant="outline"
          ...
          >{t("gen.pages.bookAnalysis.components.BookAnalysisWorkspaceToolbar.gen_2f51c18f", "归档")}</Button>
        </div>
      </details>
    </div>
  );
