    ...
    <div className="space-y-3">
      ...
      {sourceLoading || chaptersLoading ? (
        <div className="flex items-center gap-2 rounded-md border border-info/25 bg-info/5 p-3 text-sm text-muted-foreground" aria-live="polite">
          <Loader2 className="h-4 w-4 animate-spin text-info" aria-hidden="true" />{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_c8e2b6a6", "正在加载原文阅读位置，拆书结果仍可继续查看。")}</div>
      ) : null}
      ...
      {sourceError || chaptersError ? (
        <div className="flex flex-col gap-3 rounded-md border border-warning/30 bg-warning/5 p-3 text-sm sm:flex-row sm:items-center sm:justify-between" role="status">
          <div className="flex min-w-0 items-start gap-2">
            ...
            <div>
              <div className="font-medium text-foreground">{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_f44f4b76", "原文对照暂时不可用")}</div>
          ...
          >
            <RefreshCw className="mr-1.5 h-3.5 w-3.5" aria-hidden="true" />{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_0873b967", "重试原文加载")}</Button>
        </div>
      ) : null}
        ...
        <div className="space-y-3">
          <details className="rounded-md border bg-background p-3">
            <summary className="cursor-pointer list-none">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <div className="text-sm font-medium">{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_20e71449", "分析信息与发布")}</div>
                ...
                <Badge variant="outline">{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_e2edde5a", "展开")}</Badge>
              </div>
            </summary>
            <div className="mt-3 space-y-3">
              ...
              <div className="rounded-md border p-3 text-sm">
                <div className="mb-2 font-medium">{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_a3db6e46", "发布到小说知识库")}</div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    <option value="">{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_58d92a5f", "选择目标小说")}</option>
                  ...
                  >{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_7ad79546", "发布并绑定")}</Button>
                </div>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <div className="rounded-md border p-3 text-sm">
                  <div className="font-medium">{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_7f1b21a5", "概要")}</div>
                  <div className="mt-2 whitespace-pre-wrap text-muted-foreground">
                    {selectedAnalysis.summary?.trim() || "生成总览后会在此显示概要内容。"}
                  </div>
                </div>
                <div className="rounded-md border p-3 text-sm">
                  <div className="font-medium">{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_5d37ac7d", "运行元信息")}</div>
                  <div className="mt-2 space-y-1 text-muted-foreground">
                    ...
                    {budgetTokens ? (
                      <div
                        ...
                        role="progressbar"
                        aria-label={t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_0a9a0488", "拆书预算使用进度")}
                        aria-valuemin={0}
                        aria-valuemax={100}
          ...
          <section className="rounded-md border bg-background">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b p-3">
              <div className="flex flex-wrap items-center gap-2">
                <div className="text-base font-semibold">{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_ff3b883f", "拆书内容")}</div>
              ...
              <div className="flex rounded-md border bg-background p-1">
                ...
                >{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_cf503912", "重点速览")}</Button>
                <Button
                  size="sm"
                  variant={readingMode === "full" ? "default" : "ghost"}
                  onClick={() => setReadingMode("full")}
                >{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_73e4dea1", "完整阅读")}</Button>
              </div>
            </div>
            <div className="space-y-3 p-3">
              {selectedAnalysis.sections.length === 0 ? (
                <div className="rounded-md border border-dashed border-warning/40 bg-warning/5 px-5 py-8 text-center">
                  ...
                  <div className="mt-3 text-sm font-medium text-foreground">{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_3540059d", "没有可展示的拆书小节")}</div>
                  <p className="mx-auto mt-1 max-w-xl text-sm leading-6 text-muted-foreground">{t("gen.pages.bookAnalysis.components.BookAnalysisDetailPanel.gen_c948d4e9", "这份任务没有返回可阅读内容。源文档仍然安全，可以从上方重新生成或打开任务中心查看详情。")}</p>
                </div>
              ) : (
              <Tabs
