    ...
    <div className="space-y-4">
        ...
        actions={(
          <>
            <Button type="button" variant="outline" onClick={() => setCreateDialogOpen(true)}>
              <Plus className="mr-1.5 h-4 w-4" aria-hidden="true" />{t("gen.pages.bookAnalysis.BookAnalysisPage.gen_989a71a3", "新建拆书")}</Button>
            {workspace.selectedAnalysis ? (
              <OpenInCreativeHubButton
                ...
                label={t("gen.pages.bookAnalysis.BookAnalysisPage.gen_0b0ffdc8", "在创作中枢引用")}
              />
            ) : null}
          </>
        )}
      ...
      {workspace.selectedAnalysisId && workspace.queryState.detailLoading ? (
        <WorkspaceStateNotice
          loading
          tone="info"
          title={t("gen.pages.bookAnalysis.BookAnalysisPage.gen_976c1767", "正在读取拆书结果")}
          description={t("gen.pages.bookAnalysis.BookAnalysisPage.gen_b5ff0509", "结果加载完成后会直接显示可阅读小节和原文证据。")}
        />
      ) : workspace.selectedAnalysisId && workspace.queryState.detailError ? (
        <WorkspaceStateNotice
          tone="danger"
          title={t("gen.pages.bookAnalysis.BookAnalysisPage.gen_839d2c7c", "无法读取这份拆书结果")}
          ...
          action={(
            <Button type="button" size="sm" variant="outline" onClick={workspace.retryDetail}>
              <RefreshCw className="mr-1.5 h-3.5 w-3.5" aria-hidden="true" />{t("gen.pages.bookAnalysis.BookAnalysisPage.gen_af52d9d7", "重试详情")}</Button>
          )}
        />
        ...
        <WorkspaceStateNotice
          loading
          tone="info"
          title={t("gen.pages.bookAnalysis.BookAnalysisPage.gen_955d343a", "正在读取拆书列表")}
          description={t("gen.pages.bookAnalysis.BookAnalysisPage.gen_56d8783d", "正在确认已有分析和最近进度，加载完成后会给出下一步。")}
        />
      ) : !workspace.selectedAnalysisId && workspace.queryState.analysesError ? (
        <WorkspaceStateNotice
          tone="danger"
          title={t("gen.pages.bookAnalysis.BookAnalysisPage.gen_4d24e3ef", "无法读取拆书列表")}
          ...
          action={(
            <Button type="button" size="sm" variant="outline" onClick={workspace.retryAnalyses}>
              <RefreshCw className="mr-1.5 h-3.5 w-3.5" aria-hidden="true" />{t("gen.pages.bookAnalysis.BookAnalysisPage.gen_64ca9bab", "重新加载")}</Button>
          )}
        />
      ) : (
        <WorkspaceNextAction
          ...
          {workspace.selectedAnalysis ? (
            ...
            <WorkspaceStateNotice
              ...
              action={workspace.analyses.length === 0 ? (
                <Button type="button" size="sm" onClick={() => setCreateDialogOpen(true)}>{t("gen.pages.bookAnalysis.BookAnalysisPage.gen_989a71a3", "新建拆书")}</Button>
              ) : null}
            />
          )}
        </div>
