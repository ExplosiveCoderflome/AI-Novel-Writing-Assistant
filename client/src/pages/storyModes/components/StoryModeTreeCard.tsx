          ...
          <div className="flex shrink-0 flex-wrap justify-end gap-1">
            {depth === 0 ? (
              <Button type="button" variant="ghost" size="sm" onClick={() => onCreateChild(node.id)}>
                <Plus className="mr-1 h-4 w-4" />{i18next.t("gen.pages.storyModes.components.StoryModeTreeCard.gen_82537dc1", "新增子类")}</Button>
            ) : null}
            <Button type="button" variant="ghost" size="sm" onClick={() => onEdit(node.id)}>
              <Pencil className="mr-1 h-4 w-4" />{i18next.t("gen.pages.storyModes.components.StoryModeTreeCard.gen_95b351c8", "编辑")}</Button>
            <Button
              type="button"
              variant="ghost"
              size="sm"
