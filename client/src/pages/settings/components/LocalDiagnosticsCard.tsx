    ...
    <Card className="border-border">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div className="space-y-1">
          ...
          <CardDescription>{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_45d37fcd", "诊断系统平台硬件配置，自适应生成最匹配的本地模型加速级别。")}</CardDescription>
        </div>
        <Button
          variant="outline"
          size="icon"
      ...
      <CardContent className="space-y-4 pt-2">
        {isLoading ? (
          <div className="flex items-center justify-center py-6 text-sm text-muted-foreground gap-2">
            <RefreshCw className="h-4 w-4 animate-spin" />{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_fd3da9ca", "正在收集平台硬件与 GPU 拓扑信息...")}</div>
        ) : error ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-3 text-xs md:grid-cols-2">
              <div className="rounded-md border p-3 bg-muted/40 flex flex-col gap-2">
                <span className="font-bold text-muted-foreground uppercase tracking-wider">{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_14d3ddc6", "系统与算力架构")}</span>
                <div className="space-y-1.5 text-foreground">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_30d23ef4", "操作系统")}</span>
                  ...
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_ed412d8f", "逻辑核心")}</span>
                    <span className="font-semibold">{diagnostic.cpuCores} 核</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_2ccbf5c9", "系统物理内存")}</span>
              ...
              <div className="rounded-md border p-3 bg-muted/40 flex flex-col gap-2">
                <span className="font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
                  <Server className="h-3.5 w-3.5" />{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_dfabd7a3", "显卡加速详情")}</span>
                <div className="space-y-1.5 text-foreground">
                  {diagnostic.isAppleSilicon ? (
                    <>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_8636a795", "加速类型")}</span>
                        <span className="font-semibold text-emerald-600 dark:text-emerald-400">Apple Silicon / MPS</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_bd672794", "可用统一内存")}</span>
                        <span className="font-semibold">{diagnostic.totalMemoryGb} GB</span>
                      </div>
                    </>
                    ...
                    <>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_f96a07ec", "显卡型号")}</span>
                        <span className="font-semibold text-right truncate max-w-[200px]" title={diagnostic.gpu.name}>
                          {diagnostic.gpu.name}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_ce80bf17", "总显存")}</span>
                        <span className="font-semibold">{(diagnostic.gpu.totalVramMb / 1024).toFixed(2)} GB</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_19ee09b4", "空闲显存")}</span>
                        <span className="font-semibold">{(diagnostic.gpu.freeVramMb / 1024).toFixed(2)} GB</span>
                      </div>
                    </>
                  ) : (
                    <div className="h-full flex items-center justify-center text-muted-foreground text-center py-4">{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_1e6c23cd", "未检测到 NVIDIA CUDA 显卡")}<br />{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_cf1b8463", "或 Apple Silicon MPS 加速器")}</div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="text-xs text-muted-foreground bg-muted/20 border rounded p-2.5 flex items-center justify-between">
              <span>{t("gen.pages.settings.components.LocalDiagnosticsCard.gen_4ba5c68a", "当前模式下单张分镜画稿预期渲染耗时")}</span>
