    ...
    <Card className="min-w-0 overflow-hidden">
      <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 space-y-1">
          <CardTitle>{t("gen.pages.settings.components.StyleEngineRuntimeSettingsCard.gen_2419b570", "写法引擎运行设置")}</CardTitle>
          <CardDescription className={AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}>{t("gen.pages.settings.components.StyleEngineRuntimeSettingsCard.gen_918bb1f1", "控制写法提取等待模型返回的最长时间。长篇原文提取可以适当调高，短文本保持较短更容易发现异常。")}</CardDescription>
        </div>
      ...
      <CardContent className="space-y-4">
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  {active ? <Badge variant="default">{t("gen.pages.settings.components.StyleEngineRuntimeSettingsCard.gen_89051ebd", "当前选择")}</Badge> : null}
                </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          >{t("gen.pages.settings.components.StyleEngineRuntimeSettingsCard.gen_e370757f", "高级设置")}<ChevronDown className={cn("h-4 w-4 transition-transform duration-200", advancedOpen ? "rotate-180" : "")} />
          </button>
          <Button
        ...
        {advancedOpen ? (
          <div className="space-y-2 rounded-md border bg-muted/20 p-3">
            <div className="text-sm font-medium">{t("gen.pages.settings.components.StyleEngineRuntimeSettingsCard.gen_5ec3428c", "写法提取超时（分钟）")}</div>
            <Input
              type="number"
