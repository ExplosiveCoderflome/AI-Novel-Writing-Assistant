    ...
    >
      <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 space-y-1">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            ...
            {provider.kind === "custom" ? <Badge variant="outline">{i18next.t("gen.pages.settings.components.ProviderStatusCard.gen_f1d4ff50", "自定义")}</Badge> : null}
          </div>
      ...
      <div className="mb-3 grid min-w-0 gap-2 text-sm md:grid-cols-2">
        <div className="min-w-0 rounded-md border bg-background/70 p-2">
          <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.settings.components.ProviderStatusCard.gen_2415bb91", "文本模型")}</div>
          <div className={`mt-1 font-medium ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
            {provider.currentModel || "-"}
          </div>
        </div>
        <div className="min-w-0 rounded-md border bg-background/70 p-2">
          <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.settings.components.ProviderStatusCard.gen_89208d23", "图像模型")}</div>
          <div className={`mt-1 font-medium ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
            {imageModelLabel}
          </div>
        </div>
      ...
      <div className="mt-3 border-t pt-3">
        ...
        >
          <span>{i18next.t("gen.pages.settings.components.ProviderStatusCard.gen_808e9dd3", "高级详情")}</span>
      ...
      {advancedOpen ? (
        <div className="mt-3 space-y-3">
          ...
          <div className="flex flex-col gap-3 rounded-md border bg-background/60 px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0 space-y-1">
              <div className="text-xs font-medium text-muted-foreground">{i18next.t("gen.pages.settings.components.ProviderStatusCard.gen_6441c3c1", "思考功能")}</div>
              <div className={`text-xs text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>
          ...
          <div className="rounded-md border border-dashed bg-background/60 p-3">
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              <div className="text-xs font-medium text-muted-foreground">{i18next.t("gen.pages.settings.components.ProviderStatusCard.gen_bb9a2b05", "余额明细")}</div>
              {balance?.status === "available" ? (
                <Badge variant="outline">最近刷新 {formatBalanceTime(balance.fetchedAt)}</Badge>
              ) : null}
            </div>
            {provider.kind === "custom" ? (
              <div className={`text-sm text-muted-foreground ${AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}`}>{i18next.t("gen.pages.settings.components.ProviderStatusCard.gen_5d54ba46", "自定义 OpenAI 兼容厂商暂不接入余额查询。")}</div>
            ) : balance?.status === "available" ? (
