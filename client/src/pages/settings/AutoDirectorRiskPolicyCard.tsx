...
export function AutoDirectorRiskPolicyCard(props: {
  ...
  return (
    <Card className="min-w-0 overflow-hidden">
      <CardHeader>
        <CardTitle>{i18next.t("settings.autoDirectorRiskPolicyCard.h2tpc6")}</CardTitle>
        <CardDescription className={AUTO_DIRECTOR_MOBILE_CLASSES.wrapText}>{i18next.t("settings.autoDirectorRiskPolicyCard.8dnzh8")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-3 md:grid-cols-2">
          <label className="space-y-2">
            <span className="text-sm font-medium text-foreground">{i18next.t("novels.novelDirectorRiskPolicyCard.d6xobw")}</span>
          ...
          <label className="space-y-2">
            <span className="text-sm font-medium text-foreground">{i18next.t("novels.novelDirectorRiskPolicyCard.y8n3j0")}</span>
            ...
            <span className="block text-xs leading-5 text-muted-foreground">{i18next.t("settings.autoDirectorRiskPolicyCard.d9tedc")}</span>
          </label>
        </div>
        {unavailable ? <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900">{i18next.t("settings.autoDirectorRiskPolicyCard.m19em4")}</div> : null}
