...
function assetCompleteness(draft: DramaCharacterAssetInput) {
  const items = [
    { key: "screenRole", label: i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_628d3842"), done: Boolean(draft.screenRole.trim()) },
    { key: "audienceRead", label: i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_278bec03"), done: Boolean(draft.audienceRead.trim()) },
    { key: "visualAnchor", label: i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_88498354"), done: Boolean(draft.visualAnchor.trim()) },
    { key: "voiceAnchor", label: i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_d42b397c"), done: Boolean(draft.voiceAnchor.trim()) },
    { key: "lineRule", label: i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_96657d7d"), done: Boolean(draft.lineRule.trim()) },
  ];
  return {
    items,
    ...
    imageFlow.start({
      ...
      generate: async (overrides) => {
        ...
        try {
          ...
          setSheet(result.data ?? { status: "error", error: i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_c5e6d122") });
          return result;
        } catch (error) {
          setSheet({ status: "error", error: error instanceof Error ? error.message : i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_7f7de8a2") });
          throw error;
        } finally {
          setBusy(false);
        }
  ...
  return (
    <div className="space-y-3 border-t pt-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-sm font-medium">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_45d713bf")}</p>
          <p className="text-xs text-muted-foreground">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_20a3f22c")}</p>
        </div>
        {imageProviders.length > 0 ? (
          ...
          <span className="text-xs text-destructive">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_315e0fb8")}</span>
        )}
      </div>
        ...
        >
          {sheet.status === "generating" ? (
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_584fecff")}</span>
            </div>
          ) : sheet.status === "error" ? (
            <div className="flex flex-col items-center gap-1 px-4 text-center">
              <ImageIcon className="h-5 w-5 text-destructive" />
              <span className="text-xs text-destructive">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_sheeterror_k4w2")}</span>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-1">
              <ImageIcon className="h-6 w-6" />
              <span className="text-xs">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_ad282ec2")}</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          ...
          {sheet.status === "done" ? i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_52cfd5d4") : i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_1f81b704")}
        </Button>
        {sheet.status === "done" && (
          <>
            <Badge variant="outline">v{sheet.version ?? 1}</Badge>
            <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_e880376c")}</span>
          </>
        )}
        {sheet.history?.length ? <Badge variant="secondary">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_sheethisto_sa99")}</Badge> : null}
      </div>
...
function CharacterImageHistory({ history }: { history: NonNullable<DramaCharacterPortraitData["history"]> }) {
  ...
  return (
    <div className="rounded-md border border-dashed p-3 text-xs">
      <div className="mb-2 font-medium">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_ab381f57")}</div>
      ...
      <CardHeader>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-2">
            ...
            <CardDescription>{i18next.t("gen.pages.drama.components.DramaCharactersPanel.draftRolePrompt")}</CardDescription>
          </div>
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="block space-y-1.5 text-sm">
            <span className="font-medium">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_11b8c0b8")}</span>
            <input
          ...
          <label className="block space-y-1.5 text-sm">
            <span className="font-medium">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_bf513d2f")}</span>
            <input
              className="h-10 w-full rounded-md border bg-background px-3 text-sm"
              value={draft.screenRole}
              placeholder={i18next.t("gen.pages.drama.components.DramaCharactersPanel.mainOpponentSupportObject")}
        ...
        <label className="block space-y-1.5 text-sm">
          <span className="font-medium">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_766d0fce")}</span>
          <textarea
            className="min-h-20 w-full rounded-md border bg-background px-3 py-2 text-sm"
            value={draft.audienceRead}
            placeholder={i18next.t("gen.pages.drama.components.DramaCharactersPanel.exampleSheSeemedHumiliatedButAlwaysCalmAudienceMustBelieveHer")}
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <label className="block space-y-1.5 text-sm">
            <span className="flex items-center gap-1 font-medium">
              <Video className="h-4 w-4" />{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_752f815a", "固定造型锚点")}</span>
            <textarea
              className="min-h-24 w-full rounded-md border bg-background px-3 py-2 text-sm"
              value={draft.visualAnchor}
              placeholder={i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_5e8df109")}
              onChange={(event) => setDraft((current) => ({ ...current, visualAnchor: event.target.value }))}
            />
          </label>
          <label className="block space-y-1.5 text-sm">
            <span className="flex items-center gap-1 font-medium">
              <Mic2 className="h-4 w-4" />{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_9758d2f1", "表演和声音锚点")}</span>
            <textarea
              className="min-h-24 w-full rounded-md border bg-background px-3 py-2 text-sm"
              value={draft.voiceAnchor}
              placeholder={i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_485f80fb")}
        ...
        <label className="block space-y-1.5 text-sm">
          <span className="font-medium">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_96657d7d")}</span>
          <textarea
            className="min-h-20 w-full rounded-md border bg-background px-3 py-2 text-sm"
            value={draft.lineRule}
            placeholder={i18next.t("gen.pages.drama.components.DramaCharactersPanel.exampleShortSentencesSuppressExplainLessAskMoreWhenHumiliatedDoNotImmediatelyDefendCounterWithOneSentence")}
            onChange={(event) => setDraft((current) => ({ ...current, lineRule: event.target.value }))}
          />
        </label>

        <label className="block space-y-1.5 text-sm">
          <span className="font-medium">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_0b08748a")}</span>
          <textarea
            className="min-h-20 w-full rounded-md border bg-background px-3 py-2 text-sm"
            value={draft.relationMap}
            placeholder={i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_116afedb")}
        ...
        <div className="flex flex-wrap gap-2">
          <Button type="button" size="sm" disabled={props.busy} onClick={() => props.onSave(props.character, draft)}>
            <Save className="h-4 w-4" />{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_6c3f7724", "保存角色资产")}</Button>
          <Button type="button" size="sm" variant="outline" disabled={props.busy} onClick={() => props.onSaveToLibrary(props.character)}>{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_cff1212c", "保存到短剧角色库")}</Button>
        </div>
...
export function DramaCharactersPanel(props: {
  ...
  return (
    <div className="space-y-4">
      <Card className="rounded-lg">
        <CardHeader>
          <CardTitle className="text-lg">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_3f48cea6")}</CardTitle>
          <CardDescription>{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_2b042507")}</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-wrap gap-2">
          ...
          >
            <option value="" disabled>{i18next.t("gen.pages.drama.components.DramaCharactersPanel.librarySelection")}</option>
          ...
          >
            <Download className="h-4 w-4" />{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_1a4bdb2a", "导入角色资产")}</Button>
        </CardContent>
      </Card>

      {characters.length === 0 ? (
        <div className="rounded-md border border-dashed p-6 text-sm text-muted-foreground">{i18next.t("gen.pages.drama.components.DramaCharactersPanel.gen_05999521")}</div>
      ) : (
