...
export function useNovelVolumePlanning({
  ...
  const ensureCharacterGuard = () => {
    ...
    return window.confirm(i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.gen_d1f32388"));
  };
  ...
  const startStrategyCritique = () => {
    if (!strategyPlan) {
      setVolumeGenerationMessage(i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.gen_b348702c"));
      return;
    }
    startStrategyCritiqueAction({
      ensureCharacterGuard,
  ...
  const startChapterDetailGeneration = (
    ...
    if (!targetVolume || !targetChapter) {
      setStructuredMessage(i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.gen_2f1fbdeb"));
      return;
    }
    if (!findBeatSheet(beatSheets, volumeId)) {
      setStructuredMessage(i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.gen_cec42941"));
      return;
    }
    ...
    const confirmed = window.confirm([
      ...
      hasChapterDetailDraft(targetChapter, detailMode)
        ? i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.willPrioritizeCarryingOverCurrentlyFilledResultsOnlyFixMissingFuzzyOrNotExecutableParts")
        : i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.gen_0dad7103"),
      i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.unchangedChapter"),
    ].join("\n\n"));
    if (!confirmed) {
      return;
    }
  ...
  const startChapterDetailBundleGeneration = (
    ...
    if (!targetVolume) {
      setStructuredMessage(i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.gen_29193639"));
      return;
    }
    if (batch.targets.length === 0) {
      setStructuredMessage(typeof request === "string" ? i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.gen_ab82f96b") : i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.gen_72c1e215"));
      return;
    }
    if (!findBeatSheet(beatSheets, volumeId)) {
      setStructuredMessage(batch.targets.length > 1 ? i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.gen_e4dca0a1") : i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.gen_00dd7c8d"));
      return;
    }
    if (!ensureCharacterGuard()) {
      return;
  ...
  const applyCustomVolumeCount = () => {
    ...
    if (!resolved.value) {
      setVolumeGenerationMessage(resolved.message ?? i18next.i18next.t("gen.pages.novels.hooks.useNovelVolumePlanning.gen_ea9ad81e"));
      return;
    }
