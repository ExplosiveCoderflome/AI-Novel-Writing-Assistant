...
export function useNovelEditChapterRuntime({
  ...
  const generateChapterPlanMutation = useMutation({
    ...
    onSuccess: async () => {
      setChapterOperationMessage(i18next.t("dict.gen_bd375894"));
      await Promise.all([
  ...
  const fullAuditMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      setChapterOperationMessage(i18next.t("dict.gen_718913a2"));
  ...
  const handleGenerateSelectedChapter = () => {
    ...
    setChapterOperationMessage(i18next.t("dict.gen_bdfaaac4"));
    setActiveChapterStream({
      chapterId: selectedChapter.id,
      chapterLabel: `第${selectedChapter.order}章 ${selectedChapter.title || i18next.t("dict.gen_db55d102")}`,
    });
  ...
  const handleAbortChapterStream = () => {
    chapterSSE.abort();
    setChapterOperationMessage(i18next.t("dict.gen_ad7a384f"));
  };

  const handleAbortRepair = () => {
    repairSSE.abort();
    setActiveRepairStream(null);
    setChapterOperationMessage(i18next.t("dict.gen_4b810879"));
  };

  const startChapterRepair = (issues: ReviewIssue[]) => {
    if (!selectedChapterId) {
      setChapterOperationMessage(i18next.t("dict.gen_c0bd44c0"));
      return;
    }
    setChapterOperationMessage(i18next.t("dict.gen_910fe070"));
    setRepairBeforeContent(selectedChapter?.content ?? "");
    setRepairAfterContent("");
    setActiveRepairStream({
      chapterId: selectedChapterId,
      chapterLabel: selectedChapter ? `第${selectedChapter.order}章 ${selectedChapter.title || i18next.t("dict.gen_db55d102")}` : i18next.t("dict.gen_25afd065"),
    });
