...
function getChangedFields(existing: OutlineSyncChapter, chapter: StructuredChapter): string[] {
  ...
  if (!compareNullableString(existing.title, chapter.title)) {
    changed.push(i18next.i18next.t("gen.pages.novels.structuredOutlineSync.utils.gen_32c65d8d"));
  }
  if (!compareNullableString(existing.expectation, chapter.summary)) {
    changed.push(i18next.i18next.t("gen.pages.novels.structuredOutlineSync.utils.gen_3ae14696"));
  }
  if (!compareNullableNumber(existing.targetWordCount, chapter.targetWordCount)) {
    changed.push(i18next.i18next.t("gen.pages.novels.structuredOutlineSync.utils.gen_b71c2e84"));
  }
  if (!compareNullableNumber(existing.conflictLevel, chapter.conflictLevel)) {
    changed.push(i18next.i18next.t("gen.pages.novels.structuredOutlineSync.utils.gen_971cbaa4"));
  }
  if (!compareNullableNumber(existing.revealLevel, chapter.revealLevel)) {
    changed.push(i18next.i18next.t("gen.pages.novels.structuredOutlineSync.utils.gen_53fe8284"));
  }
  if (!compareNullableString(existing.mustAvoid, chapter.mustAvoid)) {
    changed.push(i18next.i18next.t("gen.pages.novels.structuredOutlineSync.utils.gen_e586f3e2"));
  }
  if (chapter.taskSheet?.trim() && !compareNullableString(existing.taskSheet, chapter.taskSheet)) {
    changed.push(i18next.i18next.t("gen.pages.novels.structuredOutlineSync.utils.singleTask"));
  }
  return changed;
}
  ...
  for (const chapter of targetChapters) {
    ...
    if (!existing) {
      ...
      items.push({
        ...
        hasContent: false,
        changedFields: [i18next.i18next.t("gen.pages.novels.structuredOutlineSync.utils.gen_0aab985d")],
      });
      continue;
    }
  ...
  for (const chapter of removed) {
    ...
    if (options.applyDeletes) {
      ...
      items.push({
        ...
        nextTitle: chapter.title,
        hasContent,
        changedFields: [i18next.i18next.t("gen.pages.novels.structuredOutlineSync.utils.removeFromOutline")],
      });
    } else {
      ...
      items.push({
        ...
        nextTitle: chapter.title,
        hasContent,
        changedFields: [i18next.i18next.t("gen.pages.novels.structuredOutlineSync.utils.gen_f85caee0")],
      });
    }
  }
