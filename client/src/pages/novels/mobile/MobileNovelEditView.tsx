...
export default function MobileNovelEditView(props: NovelEditViewProps) {
  ...
  const novelTitle = basicTab.basicForm.title.trim() || i18next.t("dict.gen_e3f46686");
        ...
        <div className="flex min-w-0 items-start justify-between gap-3">
          ...
          <Dialog open={isToolsOpen} onOpenChange={setIsToolsOpen}>
            <DialogTrigger asChild>
              <Button type="button" size="icon" variant="outline" className="shrink-0" aria-label={i18next.t("dict.gen_ecb72f4d")}>
                <MoreHorizontal className="h-4 w-4" />
              </Button>
            </DialogTrigger>
            <DialogContent className="max-h-[88vh] w-[calc(100vw-1.5rem)] overflow-y-auto rounded-2xl">
              <DialogHeader>
                <DialogTitle>{i18next.t("dict.gen_e6a2706f")}</DialogTitle>
                <DialogDescription>{i18next.t("dict.gen_6ac02d2b")}</DialogDescription>
              </DialogHeader>
              <div className="space-y-3 text-sm">
                <div className="grid grid-cols-3 gap-2">
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-3">
                    <div className="text-xs text-muted-foreground">{i18next.t("dict.gen_9290b644")}</div>
                    <div className="mt-1 font-semibold">{generatedChapters}/{Math.max(totalChapters, 1)}</div>
                  </div>
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-3">
                    <div className="text-xs text-muted-foreground">{i18next.t("dict.gen_a7a05e79")}</div>
                    <div className="mt-1 font-semibold">{pendingRepairs}</div>
                  </div>
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-3">
                    <div className="text-xs text-muted-foreground">{i18next.t("dict.task")}</div>
                    <div className="mt-1 truncate font-semibold">{i18next.t("dict.gen_taskAttent_5yi7")}</div>
                  </div>
                </div>
                ...
                {taskDrawer ? (
                  ...
                  >
                    <span>{i18next.t("dict.gen_4de94e23")}</span>
                    {taskAttentionLabel ? <Badge variant="secondary">{taskAttentionLabel}</Badge> : null}
                  </Button>
                ) : null}

                <div className="rounded-xl border border-border/70 p-3">
                  <div className="text-sm font-medium">{i18next.t("dict.gen_169a61d7")}</div>
                ...
                <div className="rounded-xl border border-border/70 p-3">
                  <div className="text-sm font-medium">{i18next.t("dict.gen_9b96a714")}</div>
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    <Button
                      type="button"
                      variant="outline"
