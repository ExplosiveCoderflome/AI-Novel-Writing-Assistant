...
export function useAutoDirectorCreateController(input: UseAutoDirectorCreateControllerInput) {
  ...
  const ideaInspirationMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : i18next.i18next.t("gen.pages.novels.autoDirector.useAutoDirectorCreateController.gen_116e1049"));
    },
  });
  ...
  const ensureWorkflowTask = async () => {
    ...
    if (!nextIdea) {
      throw new Error(i18next.i18next.t("gen.pages.novels.autoDirector.useAutoDirectorCreateController.gen_eaaf6955"));
    }
    if (workflowTaskId) {
  ...
  const confirmMutation = useMutation({
    ...
    onSuccess: async ({ command, workflowTaskId: nextWorkflowTaskId }) => {
      if (!command) {
        setDialogMode("execution_failed");
        setExecutionError(i18next.i18next.t("gen.pages.novels.autoDirector.useAutoDirectorCreateController.gen_14b51883"));
        toast.error(i18next.i18next.t("gen.pages.novels.autoDirector.useAutoDirectorCreateController.gen_14b51883"));
        return;
      }
      ...
      toast.success(i18next.i18next.t("gen.pages.novels.autoDirector.useAutoDirectorCreateController.gen_3ad0382f"));
    },
    onError: async (error, payload) => {
      setDialogMode("execution_failed");
      setExecutionError(error instanceof Error ? error.message : i18next.i18next.t("gen.pages.novels.autoDirector.useAutoDirectorCreateController.gen_3e6ed560"));
      setExecutionRequested(false);
  ...
  const continueMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!taskId) {
        throw new Error(i18next.i18next.t("gen.pages.novels.autoDirector.useAutoDirectorCreateController.gen_8b4231ef"));
      }
    ...
    onSuccess: async (response) => {
      ...
      setDialogMode("execution_progress");
      setExecutionError("");
      toast.success(i18next.i18next.t("gen.pages.novels.autoDirector.useAutoDirectorCreateController.gen_64cd9396"));
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : i18next.i18next.t("gen.pages.novels.autoDirector.useAutoDirectorCreateController.gen_bb8020bb"));
    },
  });
  ...
  const handleConfirmCandidate = async (candidate: DirectorCandidate) => {
    ...
    try {
      ...
      confirmSubmitLockedRef.current = false;
      const message = error instanceof Error ? error.message : i18next.i18next.t("gen.pages.novels.autoDirector.useAutoDirectorCreateController.gen_48bf4f93");
      setDialogMode("candidate_selection");
      setExecutionRequested(false);
  ...
  const handleBackgroundContinue = () => {
    toast.success(i18next.i18next.t("gen.pages.novels.autoDirector.useAutoDirectorCreateController.gen_b83767c2"));
    navigate("/");
  };
