    ...
    <div className="space-y-4">
      <section className="rounded-xl border border-border/70 bg-muted/10 p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="text-sm font-medium">{i18next.t("gen.pages.novels.components.characterWorkspace.CharacterVisibleProfileTab.gen_8c542af9", "外显资料生成")}</div>
            <div className="mt-1 text-xs leading-5 text-muted-foreground">{i18next.t("gen.pages.novels.components.characterWorkspace.CharacterVisibleProfileTab.gen_21459dc4", "补齐外貌、体态、声音和登场记忆点，让角色在正文中更容易被读者识别。")}</div>
          </div>
        ...
        <textarea
          ...
          placeholder={i18next.t("gen.pages.novels.components.characterWorkspace.CharacterVisibleProfileTab.gen_42466b39", "补全倾向（可选）：例如更有压迫感、带一点病弱感、声音更温和、不要写成传统美人")}
        ...
        <section className="rounded-lg border border-primary/30 bg-primary/5 p-3">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <div className="mt-1 text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.characterWorkspace.CharacterVisibleProfileTab.gen_dfee7e26", "请先看下面差异，确认后点击保存到角色卡。")}</div>
            </div>
            <Button
              size="sm"
      ...
      {!isGeneratingVisibleProfile && !hasVisibleProfileSuggestionForSelected ? (
        <div className="rounded-lg border border-dashed p-3 text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.characterWorkspace.CharacterVisibleProfileTab.gen_e301c9b1", "点击“AI 补全外显资料”后，会先在这里显示即将保存的差异；确认后再保存到角色卡。")}</div>
      ) : null}
