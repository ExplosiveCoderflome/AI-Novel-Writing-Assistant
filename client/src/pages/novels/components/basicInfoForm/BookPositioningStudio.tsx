  ...
  return (
    <div className="grid grid-cols-[1.5rem,1fr,auto] items-center gap-3 border-t border-border/55 py-3 first:border-t-0 first:pt-0 last:pb-0">
      ...
      {props.ready ? (
        ...
        <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_23fb2e57", "待补")}</span>
      )}
    </div>
  );
}
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="outline" className="border-primary/25 bg-primary/5 text-primary">{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_0a6db431", "书级资产")}</Badge>
                <span className="text-xs font-medium text-muted-foreground">{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_4344e6c7", "后续规划、角色和章节生成会优先读取这里")}</span>
              </div>
              <h2 className="mt-3 text-lg font-semibold leading-7 text-foreground">{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_374300d3", "定位这本书的读者承诺")}</h2>
              <p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground">{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_bdc21fe9", "先把标题、主冲突、读者期待和前 30 章必须兑现的内容收束清楚，再让 AI 继续展开世界、角色和章节。")}</p>
            </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[minmax(0,0.85fr),minmax(20rem,1.15fr)]">
            <div className="space-y-3 rounded-lg border border-border/60 bg-muted/10 p-3">
              <div className="space-y-2">
                <FieldLabel htmlFor="basic-title">{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_88af8bf5", "小说标题")}</FieldLabel>
                <Input
                  id="basic-title"
                  value={basicForm.title}
                  placeholder={i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_5f0c464b", "例如：雾港审判局")}
              ...
              <div className="space-y-2">
                <FieldLabel htmlFor="basic-description">{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_d4ed84ef", "一句话概述")}</FieldLabel>
                <textarea
                  ...
                  placeholder={i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_db2128bf", "用 2-4 句话说明主角、核心冲突和故事看点。")}
          ...
          <div className="space-y-5">
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-sm font-semibold text-foreground">{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_4707500c", "定位完成度")}</div>
            ...
            <div className="rounded-lg border border-border/60 bg-background/85 p-3">
              <ReadinessRow
                ...
                label={i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_65d2a655", "目标读者")}
                value={createPreview(basicForm.targetAudience, "写清楚谁会追这本书")}
                ready={hasText(basicForm.targetAudience)}
              />
              <ReadinessRow
                icon={<Target className="h-3.5 w-3.5" />}
                label={i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_144c6d68", "核心卖点")}
                value={createPreview(basicForm.bookSellingPoint, "明确最抓人的爽点或悬念")}
                ready={hasText(basicForm.bookSellingPoint)}
              />
              <ReadinessRow
                icon={<Compass className="h-3.5 w-3.5" />}
                label={i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_4677bbcf", "前 30 章牵引")}
            ...
            <div className="space-y-3 rounded-lg border border-border/60 bg-background/85 p-3">
              <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                <Sparkles className="h-4 w-4 text-primary" />{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_085ceba9", "商业标签")}</div>
              {commercialTags.length > 0 ? (
                ...
                <p className="text-sm leading-6 text-muted-foreground">{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_66c34c33", "填入 3-6 个标签，帮助后续标题、简介、章节钩子保持统一。")}</p>
              )}
            </div>
            ...
            <div className="grid gap-2 text-xs text-muted-foreground">
              <div className="flex items-start gap-2 rounded-md bg-background/70 p-2">
                ...
                <span>{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_2160aed3", "故事规划会读取读者承诺与商业标签。")}</span>
              </div>
              <div className="flex items-start gap-2 rounded-md bg-background/70 p-2">
                <Map className="mt-0.5 h-3.5 w-3.5 text-sky-600" />
                <span>{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_5290a439", "世界资产可在下方继续具现为规则、地点和势力。")}</span>
              </div>
              <div className="flex items-start gap-2 rounded-md bg-background/70 p-2">
                <FileText className="mt-0.5 h-3.5 w-3.5 text-amber-600" />
                <span>{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_54683b4f", "章节执行会用前 30 章牵引约束早期正文。")}</span>
              </div>
            </div>
            ...
            {coverSection ? (
              <div className="rounded-lg border border-border/60 bg-background/85 px-3 pb-3 [&>section]:border-t-0 [&>section]:pt-3">
                <div className="flex items-center gap-2 pt-3 text-sm font-semibold text-foreground">
                  <Image className="h-4 w-4 text-primary" />{i18next.t("gen.pages.novels.components.basicInfoForm.BookPositioningStudio.gen_3e010cd4", "封面视觉")}</div>
                {coverSection}
              </div>
            ) : null}
          </div>
