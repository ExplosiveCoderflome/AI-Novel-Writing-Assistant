...
function renderChapterDetailStatusBadge(chapter: StructuredChapter) {
  ...
  if (status === "complete") {
    return <Badge variant="secondary">{t("gen.pages.novels.components.StructuredChapterListCard.gen_757d59d3", "已细化")}</Badge>;
  }
  if (status === "partial") {
    return <Badge>{t("gen.pages.novels.components.StructuredChapterListCard.gen_9e61af80", "细化中")}</Badge>;
  }
  return <Badge variant="outline">{t("gen.pages.novels.components.StructuredChapterListCard.gen_8b4a5ea9", "待细化")}</Badge>;
}
...
export default function StructuredChapterListCard(props: StructuredChapterListCardProps) {
  ...
  function renderBeatStatusBadge(group: typeof beatGroups[number]) {
    ...
    if (isGeneratingGroup) {
      return <Badge>{t("gen.pages.novels.components.StructuredChapterListCard.gen_1ae3a984", "生成中")}</Badge>;
    }
    if (group.hasDraftContent) {
      return <Badge variant="secondary">{t("gen.pages.novels.components.StructuredChapterListCard.gen_62f15392", "已有正文锁定")}</Badge>;
    }
    if (group.chapters.length === 0) {
      return <Badge variant="outline">{t("gen.pages.novels.components.StructuredChapterListCard.gen_418dde27", "待生成")}</Badge>;
    }
    if (group.expectedCount > 0 && group.chapters.length !== group.expectedCount) {
      return <Badge variant="outline">{t("gen.pages.novels.components.StructuredChapterListCard.gen_884f7a5d", "需重试")}</Badge>;
    }
    return <Badge variant="secondary">{t("gen.pages.novels.components.StructuredChapterListCard.gen_c2ad1f29", "已生成")}</Badge>;
  }
      ...
      <CardHeader className="pb-3">
        <div className="space-y-3">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <CardTitle className="text-base leading-none">{t("gen.pages.novels.components.StructuredChapterListCard.gen_6409cb19", "节奏 / 章节导航")}</CardTitle>
          ...
          <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
            ...
            >{t("gen.pages.novels.components.StructuredChapterListCard.gen_ee545f79", "全部节奏")}</button>
            ...
            <div className="structured-chapter-navigation-list space-y-3 xl:max-h-[calc(100vh-12rem)] xl:overflow-y-auto xl:pr-1">
                    ...
                    {expanded ? (
                      <div className="mt-3 space-y-2 border-l border-border/70 pl-3">
                        {group.chapters.length > 0 ? group.chapters.map((chapter) => {
                          ...
                          <div className="rounded-lg border border-dashed p-3 text-xs text-muted-foreground">{t("gen.pages.novels.components.StructuredChapterListCard.gen_34a7fe06", "该节奏段下暂时还没有映射到章节。")}</div>
                        )}
                      </div>
                    ) : null}
                  </div>
              ...
              {unmatchedChapters.length > 0 ? (
                <div className="rounded-xl border border-dashed p-3">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge variant="outline">{t("gen.pages.novels.components.StructuredChapterListCard.gen_6035b315", "未归入节奏段")}</Badge>
                      <Badge variant="secondary">{unmatchedChapters.length}章</Badge>
                    </div>
                    <span className="text-xs text-muted-foreground">{t("gen.pages.novels.components.StructuredChapterListCard.gen_b6c40f43", "这些章节暂时没有落到任何节奏段")}</span>
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <Button
                            ...
                            title={t("gen.pages.novels.components.StructuredChapterListCard.gen_cfd7b8b9", "删除这个未归入节奏段的章节")}
                            onClick={() => {
                          ...
                          >
                            ...
                            <span className="sr-only">{t("gen.pages.novels.components.StructuredChapterListCard.gen_cfd7b8b9", "删除这个未归入节奏段的章节")}</span>
                          </Button>
                        </div>
                      );
                    })}
