...
function renderChapterDetailStatusBadge(
  ...
  if (status === "complete") {
    return <Badge variant="secondary">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_757d59d3", "已细化")}</Badge>;
  }
  if (status === "partial") {
    return <Badge>{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_9e61af80", "细化中")}</Badge>;
  }
  return <Badge variant="outline">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_8b4a5ea9", "待细化")}</Badge>;
}
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <CardTitle className="text-base leading-none">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_40d404ef", "当前章节细化")}</CardTitle>
              {selectedChapter ? (
                <>
      ...
      <CardContent className="space-y-4">
        {selectedVolume && selectedChapter ? (
          <>
            <div className="rounded-xl border border-border/70 bg-muted/20 p-4">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-1">
                  <div className="text-sm font-medium">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_d1cca200", "批量细化")}</div>
                  <div className="text-xs leading-6 text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_f36b3482", "可以从当前章起按数量连续细化，也可以直接补齐当前可见章节或本卷全部章节。")}</div>
                </div>
                <AiButton
                  size="sm"
                  variant="secondary"
              ...
              <div className="mt-3 grid gap-3 md:grid-cols-[minmax(0,1fr)_160px]">
                <label className="space-y-2 text-sm">
                  <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_df011658", "范围")}</span>
                  <SelectControl
                  ...
                  >
                    <option value="count" disabled={!hasCountBatch}>{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_dae1e946", "从当前章起连续细化")}</option>
                    {hasVisibleBatch ? <option value="visible_all">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_d73f876c", "当前可见章节")}</option> : null}
                    {hasVolumeBatch ? <option value="volume_all">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_49eb32e6", "本卷全部章节")}</option> : null}
                  </SelectControl>
                </label>
                <label className="space-y-2 text-sm">
            ...
            <label className="space-y-2 text-sm">
              <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_123f397f", "章节标题")}</span>
              <Input
            ...
            <label className="space-y-2 text-sm">
              <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_d8f0aad6", "章节摘要")}</span>
              <textarea
            ...
            <label className="space-y-2 text-sm">
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_85f9e2b5", "章节目标")}</span>
                <AiButton
                  size="sm"
                  variant="outline"
            ...
            <label className="space-y-2 text-sm">
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_c8ba67e1", "任务单")}</span>
                <AiButton
                  size="sm"
                  variant="outline"
            ...
            {showChapterAdvanced ? (
              <div className="space-y-4 rounded-xl border border-border/70 bg-muted/20 p-4">
                <div className="text-sm font-medium">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_e370757f", "高级设置")}</div>
                <div className="grid gap-3 md:grid-cols-3">
                  <label className="space-y-2 text-sm">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_971cbaa4", "冲突等级")}</span>
                      {selectedChapter.conflictLevelSource === "user" && typeof selectedChapter.conflictLevel === "number" ? (
                        <Button
                          ...
                          title={i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_be3fe21d", "解除锚定，交还 AI 优化")}
                          onClick={() => {
                        ...
                        >
                          <UnlockKeyhole className="mr-1 h-3.5 w-3.5" aria-hidden="true" />{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_50d71b11", "交还 AI")}</Button>
                      ) : null}
                    </div>
                    <Input
                      type="number"
                  ...
                  <label className="space-y-2 text-sm">
                    <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_53fe8284", "揭露等级")}</span>
                    <Input
                      type="number"
                      min={0}
                      max={100}
                  ...
                  <label className="space-y-2 text-sm">
                    <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_b71c2e84", "目标字数")}</span>
                    <Input
                      type="number"
                      min={200}
                      step={100}
                ...
                <label className="space-y-2 text-sm">
                  <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_e586f3e2", "禁止事项")}</span>
                  <textarea
                ...
                <label className="space-y-2 text-sm">
                  <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_3e47f4e2", "兑现关联")}</span>
                  <textarea
                    className={cn(textareaClassName, "min-h-[100px]")}
                ...
                <div className="flex flex-wrap gap-2">
                  <Button size="sm" variant="outline" onClick={() => onMoveChapter(selectedVolume.id, selectedChapter.id, -1)} disabled={selectedChapterIndex <= 0}>{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_315eacd1", "上移")}</Button>
                  <Button size="sm" variant="outline" onClick={() => onMoveChapter(selectedVolume.id, selectedChapter.id, 1)} disabled={selectedChapterIndex < 0 || selectedChapterIndex >= selectedVolume.chapters.length - 1}>{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_17acd250", "下移")}</Button>
                  <Button size="sm" variant="outline" onClick={() => onRemoveChapter(selectedVolume.id, selectedChapter.id)} disabled={selectedVolume.chapters.length <= 1}>{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_2f4aaddd", "删除")}</Button>
                </div>
              </div>
            ) : (
              <div className="rounded-xl border border-dashed p-3 text-sm text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_27b915f8", "冲突等级、揭露等级、字数、禁止事项和兑现关联已收进高级设置，避免一上来就把表单铺满。")}</div>
            )}
          </>
        ) : (
          <div className="rounded-md border border-dashed p-6 text-sm text-muted-foreground">{i18next.t("gen.pages.novels.components.StructuredChapterDetailCard.gen_c33e38eb", "先在左侧选择一个章节，再开始细化。")}</div>
        )}
      </CardContent>
    </Card>
  );
