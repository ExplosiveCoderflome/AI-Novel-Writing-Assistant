  ...
  return (
    <div className="mt-5 w-full">
      <div className="flex items-center justify-between gap-3">
        <div className="text-xs text-muted-foreground">{t("gen.pages.novels.components.NovelAutoDirectorIdeaInspirationPanel.gen_ddbd386e", "这些只是临时灵感，使用后仍可继续改。")}</div>
          ...
          {ideas.map((idea, index) => (
            ...
            >
              ...
              <span className="mt-0.5 inline-flex shrink-0 items-center gap-1 rounded-md px-2 py-1 text-xs text-primary opacity-0 transition group-hover:opacity-100 group-focus-visible:opacity-100">
                <Check className="h-4 w-4" />{t("gen.pages.novels.components.NovelAutoDirectorIdeaInspirationPanel.gen_ecff77a8", "使用")}</span>
            </motion.button>
          ))}
        </div>
      ) : null}
