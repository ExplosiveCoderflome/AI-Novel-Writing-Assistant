  ...
  return (
    <div className="space-y-5">
      <DirectorTakeoverEntryPanel
        title={i18next.t("gen.pages.novels.components.OutlineTab.gen_d88460f2", "从卷战略接管")}
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="text-lg font-semibold tracking-tight">{i18next.t("gen.pages.novels.components.OutlineTab.gen_0ed4c99f", "卷战略控制台")}</h2>
              <Badge variant={outlineStageReady ? "default" : "outline"}>
                {completedReadinessCount}/{readinessSteps.length} 已就绪
              </Badge>
              {hasUnsavedVolumeDraft ? <Badge variant="secondary">{i18next.t("gen.pages.novels.components.OutlineTab.gen_7fa6741a", "含未保存草稿")}</Badge> : null}
            </div>
            <div className="max-w-3xl text-sm leading-6 text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_13b83dfd", "先确定整本书的卷级推进方式，再审当前卷的承诺、压力、兑现和下卷牵引。")}</div>
          </div>
      ...
      <div className="space-y-5 p-5">
        ...
        {!hasCharacters ? (
          <div className="flex items-center justify-between gap-2 rounded-2xl bg-amber-50 px-4 py-3 text-xs text-amber-800">
            <span>{i18next.t("gen.pages.novels.components.OutlineTab.gen_341c6030", "建议先补齐角色，再生成卷战略和卷骨架。")}</span>
            <Button size="sm" variant="outline" onClick={onGoToCharacterTab}>{i18next.t("gen.pages.novels.components.OutlineTab.gen_ef2c69b9", "去角色管理")}</Button>
          </div>
        ) : null}
        ...
        <div className="grid items-start gap-3 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-3">
            <Card className="self-start border-0 bg-muted/15 shadow-none">
              <CardHeader className="pb-3">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <CardTitle className="text-base">{i18next.t("gen.pages.novels.components.OutlineTab.gen_2b46d6ca", "阶段就绪度")}</CardTitle>
                  <Badge variant={outlineStageReady ? "default" : "outline"}>
                    {completedReadinessCount}/{readinessSteps.length} 已就绪
                  </Badge>
                </div>
              ...
              <CardContent className="space-y-3 text-sm">
                <div className="rounded-xl bg-background/70 p-3">
                  <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_9ff48c30", "推荐下一步")}</div>
                ...
                {readiness.blockingReasons.length > 0 ? (
                  ...
                  <div className="rounded-xl bg-emerald-50 p-3 text-xs text-emerald-800">{i18next.t("gen.pages.novels.components.OutlineTab.gen_2eeddac4", "当前工作区已经具备继续推进的基础条件。")}</div>
                )}
            ...
            <details className="group border-t border-border/60 pt-4">
              <summary className="cursor-pointer list-none">
                <CollapsibleSummary
                  title={i18next.t("gen.pages.novels.components.OutlineTab.gen_35e25adc", "卷数建议与策略审查")}
                  description={i18next.t("gen.pages.novels.components.OutlineTab.gen_d9a6d0d6", "这些属于辅助决策信息。首屏先看推荐下一步和当前卷，确实需要时再展开审查与卷数控制。")}
              ...
              <div className="mt-4 space-y-3">
                <Card className="self-start border-0 bg-muted/15 shadow-none">
                  <CardHeader className="pb-3">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      <CardTitle className="text-base">{i18next.t("gen.pages.novels.components.OutlineTab.gen_197bb5c2", "卷数建议")}</CardTitle>
                  ...
                  <CardContent className="space-y-3 text-sm">
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div className="rounded-xl bg-background/70 p-3">
                        <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_e9fd5ff1", "总章节预算")}</div>
                        <div className="mt-1 text-lg font-semibold text-foreground">{volumeCountGuidance.chapterBudget} 章</div>
                      </div>
                      <div className="rounded-xl bg-background/70 p-3">
                        <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_d16a7644", "结构建议区间")}</div>
                        <div className="mt-1 text-lg font-semibold text-foreground">
                          {volumeCountGuidance.decisionVolumeCountRange.min}-{volumeCountGuidance.decisionVolumeCountRange.max} 卷
                        </div>
                      </div>
                      <div className="rounded-xl bg-background/70 p-3">
                        <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_3f61fa36", "系统建议卷数")}</div>
                        <div className="mt-1 text-lg font-semibold text-foreground">{volumeCountGuidance.systemRecommendedVolumeCount} 卷</div>
                      </div>
                      <div className="rounded-xl bg-background/70 p-3">
                        <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_b027af2e", "默认硬规划范围")}</div>
                        <div className="mt-1 text-lg font-semibold text-foreground">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Button size="sm" variant="outline" onClick={onRestoreSystemRecommendedVolumeCount}>{i18next.t("gen.pages.novels.components.OutlineTab.gen_1580425d", "恢复系统建议")}</Button>
                    </div>
                      ...
                      <div className="rounded-xl bg-background/70 p-3">
                        <div className="grid gap-3 sm:grid-cols-[minmax(0,180px)_auto_auto] sm:items-end">
                          <label className="space-y-1 text-sm">
                            <span className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_8ee917f8", "固定卷数")}</span>
                            <input
                              type="number"
                          ...
                          <Button size="sm" onClick={onApplyCustomVolumeCount}>{i18next.t("gen.pages.novels.components.OutlineTab.gen_ccaccd00", "应用固定卷数")}</Button>
                ...
                {critiqueReport ? (
                  <Card className="self-start border-0 bg-muted/15 shadow-none">
                    <CardHeader className="pb-3">
                      <div className="flex items-center justify-between gap-2">
                        <CardTitle className="text-base">{i18next.t("gen.pages.novels.components.OutlineTab.gen_cc89ccce", "卷战略审稿")}</CardTitle>
          ...
          <details className="group border-t border-border/60 pt-4">
            <summary className="cursor-pointer list-none">
              <CollapsibleSummary
                title={i18next.t("gen.pages.novels.components.OutlineTab.gen_1306a800", "派生文本、版本控制与影响分析")}
                description={i18next.t("gen.pages.novels.components.OutlineTab.gen_9a10e283", "这部分偏向收尾和对比，不是当前卷骨架编辑时必须一直盯着看的内容。")}
              />
            </summary>
            ...
            <div className="mt-4 space-y-3">
              <Card className="self-start border-0 bg-muted/15 shadow-none">
                <CardHeader>
                  <CardTitle className="text-base">{i18next.t("gen.pages.novels.components.OutlineTab.gen_6ca5b6b6", "派生文本预览")}</CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card className="self-start border-0 bg-muted/15 shadow-none">
                <CardHeader>
                  <CardTitle className="text-base">{i18next.t("gen.pages.novels.components.OutlineTab.gen_e9a2d9e3", "版本控制")}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2 text-sm">
                  {volumeVersions.length > 0 ? (
                    ...
                    <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_b738326a", "还没有卷版本，请先保存草稿版本。")}</div>
                  )}
                  <div className="flex flex-wrap gap-2">
                    <Button onClick={onCreateDraftVersion} disabled={isCreatingDraftVersion || volumes.length === 0}>
                      {isCreatingDraftVersion ? "保存中..." : "保存为草稿版本"}
                    </Button>
                    <Button variant="outline" onClick={onLoadSelectedVersionToDraft} disabled={!selectedVersionId}>{i18next.t("gen.pages.novels.components.OutlineTab.gen_f661191d", "覆盖当前草稿")}</Button>
              ...
              <Card className="self-start border-0 bg-muted/15 shadow-none">
                <CardHeader>
                  <CardTitle className="text-base">{i18next.t("gen.pages.novels.components.OutlineTab.gen_e8540d99", "影响分析")}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2 text-sm">
                  <div className="flex flex-wrap gap-2">
                  ...
                  {impactResult ? (
                    <div className="space-y-3 rounded-md border p-3 text-xs">
                      <div className="font-medium">{i18next.t("gen.pages.novels.components.OutlineTab.gen_274b40e6", "卷级影响预览")}</div>
                    ...
                    <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_a94f04e0", "建议在生效前先做卷级影响分析。")}</div>
                  )}
                </CardContent>
              </Card>
            </div>
        ...
        <Tabs value={workspaceTab} onValueChange={(value) => setWorkspaceTab(value as OutlineWorkspaceTab)} className="space-y-4">
          <TabsList className="h-auto flex-wrap justify-start bg-muted/60 p-1">
            <TabsTrigger value="current">{i18next.t("gen.pages.novels.components.OutlineTab.gen_8d4f455b", "当前卷")}</TabsTrigger>
            <TabsTrigger value="strategy">{i18next.t("gen.pages.novels.components.OutlineTab.gen_0c1f7d5f", "战略总览")}</TabsTrigger>
            <TabsTrigger value="assets">{i18next.t("gen.pages.novels.components.OutlineTab.gen_f8a6b30c", "资产约束")}</TabsTrigger>
          </TabsList>
        ...
        <Card className="border-0 bg-muted/15 shadow-none">
          <CardHeader className="pb-3">
            <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <CardTitle className="text-base">{i18next.t("gen.pages.novels.components.OutlineTab.gen_72852faa", "卷战略摘要")}</CardTitle>
                <div className="text-sm text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_434d22e8", "先看整本书的卷级回报和升级路线，再在下面选择某一卷进入详细编辑。")}</div>
              </div>
              <div className="flex flex-wrap gap-2">
                {strategyPlan ? (
                  <>
          ...
          <CardContent className="space-y-3 text-sm">
            {strategyPlan ? (
              <>
                <div className="grid gap-3 xl:grid-cols-3">
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-4">
                    <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_aa57c00f", "读者回报梯度")}</div>
                    <div className="mt-2 text-sm leading-6 text-foreground">{strategyPlan.readerRewardLadder}</div>
                  </div>
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-4">
                    <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_c8b6fdbc", "升级梯度")}</div>
                    <div className="mt-2 text-sm leading-6 text-foreground">{strategyPlan.escalationLadder}</div>
                  </div>
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-4">
                    <div className="text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_5692d9fb", "中盘转向")}</div>
                ...
                <div className="rounded-xl border border-border/70 p-4 text-sm text-muted-foreground">
                  <div className="text-xs">{i18next.t("gen.pages.novels.components.OutlineTab.gen_659ff89a", "卷级节奏总览")}</div>
              ...
              <div className="rounded-md border border-dashed p-4 text-xs text-muted-foreground">{i18next.t("gen.pages.novels.components.OutlineTab.gen_7da519a5", "当前还没有卷战略建议。先点击“生成卷战略建议”。")}</div>
            )}
          </CardContent>
        </Card>
        </TabsContent>
