  ...
  return (
    <section className="mx-auto max-w-5xl space-y-5 px-3 py-6 sm:px-4 lg:px-0">
      <OnboardingTip
        ...
        title={i18next.t("gen.pages.novels.components.NovelProductionExperienceHandoff.gen_693fccfa", "这是正文开始前唯一一次生产方式选择")}
        description={i18next.t("gen.pages.novels.components.NovelProductionExperienceHandoff.gen_8ea4b146", "新手推荐简易创作：AI 会持续写作、审校和修复，你只看稳定的完成稿。需要亲自修改时再选专业创作。")}
      />
      <div className="relative overflow-hidden rounded-3xl bg-foreground px-6 py-7 text-background shadow-[0_30px_80px_-50px_hsl(var(--foreground))] sm:px-8 sm:py-9">
        ...
        <div className="relative">
          <div className="flex items-center gap-2 text-sm font-medium text-background/70">
            <BookOpen className="h-4 w-4" />{i18next.t("gen.pages.novels.components.NovelProductionExperienceHandoff.gen_ccd1b8ed", "开写前准备完成")}</div>
          <h1 className="mt-4 max-w-3xl text-2xl font-semibold tracking-tight sm:text-3xl">
            选择《{novelTitle?.trim() || "这本小说"}》的正文生产方式
          </h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-background/70">{i18next.t("gen.pages.novels.components.NovelProductionExperienceHandoff.gen_5a8902a2", "故事方向、角色和卷章安排准备完毕。接下来可以让 AI 持续写完整本书，也可以进入完整工作台亲自控制。")}</p>
        </div>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        <article className="flex flex-col rounded-3xl border border-primary/30 bg-background p-6 shadow-[0_24px_65px_-50px_hsl(var(--primary))] ring-1 ring-primary/10 sm:p-7">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="rounded-full bg-primary px-2.5 py-1 text-xs font-medium text-primary-foreground">{i18next.t("gen.pages.novels.components.NovelProductionExperienceHandoff.gen_04c79172", "推荐新手")}</span>
          </div>
          <h2 className="mt-5 text-xl font-semibold text-foreground">{i18next.t("gen.pages.novels.components.NovelProductionExperienceHandoff.gen_1fd86181", "简易创作")}</h2>
        ...
        <article className="flex flex-col rounded-3xl border border-border/80 bg-background p-6 sm:p-7">
          ...
          <h2 className="mt-5 text-xl font-semibold text-foreground">{i18next.t("gen.pages.novels.components.NovelProductionExperienceHandoff.gen_30bcd4fb", "专业创作")}</h2>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">{i18next.t("gen.pages.novels.components.NovelProductionExperienceHandoff.gen_5f26e3af", "保留完整控制权，检查规划后自行安排正文生产。")}</p>
          <ul className="mt-5 flex-1 space-y-3 text-sm text-foreground">
