  ...
  return (
    <section id="novel-world-usage" className="rounded-2xl bg-background/80 p-4 shadow-sm ring-1 ring-border/35">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className="h-1.5 w-1.5 rounded-full bg-primary" />{t("gen.pages.novels.components.NovelWorldUsageCard.gen_553a34e5", "章节生成使用范围")}</div>
        ...
        <div className="flex shrink-0 flex-wrap gap-2">
          ...
          <Button type="button" variant="ghost" onClick={props.onOpenDetails} disabled={!hasWorld}>
            <SlidersHorizontal className="size-4" />{t("gen.pages.novels.components.NovelWorldUsageCard.gen_59fb60f0", "调整保留项")}</Button>
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-4">
        <MetricItem label={t("gen.pages.novels.components.NovelWorldUsageCard.gen_fcd9b267", "主舞台")} value={primaryLocation} detail={props.view?.worldName ?? "等待本书世界"} />
        <MetricItem
          label={t("gen.pages.novels.components.NovelWorldUsageCard.gen_26f05301", "活跃势力")}
          value={`${slice?.activeForces.length ?? 0} 个`}
          detail={namesLine(slice?.activeForces ?? [], "整理后显示")}
        />
        <MetricItem
          label={t("gen.pages.novels.components.NovelWorldUsageCard.gen_3fd96627", "故事地点")}
          value={`${slice?.activeLocations.length ?? 0} 处`}
          detail={namesLine(slice?.activeLocations ?? [], "整理后显示")}
        />
        <MetricItem
          label={t("gen.pages.novels.components.NovelWorldUsageCard.gen_6d93df36", "硬规则")}
      ...
      <div className="mt-4 flex flex-col gap-3 border-t border-border/50 pt-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0 text-sm leading-6 text-muted-foreground">
          <span className="font-medium text-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_b550c279", "边界：")}</span>
...
export function NovelWorldUsageDetails(props: NovelWorldUsageCardProps & {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="text-lg font-semibold text-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_d8c40a7a", "生成使用范围")}</div>
          <div className="mt-1 text-sm leading-6 text-muted-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_3ff040cb", "这里确认章节生成实际会读取的规则、势力和地点。你可以只指定少量必须保留项，其余交给系统按本书方向裁剪。")}</div>
        </div>
      ...
      {!hasWorld ? (
        <div className="rounded-md border border-dashed border-border/70 px-4 py-4 text-sm leading-6 text-muted-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_c2c3bcb7", "这本小说还没有本书世界。先从世界库导入，或根据本书主题生成一套世界，再整理当前故事会重点使用的规则、势力和地点。")}</div>
      ) : null}

      {hasWorld ? (
        <div className="grid gap-4 md:grid-cols-2">
          <MetricItem label={t("gen.pages.novels.components.NovelWorldUsageCard.gen_e83ed182", "本书世界")} value={props.view?.worldName ?? "未命名世界"} detail={t("gen.pages.novels.components.NovelWorldUsageCard.gen_ca4bb86a", "章节生成读取的是这本书的世界副本。")} />
          <MetricItem label={t("gen.pages.novels.components.NovelWorldUsageCard.gen_8ac8e795", "故事想法来源")} value={labelStoryInputSource(props.view?.storyInputSource)} detail={t("gen.pages.novels.components.NovelWorldUsageCard.gen_6ccfa1ce", "使用范围会结合当前故事方向裁剪。")} />
        </div>
      ) : null}
      ...
      {slice ? (
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(360px,0.95fr)]">
          <section className="space-y-5">
            <div>
              <div className="text-sm font-medium text-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_f3b342c6", "世界底色")}</div>
              <div className="mt-1 text-sm leading-6 text-muted-foreground">{slice.coreWorldFrame || "暂无"}</div>
            </div>
            <div>
              <div className="text-sm font-medium text-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_3419a5a3", "会用到的组织")}</div>
              <div className="mt-2 text-sm leading-6 text-muted-foreground">
                {namesLine(slice.activeForces, "暂无")}
              </div>
            </div>
            <div>
              <div className="text-sm font-medium text-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_7bc063ef", "会用到的地点")}</div>
              <div className="mt-2 text-sm leading-6 text-muted-foreground">
                {namesLine(slice.activeLocations, "暂无")}
              </div>
            </div>
            <div>
              <div className="text-sm font-medium text-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_0a431a82", "核心规则")}</div>
              <div className="mt-2 space-y-3">
                ...
                )) : <div className="text-sm text-muted-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_f61f4cf6", "暂无")}</div>}
              </div>
            </div>
            <div>
              <div className="text-sm font-medium text-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_bc6823e0", "压力来源")}</div>
              <div className="mt-2 space-y-1 text-sm text-muted-foreground">
                {slice.pressureSources.length > 0 ? slice.pressureSources.map((item) => (
                  <div key={item}>{item}</div>
                )) : <div>{t("gen.pages.novels.components.NovelWorldUsageCard.gen_f61f4cf6", "暂无")}</div>}
              </div>
            </div>
            <div>
              <div className="text-sm font-medium text-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_a3737c81", "不要越过的边界")}</div>
          ...
          <section className="space-y-4 rounded-xl bg-muted/15 p-4">
            <div>
              <div className="text-sm font-medium text-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_8a416bb1", "手动保留项")}</div>
              <div className="mt-1 text-sm leading-6 text-muted-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_7251aeea", "这里只指定必须出现或必须遵守的少量内容，不需要重填整套世界。")}</div>
            </div>
            <div>
              <label className="text-sm font-medium text-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_fcd9b267", "主舞台")}</label>
              <Select value={props.draft.primaryLocationId} onValueChange={props.draft.setPrimaryLocationId}>
                <SelectTrigger className="mt-2">
                  <SelectValue placeholder={t("gen.pages.novels.components.NovelWorldUsageCard.gen_820ef10b", "请选择主舞台")} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="__none__">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_d86d0336", "不额外指定")}</SelectItem>
            ...
            <DetailDisclosure title={t("gen.pages.novels.components.NovelWorldUsageCard.gen_3c8502ed", "必须保留的组织、地点和规则")} description={t("gen.pages.novels.components.NovelWorldUsageCard.gen_ef4627f0", "适合开局地点、关键盟友、主要敌人和不能突破的力量代价。")}>
              <div className="space-y-4">
                <OverrideGroup
                  icon={Castle}
                  title={t("gen.pages.novels.components.NovelWorldUsageCard.gen_d28cb76e", "必须保留的组织")}
                  description={t("gen.pages.novels.components.NovelWorldUsageCard.gen_04faf2b2", "主角出身、主要敌人、关键盟友这类前期不能漏掉的势力。")}
                  emptyText={t("gen.pages.novels.components.NovelWorldUsageCard.gen_70d12403", "本书世界里还没有可选组织。")}
                ...
                <OverrideGroup
                  icon={MapPinned}
                  title={t("gen.pages.novels.components.NovelWorldUsageCard.gen_e6c83226", "必须保留的地点")}
                  description={t("gen.pages.novels.components.NovelWorldUsageCard.gen_0494d750", "开局地点、试炼地、冲突爆发地和读者需要反复记住的舞台。")}
                  emptyText={t("gen.pages.novels.components.NovelWorldUsageCard.gen_05a6d26a", "本书世界里还没有可选地点。")}
                ...
                <OverrideGroup
                  icon={ShieldAlert}
                  title={t("gen.pages.novels.components.NovelWorldUsageCard.gen_c0ad1d48", "必须遵守的规则")}
                  description={t("gen.pages.novels.components.NovelWorldUsageCard.gen_b2331435", "力量代价、身份禁忌和不能被剧情随意突破的边界。")}
                  emptyText={t("gen.pages.novels.components.NovelWorldUsageCard.gen_3401905f", "本书世界里还没有可选规则。")}
            ...
            <div>
              <label className="text-sm font-medium text-foreground" htmlFor="story-world-scope-note">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_4793dda7", "前期不要越界的边界说明")}</label>
              <div className="mt-1 text-sm leading-6 text-muted-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_5d46e874", "可以补一句边界，例如“保留现实都市基底，不要转成玄幻升级文”。")}</div>
              <textarea
                ...
                placeholder={t("gen.pages.novels.components.NovelWorldUsageCard.gen_74071472", "例如：保留原作的现实商业环境和人物压迫感，不要引入超自然体系。")}
              />
            </div>
          </section>
        </div>
      ...
      {hasWorld && !hasSlice ? (
        <div className="rounded-md border border-dashed border-border/70 px-4 py-4 text-sm leading-6 text-muted-foreground">{t("gen.pages.novels.components.NovelWorldUsageCard.gen_c72e790f", "这本书还没有整理出当前故事会用到的世界范围。点击“整理本书使用范围”后，会根据本书世界和故事想法生成一版可确认的规则、势力和地点范围。")}</div>
      ) : null}
    </div>
  );
}
...
export default function NovelWorldUsageCard(props: NovelWorldUsageCardProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <DetailDisclosure title={t("gen.pages.novels.components.NovelWorldUsageCard.gen_483341fb", "使用范围详情")} description={t("gen.pages.novels.components.NovelWorldUsageCard.gen_a810386f", "查看和调整本书前期必须保留的世界约束。")}>
