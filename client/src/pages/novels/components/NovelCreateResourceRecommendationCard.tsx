    ...
    <div className="space-y-4 pt-1">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-1">
          <div className="text-sm font-semibold text-foreground">{i18next.t("gen.pages.novels.components.NovelCreateResourceRecommendationCard.gen_a265e2ec", "先让 AI 推荐一套开书底座")}</div>
          <div className="text-sm leading-6 text-muted-foreground">{i18next.t("gen.pages.novels.components.NovelCreateResourceRecommendationCard.gen_ea5cd7b9", "你不用先理解题材基底库和推进模式库。先写一句灵感、卖点或前 30 章承诺，系统会帮你推荐一套更适合新手起步的默认组合。")}</div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {hasAppliedRecommendation ? (
            <Badge variant="outline">{i18next.t("gen.pages.novels.components.NovelCreateResourceRecommendationCard.gen_c439c0d7", "已填入当前表单")}</Badge>
          ) : null}
        </div>
      </div>

      {!canRecommend ? (
        <div className="text-sm text-muted-foreground">{i18next.t("gen.pages.novels.components.NovelCreateResourceRecommendationCard.gen_4f641461", "先补一句灵感、概述、目标读者、卖点或前 30 章承诺中的任意一项，AI 才能更稳地判断你该从哪种题材和推进模式起步。")}</div>
      ) : null}
      ...
      {recommendation ? (
        <div className="space-y-3">
          ...
          {recommendationIsStale ? (
            <div className="rounded-md bg-amber-500/10 px-3 py-2 text-sm text-amber-800">{i18next.t("gen.pages.novels.components.NovelCreateResourceRecommendationCard.gen_91a9766f", "你刚刚改过开书信息，建议重新推荐一次，让题材和推进模式跟上最新方向。")}</div>
          ) : null}

          <div className="grid gap-3 lg:grid-cols-3">
            <div className="rounded-lg bg-muted/15 p-3">
              <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{i18next.t("gen.pages.novels.components.NovelCreateResourceRecommendationCard.gen_5c3b741f", "推荐题材基底")}</div>
            ...
            <div className="rounded-lg bg-muted/15 p-3">
              <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{i18next.t("gen.pages.novels.components.NovelCreateResourceRecommendationCard.gen_49a4d661", "推荐主推进模式")}</div>
            ...
            <div className="rounded-lg bg-muted/15 p-3">
              <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{i18next.t("gen.pages.novels.components.NovelCreateResourceRecommendationCard.gen_43f1e65b", "推荐副推进模式")}</div>
              {recommendation.secondaryStoryMode ? (
                ...
                <div className="mt-2 text-xs leading-5 text-muted-foreground">{i18next.t("gen.pages.novels.components.NovelCreateResourceRecommendationCard.gen_1e44453c", "当前更适合先把主推进模式跑稳，不建议一开始叠太多风味。")}</div>
              )}
            </div>
          </div>
