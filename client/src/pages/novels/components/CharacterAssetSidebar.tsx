  ...
  return (
    <div className={getCharacterCardClass(isSelected, isProtagonist)}>
          ...
          <div className="flex min-w-0 items-center gap-2">
            ...
            {isProtagonist ? (
              <Badge variant="secondary" className="gap-1 px-1.5">
                <Crown className="h-3 w-3" />{i18next.t("gen.pages.novels.components.CharacterAssetSidebar.gen_83b681f4", "主角")}</Badge>
            ) : null}
          </div>
      ...
      <Button
        ...
        title={i18next.t("gen.pages.novels.components.CharacterAssetSidebar.gen_ca40c5a5", "删除角色")}
      >
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
            <Crown className="h-3.5 w-3.5 text-emerald-600" />{i18next.t("gen.pages.novels.components.CharacterAssetSidebar.gen_19618429", "主角位")}</div>
          {protagonist ? <Badge variant="outline">{i18next.t("gen.pages.novels.components.CharacterAssetSidebar.gen_83b681f4", "主角")}</Badge> : null}
        </div>
        {protagonist ? (
          <CharacterCard
          ...
          <div className="rounded-xl border border-dashed border-primary/30 bg-primary/5 p-3 text-xs leading-5 text-muted-foreground">{i18next.t("gen.pages.novels.components.CharacterAssetSidebar.gen_4f1b7b08", "当前阵容还没有标记主角，可在角色定位中补充主角信息。")}</div>
        )}
      </section>

      <section className="space-y-2">
        <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
          <UsersRound className="h-3.5 w-3.5 text-sky-600" />{i18next.t("gen.pages.novels.components.CharacterAssetSidebar.gen_1f968469", "配角与关系角色")}</div>
        {characters.length === 0 ? (
          <div className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground">{i18next.t("gen.pages.novels.components.CharacterAssetSidebar.gen_3010513a", "当前小说还没有角色，先在上方向导里创建或导入角色。")}</div>
          ...
          <div className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground">{i18next.t("gen.pages.novels.components.CharacterAssetSidebar.gen_e7bbce33", "当前阵容只有主角，后续可补充对手、同盟或关系压力角色。")}</div>
        )}
      </section>
    </div>
  );
