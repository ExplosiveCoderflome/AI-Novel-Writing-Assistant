...
export function useWritingFormulaCreateFlow({
  ...
  useEffect(() => {
    ...
    if (!task) {
      ...
      onFlowMessage(i18next.i18next.t("gen.pages.writingFormula.useWritingFormulaCreateFlow.gen_f4b80b71"));
      return;
    }
    ...
    if (task.status === "succeeded") {
      ...
      const profileName = readCreatedProfileName(task) || form.extractName.trim() || i18next.i18next.t("gen.pages.writingFormula.useWritingFormulaCreateFlow.gen_6462c1e2");
      if (!profileId) {
        onFlowMessage(i18next.i18next.t("gen.pages.writingFormula.useWritingFormulaCreateFlow.gen_8487c9c4"));
        return;
      }
      resetCreateFlow();
    ...
    const failureMessage = task.failureSummary
      ...
      ?? (task.status === "cancelled"
        ? i18next.i18next.t("gen.pages.writingFormula.useWritingFormulaCreateFlow.gen_e59397af")
        : i18next.i18next.t("gen.pages.writingFormula.useWritingFormulaCreateFlow.gen_e47866f2"));
  ...
  const createExtractionTaskMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      if (!task) {
        onFlowMessage(i18next.i18next.t("gen.pages.writingFormula.useWritingFormulaCreateFlow.gen_d85cc95d"));
        return;
      }
      handledTerminalTaskIdRef.current = "";
