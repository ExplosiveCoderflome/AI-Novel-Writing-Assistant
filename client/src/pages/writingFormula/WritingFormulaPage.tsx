...
export default function WritingFormulaPage() {
  ...
  const reextractFeaturesMutation = useMutation({
    mutationFn: async () => {
      if (!selectedProfileId || !editor.sourceContent.trim()) {
        throw new Error(i18next.t("gen.pages.writingFormula.WritingFormulaPage.gen_6fd53c05"));
      }

      return extractStyleFeaturesFromText({
        name: editor.name.trim() || selectedProfile?.name || i18next.t("gen.pages.writingFormula.WritingFormulaPage.gen_86a15791"),
    ...
    onSuccess: (response) => {
      ...
      setMessage(
        extractedFeatures.length > 0
          ...
          : i18next.t("gen.pages.writingFormula.WritingFormulaPage.gen_51327df9"),
      );
    },
  });
  ...
  const saveProfileMutation = useMutation({
    ...
    onSuccess: async () => {
      setMessage(i18next.t("gen.pages.writingFormula.WritingFormulaPage.gen_bfef6323"));
  ...
  const deleteProfileMutation = useMutation({
    ...
    onSuccess: async (_response, deletedProfileId) => {
      setMessage(i18next.t("gen.pages.writingFormula.WritingFormulaPage.gen_bd342f05"));
  ...
  const createBindingMutation = useMutation({
    ...
    onSuccess: async () => {
      setMessage(i18next.t("gen.pages.writingFormula.WritingFormulaPage.gen_93dea550"));
      await refreshStyleData();
    },
  });
  ...
  const testWriteMutation = useMutation({
    mutationFn: () => {
      if (!selectedProfileId) {
        throw new Error(i18next.t("gen.pages.writingFormula.WritingFormulaPage.gen_f902505d"));
      }
  ...
  const rewriteMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setMessage(i18next.t("gen.pages.writingFormula.WritingFormulaPage.gen_615fc34b"));
    },
  });
    ...
    <div className="flex h-full min-h-0 flex-col gap-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <div className="text-2xl font-semibold tracking-tight text-slate-950">{i18next.t("gen.pages.writingFormula.WritingFormulaPage.gen_da3a12f6")}</div>
        </div>
        <OpenInCreativeHubButton bindings={{ styleProfileId: selectedProfileId || null }} label={i18next.t("gen.pages.writingFormula.WritingFormulaPage.gen_aa4e584c")} />
      </div>
      ...
      <WritingFormulaLanding
        ...
        onDeleteProfile={(profileId) => {
          ...
          const profileName = profile?.name ?? i18next.t("gen.pages.writingFormula.WritingFormulaPage.gen_20b79693");
