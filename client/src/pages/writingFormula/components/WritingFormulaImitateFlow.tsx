  ...
  return (
    <Card className="border-slate-200/80 bg-white/90 shadow-[0_20px_60px_rgba(15,23,42,0.06)]">
      <CardHeader>
        ...
        <div className="text-sm leading-7 text-muted-foreground">{i18next.t("gen.pages.writingFormula.components.WritingFormulaImitateFlow.gen_dcd8ba37", "你先给我一段明确的参考文本，我把它拆成可执行写法，再决定是贴近模仿、保留骨架，还是只迁移可复用部分。")}</div>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(300px,0.9fr)]">
          <section className="space-y-3 rounded-2xl border bg-slate-50/70 p-4">
            <div className="text-sm font-medium text-slate-900">{i18next.t("gen.pages.writingFormula.components.WritingFormulaImitateFlow.inputReferenceText")}</div>
            <div className="grid gap-3 md:grid-cols-2">
              <input
                className="rounded-md border bg-white p-2 text-sm"
                placeholder={i18next.t("gen.pages.writingFormula.components.WritingFormulaImitateFlow.gen_c35402c0")}
                value={form.name}
                onChange={(event) => onFormChange({ name: event.target.value })}
              />
              <input
                className="rounded-md border bg-white p-2 text-sm"
                placeholder={i18next.t("gen.pages.writingFormula.components.WritingFormulaImitateFlow.gen_1befc273")}
                value={form.category}
            ...
            <textarea
              ...
              placeholder={i18next.t("gen.pages.writingFormula.components.WritingFormulaImitateFlow.gen_87a205f3")}
            ...
            <div className="flex justify-end">
              <Button type="button" onClick={onExtract} disabled={!form.name.trim() || !form.sourceText.trim() || extractPending}>
                {extractPending ? i18next.t("gen.pages.writingFormula.components.WritingFormulaImitateFlow.gen_3e78a2a7") : i18next.t("gen.pages.writingFormula.components.WritingFormulaImitateFlow.extractFeaturesFirst")}
              </Button>
            </div>
          </section>

          <section className="space-y-3 rounded-2xl border bg-white p-4">
            <div className="text-sm font-medium text-slate-900">{i18next.t("gen.pages.writingFormula.components.WritingFormulaImitateFlow.selectRetentionDegree")}</div>
            {draft ? (
              <>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          ...
                          {active ? <Badge variant="secondary" className="bg-white/10 text-white">{i18next.t("gen.pages.writingFormula.components.WritingFormulaImitateFlow.gen_ce717abb")}</Badge> : null}
                        </div>
              ...
              <div className="rounded-xl border border-dashed p-4 text-sm leading-7 text-muted-foreground">{i18next.t("gen.pages.writingFormula.components.WritingFormulaImitateFlow.gen_cf41f32a", "先完成特征提取，这里才会出现三种保留策略和对应的读感说明。")}</div>
            )}
          </section>
        </div>
        ...
        <section className="rounded-2xl border bg-white p-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="mt-1 text-xs leading-6 text-muted-foreground">{i18next.t("gen.pages.writingFormula.components.WritingFormulaImitateFlow.gen_a394472e", "保存后会自动进入当前写法编辑，你可以继续调整规则、绑定到目标，或者直接拿去试写。")}</div>
            </div>
          ...
          {draft ? (
            ...
            <div className="mt-4 rounded-xl border border-dashed p-4 text-sm text-muted-foreground">{i18next.t("gen.pages.writingFormula.components.WritingFormulaImitateFlow.gen_61b5d946", "还没有可预览的特征。先完成特征提取，再决定保存方式。")}</div>
          )}
        </section>
      </CardContent>
    </Card>
