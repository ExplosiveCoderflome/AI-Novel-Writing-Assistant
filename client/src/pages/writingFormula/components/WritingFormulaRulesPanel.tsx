    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <ShieldCheck className="h-5 w-5" />{t("gen.pages.writingFormula.components.WritingFormulaRulesPanel.gen_d6a7d091", "反 AI 规则")}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="rounded-md border bg-muted/20 p-3 text-sm">
          启用 {enabledCount} / {antiAiRules.length} 条规则
        </div>
        <div className="text-sm leading-6 text-muted-foreground">{t("gen.pages.writingFormula.components.WritingFormulaRulesPanel.gen_05005f3c", "在规则中心查看、创建和调整反 AI 规则；写法编辑区继续负责选择哪些规则绑定到当前写法。")}</div>
