    ...
    <Card className="home-recent-novels border-0 bg-transparent shadow-none">
      <CardHeader className="px-0 pb-4 pt-0">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <CardTitle className="flex items-center gap-2 text-xl tracking-normal"><BookOpenText className="h-5 w-5 text-sky-700" aria-hidden="true" />My novel</CardTitle>
      ...
      <CardContent className="px-0 pb-0">
        {props.loading ? (
          ...
          <div className="rounded-lg border border-dashed p-5 text-sm text-muted-foreground">After creating a novel, works that can be continued will be displayed here.</div>
        ) : (
    ...
    <div role="link" tabIndex={0} className="group cursor-pointer rounded-lg border border-border/80 bg-card px-5 py-4 transition hover:border-sky-300 hover:shadow-[0_8px_24px_rgba(15,23,42,0.06)] focus:outline-none focus:ring-2 focus:ring-ring" onClick={() => props.onOpenNovel(props.novel.id)} onKeyDown={handleKeyDown}>
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {workflowTask ? <Badge variant="outline">Progress {Math.round(workflowTask.progress * 100)}%</Badge> : null}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-x-5 gap-y-2 border-l border-border pl-5 text-xs text-muted-foreground sm:grid-cols-4 lg:min-w-[29rem]">
          <Fact label="chapter" value={String(props.novel._count.chapters)} />
          <Fact label="Role" value={String(props.novel._count.characters)} />
          <Fact label="world view" value={props.novel.world?.name ?? "Not bound"} />
          <Fact label="renew" value={formatHomeDate(props.novel.updatedAt)} />
        </div>
      </div>
    </div>
  );
...
function Fact(props: { label: string; value: string }) {
  return <div className="min-w-0"><div className="text-[11px]">{props.label}</div><div className={cn("mt-0.5 truncate text-sm font-medium text-foreground", props.value === "Not bound" ? "text-amber-700" : "")}>{props.value}</div></div>;
}
