    ...
    <Card className="home-recent-novels border-0 bg-transparent shadow-none">
      <CardHeader className="px-0 pb-4 pt-0">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <CardTitle className="flex items-center gap-2 text-xl tracking-normal"><BookOpenText className="h-5 w-5 text-sky-700" aria-hidden="true" />{t("gen.pages.home.components.HomeRecentNovels.gen_2eafa38f", "我的小说")}</CardTitle>
          <Button asChild size="sm" variant="ghost"><Link to="/novels">{t("gen.pages.home.components.HomeRecentNovels.gen_0467cc92", "查看全部")}<ArrowRight className="ml-1 h-4 w-4" /></Link></Button>
        </div>
      </CardHeader>
      <CardContent className="px-0 pb-0">
        {props.loading ? (
          ...
          <div className="space-y-3"><div className="text-sm text-muted-foreground">{t("gen.pages.home.components.HomeRecentNovels.gen_24c1eda7", "当前无法加载小说项目。")}</div><Button variant="outline" onClick={props.onRetry}>{t("gen.pages.home.components.HomeRecentNovels.gen_64ca9bab", "重新加载")}</Button></div>
        ) : props.novels.length === 0 ? (
          <div className="rounded-lg border border-dashed p-5 text-sm text-muted-foreground">{t("gen.pages.home.components.HomeRecentNovels.gen_d74b1c8b", "创建小说后，这里会显示可以继续推进的作品。")}</div>
        ) : (
    ...
    <div role="link" tabIndex={0} className="group cursor-pointer rounded-lg border border-border/80 bg-card px-5 py-4 transition hover:border-sky-300 hover:shadow-[0_8px_24px_rgba(15,23,42,0.06)] focus:outline-none focus:ring-2 focus:ring-ring" onClick={() => props.onOpenNovel(props.novel.id)} onKeyDown={handleKeyDown}>
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {workflowBadge ? <Badge variant={workflowBadge.variant}>{workflowBadge.label}</Badge> : <Badge variant="outline">{t("gen.pages.home.components.HomeRecentNovels.gen_1bdfef94", "项目资料")}</Badge>}
        ...
        <div className="grid grid-cols-2 gap-x-5 gap-y-2 border-l border-border pl-5 text-xs text-muted-foreground sm:grid-cols-4 lg:min-w-[29rem]">
          <Fact label={t("gen.pages.home.components.HomeRecentNovels.gen_9290b644", "章节")} value={String(props.novel._count.chapters)} />
          <Fact label={t("gen.pages.home.components.HomeRecentNovels.gen_464f3d4e", "角色")} value={String(props.novel._count.characters)} />
          <Fact label={t("gen.pages.home.components.HomeRecentNovels.gen_cfb83c02", "世界观")} value={props.novel.world?.name ?? "未绑定"} />
          <Fact label={t("gen.pages.home.components.HomeRecentNovels.gen_32ac152b", "更新")} value={formatHomeDate(props.novel.updatedAt)} />
        </div>
      </div>
    </div>
  );
