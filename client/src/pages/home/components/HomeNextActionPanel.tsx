      ...
      <Card className="home-next-action-panel overflow-hidden border-0 bg-[#122033] text-white shadow-[0_18px_50px_rgba(15,23,42,0.18)]">
        <CardContent className="p-7">
          <div className="flex items-center gap-3 text-sm text-slate-300">
            <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" />{i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_45f63aa7", "正在整理你的创作工作台...")}</div>
    ...
    return (
      <Card className="home-next-action-panel border-destructive/35 shadow-sm">
        <CardContent className="space-y-4 p-6">
          <Badge variant="destructive">{i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_1a7362f2", "暂时无法读取项目")}</Badge>
          <div>
            <h1 className="text-2xl font-semibold tracking-normal">{i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_d551eef4", "还不能为你判断下一步")}</h1>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">{i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_f2913c89", "重新加载项目后，系统会继续为你整理最合适的创作入口。")}</p>
          </div>
          <Button onClick={props.onRetry}>{i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_89c9b2e8", "重新加载项目")}</Button>
        </CardContent>
      </Card>
    );
  }
          ...
          <div className="grid gap-5 border-t border-white/10 pt-5 lg:grid-cols-[minmax(0,1.15fr)_minmax(17rem,0.85fr)]">
            <div className="border-l border-sky-300/70 pl-4">
              <div className="mb-2 flex items-center gap-2 text-sm font-medium text-sky-100">
                <ArrowRight className="h-4 w-4" aria-hidden="true" />{i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_765aa48e", "为什么是现在")}</div>
            ...
            <div className="grid grid-cols-2 gap-x-5 gap-y-3 border-l border-white/10 pl-5 text-xs text-slate-400 sm:grid-cols-4 lg:grid-cols-2">
              <HeroFact label={i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_9290b644", "章节")} value={String(novel._count.chapters)} />
              <HeroFact label={i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_464f3d4e", "角色")} value={String(novel._count.characters)} />
              <HeroFact label={i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_cfb83c02", "世界观")} value={novel.world?.name ?? "未绑定"} />
              <HeroFact label={i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_06dc9b38", "最近更新")} value={formatHomeDate(novel.updatedAt)} />
            </div>
          </div>
        </div>
        ...
        <aside className="flex flex-col justify-between gap-7 border-l border-white/10 pl-6">
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-sm font-medium text-slate-200">
              <BookOpenText className="h-4 w-4 text-sky-200" aria-hidden="true" />{i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_70b6302d", "正在创作")}</div>
      ...
      <CardContent className="grid gap-7 p-7 lg:grid-cols-[minmax(0,1fr)_17rem] lg:items-end">
        ...
        <div className="grid gap-2">
          <Button asChild size="lg" className="bg-white text-slate-950 hover:bg-slate-100">
            <Link to={DIRECTOR_CREATE_LINK}><PlusCircle className="mr-2 h-4 w-4" aria-hidden="true" />{i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_a4cc3fca", "让 AI 带我开始")}</Link>
          </Button>
          <Button asChild size="lg" variant="outline" className="border-white/20 bg-white/5 text-white hover:bg-white/10 hover:text-white">
            <Link to={MANUAL_CREATE_LINK}>{i18next.t("gen.pages.home.components.HomeNextActionPanel.gen_e40b8718", "手动创建小说")}</Link>
          </Button>
        </div>
      </CardContent>
    </Card>
