...
async function requireCheckpointIdForBranch(
  ...
  const message = i18next.i18next.t("gen.pages.creativeHub.hooks.useCreativeHubRuntime.gen_004d7b1a");
  toast.error(message);
  ...
  const stream = useMemo<LangGraphStreamCallback<LangChainMessage>>(
    () =>
      async function* streamCallback(messages, config) {
        if (!isThreadReady) {
          throw new Error(i18next.i18next.t("gen.pages.creativeHub.hooks.useCreativeHubRuntime.gen_78424316"));
        }
