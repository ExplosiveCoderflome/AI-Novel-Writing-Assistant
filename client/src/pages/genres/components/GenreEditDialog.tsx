...
export default function GenreEditDialog({
  ...
  const updateMutation = useMutation({
    mutationFn: () => {
      if (!genre) {
        throw new Error(i18next.t("gen.pages.genres.components.GenreEditDialog.gen_ea1adb0b"));
      }
    ...
    onSuccess: async () => {
      ...
      toast.success(i18next.t("gen.pages.genres.components.GenreEditDialog.gen_14c7a9d3"));
      onOpenChange(false);
    },
  });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <AppDialogContent
        className="max-w-2xl"
        title={i18next.t("gen.pages.genres.components.GenreEditDialog.gen_41b8084b")}
      ...
      >
        <div className="space-y-4">
          <label className="space-y-2 text-sm">
            <span className="font-medium text-foreground">{i18next.t("gen.pages.genres.components.GenreEditDialog.gen_d7ec2d3f")}</span>
            <Input value={name} onChange={(event) => setName(event.target.value)} />
          </label>

          <label className="space-y-2 text-sm">
            <span className="font-medium text-foreground">{i18next.t("gen.pages.genres.components.GenreEditDialog.gen_3bdd08ad")}</span>
            <textarea
              rows={4}
          ...
          <label className="space-y-2 text-sm">
            <span className="font-medium text-foreground">{i18next.t("gen.pages.genres.components.GenreEditDialog.gen_3f0265bd")}</span>
            <SelectControl
              className="w-full rounded-md border bg-background p-2 text-sm"
              value={parentId}
              onChange={(event) => setParentId(event.target.value)}
            >
              <option value="">{i18next.t("gen.pages.genres.components.GenreEditDialog.gen_a6072d41")}</option>
