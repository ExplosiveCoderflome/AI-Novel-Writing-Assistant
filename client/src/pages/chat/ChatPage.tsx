...
function toRunStatusLabel(status: string): string {
  if (status === "queued") return i18next.t("gen.pages.chat.ChatPage.gen_e5ac1d20");
  if (status === "running") return i18next.t("gen.pages.chat.ChatPage.gen_d679aea3");
  if (status === "waiting_approval") return i18next.t("gen.pages.chat.ChatPage.gen_b0bf01a4");
  if (status === "succeeded") return i18next.t("gen.pages.chat.ChatPage.gen_fad5222c");
  if (status === "failed") return i18next.t("gen.pages.chat.ChatPage.gen_acd5cb84");
  if (status === "cancelled") return i18next.t("gen.pages.chat.ChatPage.gen_2111ccbb");
  return status;
}

function toApprovalActionLabel(action: string): string {
  if (action === "approved") return i18next.t("gen.pages.chat.ChatPage.gen_ecfa64c1");
  if (action === "rejected") return i18next.t("gen.pages.chat.ChatPage.gen_81233d75");
  return action;
}

function toStepTypeLabel(stepType: string): string {
  if (stepType === "planning") return i18next.t("gen.pages.chat.ChatPage.gen_335d6d3b");
  if (stepType === "tool_call") return i18next.t("gen.pages.chat.ChatPage.gen_850b4e4d");
  if (stepType === "tool_result") return i18next.t("gen.pages.chat.ChatPage.gen_879cbfca");
  if (stepType === "approval") return i18next.t("gen.pages.chat.ChatPage.gen_0273ba5c");
  if (stepType === "completion") return i18next.t("gen.pages.chat.ChatPage.gen_bf3971dc");
  if (stepType === "analysis") return i18next.t("gen.pages.chat.ChatPage.gen_72fa7c88");
  if (stepType === "review") return i18next.t("gen.pages.chat.ChatPage.gen_4719af71");
  if (stepType === "repair") return i18next.t("gen.pages.chat.ChatPage.gen_f82661e8");
  if (stepType === "writing") return i18next.t("gen.pages.chat.ChatPage.gen_d58e850d");
  if (stepType === "context") return i18next.t("gen.pages.chat.ChatPage.context");
  return stepType;
}

function toAgentNameLabel(name: string): string {
  const normalized = name.toLowerCase();
  if (normalized === "planner") return i18next.t("gen.pages.chat.ChatPage.gen_58a5a703");
  if (normalized === "writer") return i18next.t("gen.pages.chat.ChatPage.gen_2433d1d0");
  if (normalized === "reviewer") return i18next.t("gen.pages.chat.ChatPage.gen_70db501e");
  if (normalized === "continuity") return i18next.t("gen.pages.chat.ChatPage.gen_7cce156b");
  if (normalized === "repair") return i18next.t("gen.pages.chat.ChatPage.gen_0146d371");
  return name;
}
...
function formatEvent(event: RuntimeEvent): string {
  ...
  if (event.type === "tool_result") {
    return `${event.toolName} ${event.success ? i18next.t("gen.pages.chat.ChatPage.gen_330363df") : i18next.t("gen.pages.chat.ChatPage.gen_acd5cb84")}: ${event.outputSummary}`;
  }
...
function safePreview(json: string | null | undefined): string {
  if (!json?.trim()) {
    return i18next.t("gen.pages.chat.ChatPage.gen_d81bb206");
  }
  try {
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    void chatStore.createSession(i18next.t("gen.pages.chat.ChatPage.gen_1ac07a4b"));
  ...
  const ensureSession = useCallback(async () => {
    ...
    return chatStore.createSession(i18next.t("gen.pages.chat.ChatPage.gen_1ac07a4b"));
  }, [chatStore]);
  ...
  const buildPayloadMessages = (
    ...
    return [{ role: "user" as const, content: i18next.t("gen.pages.chat.ChatPage.gen_37d781e1") }];
  };
  ...
  const submitApproval = async (action: "approve" | "reject") => {
    ...
    const pending = livePending
      ?? (persistedPendingApproval
        ...
        : null);
    if (!runId || !pending) {
      setLocalError(i18next.t("gen.pages.chat.ChatPage.gen_5290bfc4"));
      return;
    }
    setLocalError("");
    ...
    setRuntimeLatestRun({
      ...
      runId,
      status: "running",
      message: action === "approve" ? i18next.t("gen.pages.chat.ChatPage.gen_f207e03d") : i18next.t("gen.pages.chat.ChatPage.gen_92b20fd5"),
    });
  ...
  const triggerReplay = async (mode: "continue" | "dry_run") => {
    if (!currentRunId || !effectiveReplayStepId) {
      setLocalError(i18next.t("gen.pages.chat.ChatPage.gen_9bac9865"));
      return;
    }
    setLocalError("");
    try {
      ...
      if (!newRunId) {
        setLocalError(response.error ?? i18next.t("gen.pages.chat.ChatPage.gen_ff7c0d46"));
        return;
      }
      if (chatStore.currentSessionId) {
      ...
      const message = error instanceof Error
        ? error.message
        : i18next.t("gen.pages.chat.ChatPage.gen_ff7c0d46");
      setLocalError(
        message === "No replayable tool steps after source step."
          ? i18next.t("gen.pages.chat.ChatPage.gen_766c8754")
          : message,
      );
      return;
    }
  ...
  const headerRunMessage = headerRunState?.status === "waiting_approval"
    ? i18next.t("gen.pages.chat.ChatPage.gen_1c761bfe")
    : headerRunState?.status === "running"
      ? (headerRunState.message?.trim() || i18next.t("gen.pages.chat.ChatPage.gen_24f86364"))
      : headerRunState?.status === "succeeded"
        ? i18next.t("gen.pages.chat.ChatPage.gen_07514175")
        : headerRunState?.status === "failed"
          ? (headerRunState.message?.trim() || i18next.t("gen.pages.chat.ChatPage.gen_4964c85b"))
          : headerRunState?.status === "cancelled"
            ? i18next.t("gen.pages.chat.ChatPage.gen_320ec649")
            : "";
    ...
    <div className="grid min-h-[70vh] gap-4 lg:grid-cols-[240px_minmax(0,1fr)_360px]">
      <Card>
        ...
        <CardContent className="space-y-2">
          <Button className="w-full" onClick={() => void chatStore.createSession(i18next.t("gen.pages.chat.ChatPage.gen_1ac07a4b"))}>{i18next.t("gen.pages.chat.ChatPage.gen_f2b14f27", "新建对话")}</Button>
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-3 space-y-0">
          <div className="space-y-1">
            <CardTitle className="text-base">{i18next.t("gen.pages.chat.ChatPage.gen_4369b958")}</CardTitle>
            {headerRunMessage ? (
