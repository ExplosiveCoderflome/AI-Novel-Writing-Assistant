...
function extractMessageText(message: ThreadMessage): string {
  return message.content
    .map((part) => {
      ...
      if (part.type === "image") {
        return `[图片:${part.filename ?? "Unnamed"}]`;
      }
      if (part.type === "file") {
        return `[文件:${part.filename ?? "Unnamed"}]`;
      }
      return "";
    })
    .filter(Boolean)
            ...
            <div className="mx-auto mt-8 max-w-[680px] px-2 text-center">
              ...
              <div className="mt-8 grid gap-3 md:grid-cols-2">
                <ThreadPrimitive.Suggestion
                  prompt="Error 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know."
                  send={false}
                  asChild
                >
                  <button
