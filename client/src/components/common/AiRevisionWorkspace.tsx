...
export default function AiRevisionWorkspace(props: AiRevisionWorkspaceProps) {
  const {
    ...
    onCancelPreview,
    leftLabel,
    rightLabel = "AI correction instructions",
      ...
      <Panel defaultSize={28} minSize={20}>
        <div className={`space-y-3 border-l p-3 ${minHeightClassName}`}>
          ...
          <textarea
            ...
            placeholder="Enter correction requirements, such as: compress redundancy, strengthen conflicts, keep existing settings unchanged."
          />
          {selectedText ? (
            <div className="rounded-md border bg-muted/20 p-2 text-xs text-muted-foreground">
              <div className="mb-1 font-medium">The currently selected content (will be used for precise optimization)</div>
              <div className="max-h-24 overflow-auto whitespace-pre-wrap">{selectedText}</div>
            </div>
          ) : (
            <div className="text-xs text-muted-foreground">Only full-text optimization is supported when the content is not selected.</div>
          )}
          <div className="flex flex-wrap gap-2">
            ...
            >
              {isOptimizing ? "Optimizing..." : "Full text optimization preview"}
            </Button>
            <Button
              size="sm"
              variant="outline"
            ...
            >
              Optimize selected content only
                                      </Button>
          </div>
          {preview.trim() ? (
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                <Button size="sm" onClick={onApplyPreview}>App Preview</Button>
                <Button size="sm" variant="outline" onClick={onCancelPreview}>Cancel preview</Button>
              </div>
            </div>
          ) : null}
        </div>
