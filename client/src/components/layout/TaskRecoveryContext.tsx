...
export function TaskRecoveryProvider({ children }: { children: ReactNode }) {
  ...
  const resumeSingleMutation = useMutation({
    ...
    onSuccess: (_response, variables) => {
      ...
      toast.success(i18next.t("gen.components.layout.TaskRecoveryContext.gen_07826f27"));
      refreshTaskState();
      void recoveryQuery.refetch();
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : i18next.t("gen.components.layout.TaskRecoveryContext.gen_2cf820a2"));
    },
  });
  ...
  const resumeAllMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      toast.success(resumedCount > 0 ? `已开始恢复 ${resumedCount} 个任务。` : i18next.t("gen.components.layout.TaskRecoveryContext.gen_3c783b2d"));
      refreshTaskState();
      void recoveryQuery.refetch();
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : i18next.t("gen.components.layout.TaskRecoveryContext.gen_12c15229"));
    },
  });
