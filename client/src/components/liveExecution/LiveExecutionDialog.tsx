          ...
          >
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <DialogPrimitive.Title className="font-mono text-sm font-semibold tracking-wide text-emerald-100">AI creation live / LIVE LOG</DialogPrimitive.Title>
              ...
              <Badge variant="outline" className="shrink-0 border-emerald-400/50 bg-emerald-400/10 font-mono text-emerald-200">
                {activeCount > 0 ? `${activeCount} 项进行中` : connected ? "Waiting for generation" : "Connecting"}
              </Badge>
              <Button
                type="button"
                variant="ghost"
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <span className="shrink-0 text-[11px] text-emerald-100/55">{session.totalChars.toLocaleString()} character</span>
                        ...
                        {!collapsed ? (
                          <div className="border-t border-emerald-400/15 px-3 py-2">
                            ...
                            <pre className="m-0 whitespace-pre-wrap break-words text-emerald-100">{session.preview || "Waiting for the model to start returning content..."}</pre>
                          </div>
                        ) : null}
                      </section>
                    );
            ...
            <footer className="flex shrink-0 items-center justify-between gap-3 border-t border-emerald-400/25 bg-[#0d1714] px-3 py-2 text-xs text-emerald-100/65">
              <span>{followingLatest ? "Following the latest output" : "Standing at the current reading position"}</span>
