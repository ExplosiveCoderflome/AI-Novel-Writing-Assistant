  ...
  return (
    <>
      <Button
        ...
        title={t("gen.components.liveExecution.LiveExecutionDialog.gen_a98bdcf2", "查看 AI 创作实况")}
      >
          ...
          >
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <DialogPrimitive.Description id="live-execution-description" className="mt-1 text-xs leading-5 text-emerald-100/65">{t("gen.components.liveExecution.LiveExecutionDialog.gen_254e8df5", "每次调用独立显示。新调用会自动聚焦，已完成调用会收起；清空只影响当前窗口。")}</DialogPrimitive.Description>
              </div>
              ...
              >
                <Eraser className="h-3.5 w-3.5" />{t("gen.components.liveExecution.LiveExecutionDialog.gen_f443ce45", "清空前台")}</Button>
              <DialogPrimitive.Close asChild>
                <Button
                  ...
                  aria-label={t("gen.components.liveExecution.LiveExecutionDialog.gen_5ed5bed1", "关闭 AI 创作实况")}
                  onPointerDown={(event) => event.stopPropagation()}
                  onPointerMove={(event) => event.stopPropagation()}
                  onPointerUp={(event) => event.stopPropagation()}
                >
            ...
            <footer className="flex shrink-0 items-center justify-between gap-3 border-t border-emerald-400/25 bg-[#0d1714] px-3 py-2 text-xs text-emerald-100/65">
              ...
              <Button type="button" size="sm" variant="ghost" className="h-7 px-2 font-mono text-xs text-emerald-200 hover:bg-emerald-400/10 hover:text-emerald-50" onClick={scrollToLatest}>{t("gen.components.liveExecution.LiveExecutionDialog.gen_72360f91", "回到最新输出")}</Button>
            </footer>
          </DialogPrimitive.Content>
        </DialogPrimitive.Portal>
