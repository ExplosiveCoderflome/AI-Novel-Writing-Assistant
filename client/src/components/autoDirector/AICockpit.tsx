...
function displayStateLabel(state: DirectorBookAutomationDisplayState): string {
  const labels: Record<DirectorBookAutomationDisplayState, string> = {
    ...
    paused: "Suspended",
    needs_attention: "Error needs to be handled",
...
function fallbackProjectionReason(props: Pick<AICockpitProps, "fallbackSummary">): string {
  return props.fallbackSummary?.trim() || "There are no AI auto-advance tasks for you to handle.";
}
...
function renderActionLabel(
  ...
  return action.label || "Continue processing";
}

function artifactTypeLabel(type: string): string {
  const labels: Record<string, string> = {
    book_contract: "book level agreement",
    story_macro: "story planning",
    character_cast: "Role",
    volume_strategy: "Divide into volumes",
    chapter_task_sheet: "Error 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know.",
    chapter_draft: "text",
    audit_report: "Review",
    repair_ticket: "repair",
    reader_promise: "reader commitment",
    character_governance_state: "character status",
    world_skeleton: "world frame",
    source_knowledge_pack: "Information package",
    chapter_retention_contract: "retention agreement",
    continuity_state: "continuity",
    rolling_window_review: "Recent review",
  };
...
function recoveryActionLabel(
  ...
  const labels: Record<string, string> = {
    ...
    resume_after_review: "Continue after checking the reason",
    switch_model: "Continue after switching models",
    confirm_protected_content: "Confirm protected content boundaries",
    manual_repair: "Deal with chapter issues first",
  };
...
function workerStateLabel(
  ...
  const labels: Record<NonNullable<DirectorBookAutomationProjection["workerHealth"]>["derivedState"], string> = {
    idle: "Not running",
    queued_waiting_worker: "Waiting to take over",
    leased_starting: "Taking over",
    running_step: "Automatically advancing",
    waiting_gate: "Waiting for confirmation",
    auto_recovering: "Recovering",
    cancelled: "Stopped",
    failed_recoverable: "Waiting for recovery",
    failed_hard: "Need to be processed",
    succeeded: "Error 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know.",
  };
...
function workerStateDetail(health: NonNullable<DirectorBookAutomationProjection["workerHealth"]>): string {
  ...
  if (health.queuedCommandCount > 0) {
    return "The task has been queued and will continue to advance after the background execution takes over.";
  }
  if (health.runningCommandCount > 0 || health.leasedCommandCount > 0) {
    return "Background execution is processing the current task.";
  }
  if (health.staleCommandCount > 0) {
    return "After background execution is interrupted, it will try to resume from the latest progress.";
  }
  return "There are currently no background actions queued or executed.";
}
...
export default function AICockpit(props: AICockpitProps) {
      ...
      <div className="rounded-2xl bg-muted/25 p-4">
        <div className="flex items-start justify-between gap-2">
          <div className="flex min-w-0 items-start gap-2">
            ...
            <div className="min-w-0">
              <div className="text-sm font-semibold text-foreground">AI Cockpit</div>
              <div className="mt-1 text-xs leading-5 text-muted-foreground">{fallbackProjectionReason(props)}</div>
            </div>
          </div>
          <Badge variant="secondary" className="shrink-0">{fallbackStatusLabel ?? "Not turned on"}</Badge>
        </div>
        {onOpenFallbackDetails ? (
  ...
  const statusDetail = reason === statusHeadline
    ? focusProjection.progressSummary?.trim() || "AI will summarize the latest status of the book's automatic advancement here."
    : reason;
  const latestRecordText = recentItems[0] ? formatDate(recentItems[0].occurredAt) : "None yet";
    ...
    <div className="space-y-4">
      <section className={cn("rounded-2xl p-5 shadow-sm", stateSoftSurfaceClassName(focusProjection.displayState))}>
        ...
        <div className="mt-5 grid gap-3 rounded-xl bg-background/60 p-3 sm:grid-cols-3">
          ...
          />
          <SummaryMetric label="Error 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know." value={focusProjection.progressSummary || "No progress summary yet"} />
          <SummaryMetric label="Error 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know." value={latestRecordText} />
        </div>

        <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div className="min-w-0">
            <div className="text-[11px] text-muted-foreground">Error 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know.</div>
            <div className="mt-1 text-sm font-medium leading-5 text-foreground">
              {focusProjection.nextActionLabel || "Open the novel to view the current content"}
            </div>
          </div>
          <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
            <Button type="button" size="sm" onClick={handlePrimaryAction} disabled={isActionPending}>
              {isActionPending ? "Processing..." : renderActionLabel(primaryAction ?? {
                type: "open_novel",
                label: "Open novel",
      ...
      {circuitBreaker ? (
        <section className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm leading-6 text-destructive">
          <div className="font-medium">Auto-advance paused</div>
          <div className="mt-1">{circuitBreaker.message || "The system has detected that continued automatic advancement may fail repeatedly."}</div>
          {circuitRecovery ? <div className="mt-1">suggestion:{circuitRecovery}。</div> : null}
        </section>
      ) : null}
      ...
      {workerHealth ? (
        <section className="rounded-2xl bg-muted/25 px-4 py-3">
          ...
          <div className="mt-3 grid grid-cols-4 gap-3">
            <SummaryMetric label="queue" value={workerHealth.queuedCommandCount} />
            <SummaryMetric label="Error 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know." value={workerHealth.leasedCommandCount} />
            <SummaryMetric label="implement" value={workerHealth.runningCommandCount} />
            <SummaryMetric label="recover" value={workerHealth.staleCommandCount} />
          </div>
          ...
          <div className="space-y-3 text-xs leading-5 text-muted-foreground">
            ...
            {promptUsage.length > 0 ? (
              <div className="space-y-1">
                <div className="font-medium text-foreground">Phase dosage</div>
            ...
            {stepUsage.length > 0 ? (
              <div className="space-y-1">
                <div className="font-medium text-foreground">Advance steps</div>
                <div className="divide-y divide-border/60">
                  {stepUsage.map((item) => (
            ...
            {recentItems.map((item) => (
              <div key={item.id} className="py-2">
                ...
                {item.usage ? (
                  ...
                  <div className="mt-1 text-muted-foreground">time consuming {formatDuration(item.durationMs)}</div>
                ) : null}
