...
阶段状态：P0 收束完成，下一步进入治理兼容与不可达旧逻辑清理
...
- [x] 问题治理生命周期快照已进入 `beta`，当前功能分支已同步至 `origin/beta@f8beac96`。
...
- [ ] 剩余 P0/P1 完成后再次同步到 `beta`，重跑相关定向测试和组合回归。
...
- `client/src/pages/novels/autoDirector/directorCreateStages.ts` 存在未提交工程改动，属于用户工作区内容；必须保留并纳入本次快照，不得擅自还原。
